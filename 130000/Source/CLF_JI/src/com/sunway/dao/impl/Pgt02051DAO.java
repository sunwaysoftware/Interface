 ...
 *
 */
public class Pgt02051DAO extends BaseDAO implements IPgt02051DAO {
	...
	public boolean GetDeleteCommand(Pgt02051 fwcx) throws Exception {
		...
		try {
			...
			call = conn.prepareCall("{call PGSP_DELT02051(?,?,?,?,?,?,?)}");
	...
	public boolean GetInsertCommand(Pgt02051 fwcx) throws Exception {
		...
		try {
			...
			tran = session.beginTransaction();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_ADDT02051(?,?,?,?,?,?,?)}");
			call.setString(1, fwcx.getCd00001Fwcx());
	...
	public boolean GetUpdateCommand(Pgt02051 fwcx) throws Exception {
		...
		try {
			...
			tran = session.beginTransaction();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_UPDT02051(?,?,?,?,?,?,?,?,?)}");
	...
	public ArrayList<Pgv02051> LoadAll(Pgv02051 fwcx) throws Exception {
		ArrayList<Pgv02051> listResult = new ArrayList<Pgv02051>();
		ResultSet rs = null;
		CallableStatement call = null;
		Connection conn = null;
		Session session = null;
		try {
			session = getSession();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_GETALLT02051(?,?,?,?,?,?,?)}");
			//注册输出参数
	 ...
	 * @throws Exception
	 */
	protected Pgv02051 SetVParameters(ResultSet rs) throws Exception {
		Pgv02051 e = new Pgv02051();
	...
	public Pgt02051 LoadByPrimaryKey(Pgt02051 fwcx) throws Exception {
		ArrayList<Pgt02051> listResult = new ArrayList<Pgt02051>();
		ResultSet rs = null;
		CallableStatement call = null;
		Connection conn = null;
		Session session = null;
		try {
			session = getSession();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_GETT02051(?,?,?,?,?,?)}");
			//注册输出参数
			call.registerOutParameter(1, OracleTypes.CURSOR);
			//传入输入参数
			call.setString(2, fwcx.getCd00001Fwcx());
	 ...
	 * @throws Exception
	 */
	protected Pgt02051 SetTParameters(ResultSet rs) throws Exception {
		Pgt02051 e = new Pgt02051();
		e.setCd00001Fwcxlx(rs.getString(CD00001FWCXLX));
		e.setCd00001Fwcx(rs.getString(CD00001FWCX));
		e.setCd00001Szqylx(rs.getString(CD00001SZQYLX));
		e.setCd00001Szqy(rs.getString(CD00001SZQY));
	...
	public boolean ExecuteParamCopy(Pgt02051 fwcx) throws Exception {
		...
		try {	
			...
			tran = session.beginTransaction();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_CZ_02051(?,?,?,?,?)}");
