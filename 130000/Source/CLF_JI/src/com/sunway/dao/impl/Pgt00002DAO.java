...
public class Pgt00002DAO extends BaseDAO implements IPgt00002DAO {
	...
	public boolean GetInsertCommand(Pgt00002 user) throws Exception {
		...
		try {
			...
			call.setDate(8, Common.converDate(user.getLastlockedoutdate()));
			call.setInt(9, user.getPagecount());
			call.setDate(10, Common.converDate(user.getPssd()));
	...
	public boolean GetUpdateCommand(Pgt00002 user) throws Exception {
		...
		try {
			...
			call.setInt(11, user.getPagecount());
			call.setDate(12, Common.converDate(user.getPssd()));
			call.setString(13, user.getRoleids());
			call.setString(14, user.getRightids());
	...
	protected Pgv00002 SetVParameters(ResultSet rs) throws Exception {
		...
		e.setUpddate(Common.formatToYMDHMS(rs.getTimestamp(UPDDATE)));
		...
		e.setUserip(rs.getString(USERIP));
		e.setLastlogindate(Common.formatToYMDHMS(rs.getTimestamp(LASTLOGINDATE)));
		e.setPhone(rs.getString(PHONE));
		e.setIslockedout(rs.getBoolean(ISLOCKEDOUT));
		e.setLastlockedoutdate(Common.formatToYMDHMS(rs.getTimestamp(LASTLOCKEDOUTDATE)));
		...
		try {
			...
			if(rs!=null){
				...
				while(null!=rs && rs.next()){
					...
					sheet.addCell(label);
					label = new Label(9, rowIndex, Common.formatExportDateTime(rs, UPDDATE));
					sheet.addCell(label);
				}
	            workbook.write();   
	            workbook.close();  }
