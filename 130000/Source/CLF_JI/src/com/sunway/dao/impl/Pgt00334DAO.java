 ...
 *
 */
public class Pgt00334DAO extends BaseDAO implements IPgt00334DAO {
	...
	/* (non-Javadoc)
	 * @see com.sunway.dao.IPgt00334DAO#LoadAll(com.sunway.vo.Pgt00334)
	 */
	
	@Override
	public ArrayList<Pgt00334> LoadAll(Pgt00334 bean) throws Exception {
		ArrayList<Pgt00334> listResult = new ArrayList<Pgt00334>();
		ResultSet rs = null;
		...
		try {
			...
			call = conn.prepareCall("{call PGSP_GETALLT00334(?,?)}");
			// 注冊輸出參數
	...
	public ArrayList<Pgt00334> LoadAll_B(Pgt00334 bean) throws Exception {
		...
		try {
			session = getSession();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_GETALLT00334_B(?,?)}");
			// 注冊輸出參數
			call.registerOutParameter(1, OracleTypes.CURSOR);
			// 传入输入参数
			call.setString(2, bean.getCd00302Fcid());
	 ...
	 * @throws Exception
	 */
	protected Pgt00334 SetTParameters(ResultSet rs) throws Exception {
		Pgt00334 e = new Pgt00334();
