 ...
 *
 */
public class Sh10002DAO extends BaseDAO implements ISh10002DAO {

	/* (non-Javadoc)
	 * @see com.sunway.dao.ISh10002DAO#GetExecShAgain(com.sunway.vo.Pgv12001)
	 */
	...
	public Boolean GetExecShAgain(Pgv12001 bean) throws Exception {
		...
		try {
			...
			call = conn.prepareCall("{call PGSP_CZ_10083(?,?,?,?)}");
			//传入输入参数
	...
	/* (non-Javadoc)
	 * @see com.sunway.dao.ISh10002DAO#GetExecShAgainAll(com.sunway.vo.Pgv12001)
	 */
	...
	public Boolean GetExecShAgainAll(Pgv12001 bean) throws Exception {
		...
		try {
			...
			tran = session.beginTransaction();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_CZ_10084(?,?,?,?,?)}");
			//传入输入参数
			call.setString(1, bean.getSwid());
	...
	/* (non-Javadoc)
	 * @see com.sunway.dao.ISh10002DAO#LoadShOK(com.sunway.vo.Pgv12001)
	 */
	...
	public ArrayList<Pgv12001> LoadShOK(Pgv12001 bean) throws Exception {
		...
		try {
			session = getSession();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_GETALLT120012(?,?,?,?,?,?,?)}");
			//注冊輸出參數
