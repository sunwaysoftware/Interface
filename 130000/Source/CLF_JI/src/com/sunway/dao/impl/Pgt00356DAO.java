 ...
 *
 */
public class Pgt00356DAO extends BaseDAO implements IPgt00356DAO {
	...
	public boolean GetDeleteCommand(Pgt00356 wjzs) throws Exception {
		...
		try {
			...
			call = conn.prepareCall("{call PGSP_DELT00356(?,?,?,?,?)}");
	...
	public boolean GetInsertCommand(Pgt00356 wjzs) throws Exception {
		...
		try {
			...
			tran = session.beginTransaction();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_ADDT00356(?,?,?,?,?,?)}");
			call.setString(1, wjzs.getCd00001Szqy());
	...
	public boolean GetUpdateCommand(Pgt00356 wjzs) throws Exception {
		...
		try {
			...
			tran = session.beginTransaction();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_UPDT00356(?,?,?,?,?,?,?)}");
	...
	public ArrayList<Pgv00356> LoadAll(Pgv00356 wjzs) throws Exception {
		ArrayList<Pgv00356> listResult = new ArrayList<Pgv00356>();
		ResultSet rs = null;
		CallableStatement call = null;
		Connection conn = null;
		Session session = null;
		try {
			session = getSession();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_GETALLT00356(?,?,?,?,?,?)}");
			//注册输出参数
			...
			call.setDate(5, Common.converDate(wjzs.getPssd()));
	...
	@Override
	public OutputStream ExportjyjgSjcx(Pgv00356 wjzs) throws Exception {
	 ...
	 * @throws Exception
	 */
	protected Pgv00356 SetVParameters(ResultSet rs) throws Exception {
		Pgv00356 e = new Pgv00356();
	...
	public Pgt00356 LoadByPrimaryKey(Pgt00356 wjzs) throws Exception {
		ArrayList<Pgt00356> listResult = new ArrayList<Pgt00356>();
		ResultSet rs = null;
		CallableStatement call = null;
		Connection conn = null;
		Session session = null;
		try {
			session = getSession();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_GETT00356(?,?,?,?)}");
			//注册输出参数
			call.registerOutParameter(1, OracleTypes.CURSOR);
			//传入输入参数
			call.setString(2, wjzs.getCd00001Szqy());
	 ...
	 * @throws Exception
	 */
	protected Pgt00356 SetTParameters(ResultSet rs) throws Exception {
		Pgt00356 e = new Pgt00356();
		e.setCd00001Szqylx(rs.getString(CD00001SZQYLX));
		e.setCd00001Szqy(rs.getString(CD00001SZQY));
		e.setCd00002Pssd(rs.getString(CD00002PSSD));
		e.setXzxs(rs.getDouble(XZXS));
	...
	public boolean ExecuteParamCopy(Pgt00356 wjzs) throws Exception {
		...
		try {	
			...
			tran = session.beginTransaction();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_CZ_00356(?,?,?,?,?)}");
	...
	public Pgv00356 ImportExcelData(ArrayList<Pgv00356> jyjgzsList) throws Exception{
		...
		try{
			...
			conn = super.getConnection();
			tran = session.beginTransaction();
			call = conn.prepareCall("{call PGSP_ADDT003561(?,?,?,?,?,?,?)}");
			for(int i=0; i<jyjgzsList.size(); i++){
				Pgv00356 jyjgzs = jyjgzsList.get(i);
				try {
					call.registerOutParameter(1, OracleTypes.CURSOR);
					// 传入输入参数
