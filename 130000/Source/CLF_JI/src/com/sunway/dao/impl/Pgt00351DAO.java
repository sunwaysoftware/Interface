...
//	private static final String zh = "ZH";
	...
	public ArrayList<Pgv00351> LoadAll(Pgv00351 v00351) throws Exception {
		...
		try {
			...
			call.setDate(6,  Common.converDate(v00351.getJysj()));
	...
	public boolean GetInsertCommand(Pgt00351 t00351) throws Exception {
		...
		try {
			...
			call.setDate(8, Common.converDate(t00351.getJysj()));
			...
			call.setBoolean(20, Common.checkNull(t00351.getYwjkc()));
			call.execute();
			tran.commit();
			bResult = true;
		} catch (Exception e) {
	...
	public boolean GetUpdateCommand(Pgt00351 t00351) throws Exception {
		...
		try {
			...
			call.setBoolean(19, Common.checkNull(t00351.getYwjkc()));
			call.execute();
			tran.commit();
			bResult = true;
		} catch (Exception e) {
...
//					sheet.addCell(label);
					...
					sheet.addCell(label);
					label = new Label(6, rowIndex, rs.getString("JYSJ"));
					sheet.addCell(label);
	...
	public Boolean ExecCsSame(Pgv00351 v00351) throws Exception {
		...
		try {
			...
			// 输入参数
			call.setString(2, v00351.getSlid());
			call.setDate(3, Common.converDate(v00351.getJysj()));
	...
	public ArrayList<Pgv00351> LoadAllCsSame(Pgv00351 v00351) throws Exception {
		...
		try {
			...
			call.setDate(10, Common.converDate(v00351.getJysj()));
			call.execute();
			// 返回数据集
			rs = (ResultSet) call.getObject(1);
			while (null != rs && rs.next()) {
	...
	public ArrayList<Pgv00351> LoadAllCsSameByBzfID(Pgv00351 v00351) throws Exception {
		...
		try {
			...
			call.setDate(8, Common.converDate(v00351.getJysj()));
			call.execute();
			// 返回数据集
			rs = (ResultSet) call.getObject(1);
			while (null != rs && rs.next()) {
	...
	public ArrayList<Pgv00351> LoadAllCsDiffW(Pgv00351 v00351) throws Exception {
		...
		try {
			...
			call.setInt(3, v00351.getPageSize());
			call.setString(4, v00351.getCd00352Xqdm());
			call.setDate(5, Common.converDate(v00351.getJysj()));
			call.execute();
			// 返回数据集
			rs = (ResultSet) call.getObject(1);
			while (null != rs && rs.next()) {
	...
	public ArrayList<Pgt00352> LoadXqW(Pgt00351 bean) throws Exception {
		...
		try {
			...
			call.setDate(3, Common.converDate(bean.getJysj()));	
		...
		try{
			...
			for(int i=0; i<ebList.size(); i++){
				...
				try {
					...
					call.setDate(9, Common.converDate(bean.getJysj()));
					...
					call.setInt(22, Common.checkNull(bean.getYwjkc()));
...
//					sheet.addCell(label);
					...
					sheet.addCell(label);
					label = new Label(7, rowIndex, Common.formatExportDateTime(rs, upddate));
					sheet.addCell(label);
