package com.sunway.vo;

import java.util.Date;

/**
 * Pgv12010a1Id entity. @author MyEclipse Persistence Tools
 */


public class Pgv12010a2 implements java.io.Serializable {

	private static final long serialVersionUID = -8199651559112194650L;
	private String cjid;
	private String cd12004Mxid;
	private String cd12003Fcid;
	private String cd12002Dcid;
	private String cd12001Swid;
	private String nsrmc;
	private String cd00001Ssgx;
	private String cd00001Szqy;
	private String cd00001Tdyt;
	private String cd12054Tddjid;
	private Double fttdmj;
	private String cd00001Fwyt;
	private String cd00001Jzjg;
	private String jcnf;
	private Double ytjzmj;
	private Double fcyz;
	private String cd00002Pssd;
	private Double pgjg;
	private Boolean pglx;
	private String szqy;
	private String tdyt;
	private String tddj;
	private String fwyt;
	private String jzjg;
	private Double jzqs;
	private Double lsxs;
	private Double jgxgc;
	private Integer pjzqs;
	private Integer plsxs;
	private Integer pjgxgc;
	private Double bl;
	private Double lc;
	private Date upddate;
	private String cd00002Lrr;
	private String note;
	private Integer recordCount;
    private Integer recordIndex;    
    private Integer pageIndex;
    private Integer pageSize; 
    private String jcnfbgn;
    private String jcnfend;
    private String cdjybz;
    
    private Double blMin;
    private Double blMax;
    private Double lcMin;
    private Double lcMax;
	// Constructors

	/**
	 * @return the blMin
	 */
	public Double getBlMin() {
		return blMin;
	}

	/**
	 * @param blMin the blMin to set
	 */
	public void setBlMin(Double blMin) {
		this.blMin = blMin;
	}

	/**
	 * @return the blMax
	 */
	public Double getBlMax() {
		return blMax;
	}

	/**
	 * @param blMax the blMax to set
	 */
	public void setBlMax(Double blMax) {
		this.blMax = blMax;
	}

	/**
	 * @return the lcMin
	 */
	public Double getLcMin() {
		return lcMin;
	}

	/**
	 * @param lcMin the lcMin to set
	 */
	public void setLcMin(Double lcMin) {
		this.lcMin = lcMin;
	}

	/**
	 * @return the lcMax
	 */
	public Double getLcMax() {
		return lcMax;
	}

	/**
	 * @param lcMax the lcMax to set
	 */
	public void setLcMax(Double lcMax) {
		this.lcMax = lcMax;
	}

	/** default constructor */
	public Pgv12010a2() {
	}

	/** minimal constructor */
	public Pgv12010a2(String cjid, String cd12003Fcid, String cd12002Dcid,
			String cd12001Swid, String nsrmc, String cd00001Ssgx,
			String cd00001Szqy, String cd00001Tdyt, String cd12054Tddjid,
			Double fttdmj, String cd00001Fwyt, String cd00001Jzjg, String jcnf,
			Double ytjzmj, Double fcyz, String cd00002Pssd, Double pgjg,
			Boolean pglx, Double jzqs, Double lsxs, Double jgxgc, Double bl,
			Double lc, Date upddate, String cd00002Lrr) {
		this.cjid = cjid;
		this.cd12003Fcid = cd12003Fcid;
		this.cd12002Dcid = cd12002Dcid;
		this.cd12001Swid = cd12001Swid;
		this.nsrmc = nsrmc;
		this.cd00001Ssgx = cd00001Ssgx;
		this.cd00001Szqy = cd00001Szqy;
		this.cd00001Tdyt = cd00001Tdyt;
		this.cd12054Tddjid = cd12054Tddjid;
		this.fttdmj = fttdmj;
		this.cd00001Fwyt = cd00001Fwyt;
		this.cd00001Jzjg = cd00001Jzjg;
		this.jcnf = jcnf;
		this.ytjzmj = ytjzmj;
		this.fcyz = fcyz;
		this.cd00002Pssd = cd00002Pssd;
		this.pgjg = pgjg;
		this.pglx = pglx;
		this.jzqs = jzqs;
		this.lsxs = lsxs;
		this.jgxgc = jgxgc;
		this.bl = bl;
		this.lc = lc;
		this.upddate = upddate;
		this.cd00002Lrr = cd00002Lrr;
	}

	/** full constructor */
	public Pgv12010a2(String cjid, String cd12004Mxid, String cd12003Fcid,
			String cd12002Dcid, String cd12001Swid, String nsrmc,
			String cd00001Ssgx, String cd00001Szqy, String cd00001Tdyt,
			String cd12054Tddjid, Double fttdmj, String cd00001Fwyt,
			String cd00001Jzjg, String jcnf, Double ytjzmj, Double fcyz,
			String cd00002Pssd, Double pgjg, Boolean pglx, String szqy,
			String tdyt, String tddj, String fwyt, String jzjg, Double jzqs,
			Double lsxs, Double jgxgc, Double bl, Double lc, Date upddate,
			String cd00002Lrr, String note) {
		this.cjid = cjid;
		this.cd12004Mxid = cd12004Mxid;
		this.cd12003Fcid = cd12003Fcid;
		this.cd12002Dcid = cd12002Dcid;
		this.cd12001Swid = cd12001Swid;
		this.nsrmc = nsrmc;
		this.cd00001Ssgx = cd00001Ssgx;
		this.cd00001Szqy = cd00001Szqy;
		this.cd00001Tdyt = cd00001Tdyt;
		this.cd12054Tddjid = cd12054Tddjid;
		this.fttdmj = fttdmj;
		this.cd00001Fwyt = cd00001Fwyt;
		this.cd00001Jzjg = cd00001Jzjg;
		this.jcnf = jcnf;
		this.ytjzmj = ytjzmj;
		this.fcyz = fcyz;
		this.cd00002Pssd = cd00002Pssd;
		this.pgjg = pgjg;
		this.pglx = pglx;
		this.szqy = szqy;
		this.tdyt = tdyt;
		this.tddj = tddj;
		this.fwyt = fwyt;
		this.jzjg = jzjg;
		this.jzqs = jzqs;
		this.lsxs = lsxs;
		this.jgxgc = jgxgc;
		this.bl = bl;
		this.lc = lc;
		this.upddate = upddate;
		this.cd00002Lrr = cd00002Lrr;
		this.note = note;
	}

	// Property accessors

	public String getCjid() {
		return this.cjid;
	}

	public void setCjid(String cjid) {
		this.cjid = cjid;
	}

	public String getCd12004Mxid() {
		return this.cd12004Mxid;
	}

	public void setCd12004Mxid(String cd12004Mxid) {
		this.cd12004Mxid = cd12004Mxid;
	}

	public String getCd12003Fcid() {
		return this.cd12003Fcid;
	}

	public void setCd12003Fcid(String cd12003Fcid) {
		this.cd12003Fcid = cd12003Fcid;
	}

	public String getCd12002Dcid() {
		return this.cd12002Dcid;
	}

	public void setCd12002Dcid(String cd12002Dcid) {
		this.cd12002Dcid = cd12002Dcid;
	}

	public String getCd12001Swid() {
		return this.cd12001Swid;
	}

	public void setCd12001Swid(String cd12001Swid) {
		this.cd12001Swid = cd12001Swid;
	}

	public String getNsrmc() {
		return this.nsrmc;
	}

	public void setNsrmc(String nsrmc) {
		this.nsrmc = nsrmc;
	}

	public String getCd00001Ssgx() {
		return this.cd00001Ssgx;
	}

	public void setCd00001Ssgx(String cd00001Ssgx) {
		this.cd00001Ssgx = cd00001Ssgx;
	}

	public String getCd00001Szqy() {
		return this.cd00001Szqy;
	}

	public void setCd00001Szqy(String cd00001Szqy) {
		this.cd00001Szqy = cd00001Szqy;
	}

	public String getCd00001Tdyt() {
		return this.cd00001Tdyt;
	}

	public void setCd00001Tdyt(String cd00001Tdyt) {
		this.cd00001Tdyt = cd00001Tdyt;
	}

	public String getCd12054Tddjid() {
		return this.cd12054Tddjid;
	}

	public void setCd12054Tddjid(String cd12054Tddjid) {
		this.cd12054Tddjid = cd12054Tddjid;
	}

	public Double getFttdmj() {
		return this.fttdmj;
	}

	public void setFttdmj(Double fttdmj) {
		this.fttdmj = fttdmj;
	}

	public String getCd00001Fwyt() {
		return this.cd00001Fwyt;
	}

	public void setCd00001Fwyt(String cd00001Fwyt) {
		this.cd00001Fwyt = cd00001Fwyt;
	}

	public String getCd00001Jzjg() {
		return this.cd00001Jzjg;
	}

	public void setCd00001Jzjg(String cd00001Jzjg) {
		this.cd00001Jzjg = cd00001Jzjg;
	}

	public String getJcnf() {
		return this.jcnf;
	}

	public void setJcnf(String jcnf) {
		this.jcnf = jcnf;
	}

	public Double getYtjzmj() {
		return this.ytjzmj;
	}

	public void setYtjzmj(Double ytjzmj) {
		this.ytjzmj = ytjzmj;
	}

	public Double getFcyz() {
		return this.fcyz;
	}

	public void setFcyz(Double fcyz) {
		this.fcyz = fcyz;
	}

	public String getCd00002Pssd() {
		return this.cd00002Pssd;
	}

	public void setCd00002Pssd(String cd00002Pssd) {
		this.cd00002Pssd = cd00002Pssd;
	}

	public Double getPgjg() {
		return this.pgjg;
	}

	public void setPgjg(Double pgjg) {
		this.pgjg = pgjg;
	}

	public Boolean getPglx() {
		return this.pglx;
	}

	public void setPglx(Boolean pglx) {
		this.pglx = pglx;
	}

	public String getSzqy() {
		return this.szqy;
	}

	public void setSzqy(String szqy) {
		this.szqy = szqy;
	}

	public String getTdyt() {
		return this.tdyt;
	}

	public void setTdyt(String tdyt) {
		this.tdyt = tdyt;
	}

	public String getTddj() {
		return this.tddj;
	}

	public void setTddj(String tddj) {
		this.tddj = tddj;
	}

	public String getFwyt() {
		return this.fwyt;
	}

	public void setFwyt(String fwyt) {
		this.fwyt = fwyt;
	}

	public String getJzjg() {
		return this.jzjg;
	}

	public void setJzjg(String jzjg) {
		this.jzjg = jzjg;
	}

	public Double getJzqs() {
		return this.jzqs;
	}

	public void setJzqs(Double jzqs) {
		this.jzqs = jzqs;
	}

	public Double getLsxs() {
		return this.lsxs;
	}

	public void setLsxs(Double lsxs) {
		this.lsxs = lsxs;
	}

	public Double getJgxgc() {
		return this.jgxgc;
	}

	public void setJgxgc(Double jgxgc) {
		this.jgxgc = jgxgc;
	}

	public Double getBl() {
		return this.bl;
	}

	public void setBl(Double bl) {
		this.bl = bl;
	}

	public Double getLc() {
		return this.lc;
	}

	public void setLc(Double lc) {
		this.lc = lc;
	}

	public Date getUpddate() {
		return this.upddate;
	}

	public void setUpddate(Date upddate) {
		this.upddate = upddate;
	}

	public String getCd00002Lrr() {
		return this.cd00002Lrr;
	}

	public void setCd00002Lrr(String cd00002Lrr) {
		this.cd00002Lrr = cd00002Lrr;
	}

	public String getNote() {
		return this.note;
	}

	public void setNote(String note) {
		this.note = note;
	}

	/**
	 * @return the recordCount
	 */
	public Integer getRecordCount() {
		return recordCount;
	}

	/**
	 * @param recordCount the recordCount to set
	 */
	public void setRecordCount(Integer recordCount) {
		this.recordCount = recordCount;
	}

	/**
	 * @return the recordIndex
	 */
	public Integer getRecordIndex() {
		return recordIndex;
	}

	/**
	 * @param recordIndex the recordIndex to set
	 */
	public void setRecordIndex(Integer recordIndex) {
		this.recordIndex = recordIndex;
	}

	/**
	 * @return the pageIndex
	 */
	public Integer getPageIndex() {
		return pageIndex;
	}

	/**
	 * @param pageIndex the pageIndex to set
	 */
	public void setPageIndex(Integer pageIndex) {
		this.pageIndex = pageIndex;
	}

	/**
	 * @return the pageSize
	 */
	public Integer getPageSize() {
		return pageSize;
	}

	/**
	 * @param pageSize the pageSize to set
	 */
	public void setPageSize(Integer pageSize) {
		this.pageSize = pageSize;
	}

	/**
	 * @return the jcnfbgn
	 */
	public String getJcnfbgn() {
		return jcnfbgn;
	}

	/**
	 * @param jcnfbgn the jcnfbgn to set
	 */
	public void setJcnfbgn(String jcnfbgn) {
		this.jcnfbgn = jcnfbgn;
	}

	/**
	 * @return the jcnfend
	 */
	public String getJcnfend() {
		return jcnfend;
	}

	/**
	 * @param jcnfend the jcnfend to set
	 */
	public void setJcnfend(String jcnfend) {
		this.jcnfend = jcnfend;
	}

	/**
	 * @return the pjzqs
	 */
	public Integer getPjzqs() {
		return pjzqs;
	}

	/**
	 * @param pjzqs the pjzqs to set
	 */
	public void setPjzqs(Integer pjzqs) {
		this.pjzqs = pjzqs;
	}

	/**
	 * @return the plsxs
	 */
	public Integer getPlsxs() {
		return plsxs;
	}

	/**
	 * @param plsxs the plsxs to set
	 */
	public void setPlsxs(Integer plsxs) {
		this.plsxs = plsxs;
	}

	/**
	 * @return the pjgxgc
	 */
	public Integer getPjgxgc() {
		return pjgxgc;
	}

	/**
	 * @param pjgxgc the pjgxgc to set
	 */
	public void setPjgxgc(Integer pjgxgc) {
		this.pjgxgc = pjgxgc;
	}

	/**
	 * @return the cdjybz
	 */
	public String getCdjybz() {
		return cdjybz;
	}

	/**
	 * @param cdjybz the cdjybz to set
	 */
	public void setCdjybz(String cdjybz) {
		this.cdjybz = cdjybz;
	}

}