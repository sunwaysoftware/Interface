package com.sunway.vo;

import java.util.Date;

/**
 * PgvBb00010_b entity. 
 * @author Lee
 */

public class PgvBb00010_b implements java.io.Serializable {

	private static final long serialVersionUID = -8541138775106600923L;
	private String cd00002Pssd;
	private String cd00001Ssgx;
	private String cd00001Tdyt;
	private Double tdse;
	private Double fcse;
	private Double syqmj;
	private Double zjzmj;
	private Double fcpg;
	private Date upddate;
	private String cd00002Czr;
	private String note;
	private Short mxCount;
	private Short pgCount;
	private String tdyt;
	private Integer bm;
	private Boolean sfssgx;

	/** default constructor */
	public PgvBb00010_b() {
	}

	/**
	 * @return the cd00002Pssd
	 */
	public String getCd00002Pssd() {
		return cd00002Pssd;
	}
	/**
	 * @param cd00002Pssd the cd00002Pssd to set
	 */
	public void setCd00002Pssd(String cd00002Pssd) {
		this.cd00002Pssd = cd00002Pssd;
	}
	/**
	 * @return the cd00001Ssgx
	 */
	public String getCd00001Ssgx() {
		return cd00001Ssgx;
	}
	/**
	 * @param cd00001Ssgx the cd00001Ssgx to set
	 */
	public void setCd00001Ssgx(String cd00001Ssgx) {
		this.cd00001Ssgx = cd00001Ssgx;
	}
	/**
	 * @return the cd00001Tdyt
	 */
	public String getCd00001Tdyt() {
		return cd00001Tdyt;
	}
	/**
	 * @param cd00001Tdyt the cd00001Tdyt to set
	 */
	public void setCd00001Tdyt(String cd00001Tdyt) {
		this.cd00001Tdyt = cd00001Tdyt;
	}
	/**
	 * @return the tdse
	 */
	public Double getTdse() {
		return tdse;
	}
	/**
	 * @param tdse the tdse to set
	 */
	public void setTdse(Double tdse) {
		this.tdse = tdse;
	}
	/**
	 * @return the fcse
	 */
	public Double getFcse() {
		return fcse;
	}
	/**
	 * @param fcse the fcse to set
	 */
	public void setFcse(Double fcse) {
		this.fcse = fcse;
	}
	/**
	 * @return the syqmj
	 */
	public Double getSyqmj() {
		return syqmj;
	}
	/**
	 * @param syqmj the syqmj to set
	 */
	public void setSyqmj(Double syqmj) {
		this.syqmj = syqmj;
	}
	/**
	 * @return the zjzmj
	 */
	public Double getZjzmj() {
		return zjzmj;
	}
	/**
	 * @param zjzmj the zjzmj to set
	 */
	public void setZjzmj(Double zjzmj) {
		this.zjzmj = zjzmj;
	}
	/**
	 * @return the fcpg
	 */
	public Double getFcpg() {
		return fcpg;
	}
	/**
	 * @param fcpg the fcpg to set
	 */
	public void setFcpg(Double fcpg) {
		this.fcpg = fcpg;
	}
	/**
	 * @return the upddate
	 */
	public Date getUpddate() {
		return upddate;
	}
	/**
	 * @param upddate the upddate to set
	 */
	public void setUpddate(Date upddate) {
		this.upddate = upddate;
	}
	/**
	 * @return the cd00002Czr
	 */
	public String getCd00002Czr() {
		return cd00002Czr;
	}
	/**
	 * @param cd00002Czr the cd00002Czr to set
	 */
	public void setCd00002Czr(String cd00002Czr) {
		this.cd00002Czr = cd00002Czr;
	}
	/**
	 * @return the note
	 */
	public String getNote() {
		return note;
	}
	/**
	 * @param note the note to set
	 */
	public void setNote(String note) {
		this.note = note;
	}
	/**
	 * @return the mxCount
	 */
	public Short getMxCount() {
		return mxCount;
	}
	/**
	 * @param mxCount the mxCount to set
	 */
	public void setMxCount(Short mxCount) {
		this.mxCount = mxCount;
	}
	/**
	 * @return the pgCount
	 */
	public Short getPgCount() {
		return pgCount;
	}
	/**
	 * @param pgCount the pgCount to set
	 */
	public void setPgCount(Short pgCount) {
		this.pgCount = pgCount;
	}
	/**
	 * @return the tdyt
	 */
	public String getTdyt() {
		return tdyt;
	}
	/**
	 * @param tdyt the tdyt to set
	 */
	public void setTdyt(String tdyt) {
		this.tdyt = tdyt;
	}
	/**
	 * @return the bm
	 */
	public Integer getBm() {
		return bm;
	}
	/**
	 * @param bm the bm to set
	 */
	public void setBm(Integer bm) {
		this.bm = bm;
	}
	/**
	 * @return the sfssgx
	 */
	public Boolean getSfssgx() {
		return sfssgx;
	}
	/**
	 * @param sfssgx the sfssgx to set
	 */
	public void setSfssgx(Boolean sfssgx) {
		this.sfssgx = sfssgx;
	}
}
