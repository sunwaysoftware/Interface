...
public class LogManageAction extends ActionSupport implements SessionAware{
	private static Logger logger = LogManager.getLogger(LogManageAction.class);
	...
	public void validateLogin() {
		if (logger.isDebugEnabled()) {
			logger.debug("validateLogin() ********** start **********"+Common.getCurrentTime());
		}
		...
		logger.info("用户尝试登陆：{}/{}", user.getUserid(), user.getUserpwd());
		try {
			//用户登陆判读
			user = t00002Service.CheckLogin(user);
			if(null!=user && user.getLoginState()==0){
				logger.info("登陆失败：{}", user.getLoginMessage());
		...
		if (logger.isDebugEnabled()) {
			logger.debug("validateLogin() ********** end **********"+Common.getCurrentTime());
		}
	}
	...
	public String login() throws Exception {
		if (logger.isDebugEnabled()) {
			logger.debug("login() ********** start **********"+Common.getCurrentTime());
		}
		try {
			//获得用户session
		...
		if (logger.isDebugEnabled()) {
			logger.debug("login() ********** end **********"+Common.getCurrentTime());
		}
		return SUCCESS;
	}
	...
	public String flash() throws Exception {
		if (logger.isDebugEnabled()) {
			logger.debug("flash() ********** start **********"+Common.getCurrentTime());
		}
		...
		if (logger.isDebugEnabled()) {
			logger.debug("flash() ********** end **********"+Common.getCurrentTime());
		}
		return SUCCESS;
	}
	...
	private void SetAppSession() throws Exception{
		if (logger.isDebugEnabled()) {
			logger.debug("SetAppSession() ********** start **********"+Common.getCurrentTime());
		}
...
//		sessionCtrl.Add(SessionCtrl.SESSION_INFO_CGZK_SC, t00001Service.GetInfoCGZK_SC());
		...
		if (logger.isDebugEnabled()) {
			logger.debug("SetAppSession() ********** end **********"+Common.getCurrentTime());
		}
	}
	...
	private void SetSSGXandSZQYSession() throws Exception{
		if (logger.isDebugEnabled()) {
			logger.debug("SetSSGXandSZQYSession() ********** start **********"+Common.getCurrentTime());
		}
		...
		if (logger.isDebugEnabled()) {
			logger.debug("SetSSGXandSZQYSession() ********** end **********"+Common.getCurrentTime());
		}
	}
