...
public class Pg30001gAction extends ActionSupport implements SessionAware {
	private static Logger logger = LogManager.getLogger(Pg30001gAction.class);
	...
	public String query() throws Exception {
		...
		try {
			...
			v00331.setCd00302Fcid(Common.trim(txtFCIDFind));
			...
			//执行查询
			tabList = g30001Service.LoadPgMx(v00331);
			//分页设置
			...
			logger.error("Pg30001gAction -- query()", e);
	...
	public String execPG() throws Exception {
		if (logger.isDebugEnabled()) {
			logger.debug("execPG() ********** start **********");
		}
		
		//Pgv00331 v00331 = new Pgv00331();
		try {
			//sessionCtrl = new SessionCtrl(ActionContext.getContext().getSession());
			/*v00331.setCd00002Pssd(Common.convertToDate(sessionCtrl.Get(SessionCtrl.SESSION_KEY_PSSD_YMD)));
...
//			t00332Bean = t00332Service.LoadByPrimaryKey(new Pgt00332(txtFCID, sessionCtrl.Get(SessionCtrl.SESSION_KEY_PSSD)));
			...
			logger.error("Pg30001gAction -- execPG()", e);
			this.addActionError(e.getMessage());		
			return INPUT;
		}
		if (logger.isDebugEnabled()) {
			logger.debug("execPG() ********** end **********");
		}
		return SUCCESS;
	}
