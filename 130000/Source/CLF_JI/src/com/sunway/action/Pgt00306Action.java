...
public class Pgt00306Action extends ActionSupport implements SessionAware {
	...
	private ArrayList<Pgv00306> tabList;
	...
	public String query()throws Exception{
		...
		try{
			...
			v00306.setXqnm(Common.getSearchLike(txtXQTIP));
			v00306.setZcdzl(Common.getSearchLike(txtZCDZL));
			...
			tabList = t00306Service.LoadAll(v00306);
			// 分页设置
	...
	public String create() throws Exception{
		...
		try {
			...
			if(!Constant.MOD_CREATE.equals(getACT())){
				...
				if (Common.isNullOrEmpty(v00306Bean.getUpddate())) {
	...
	public void validateUpdate() throws Exception{
		...
		//判断PK是否重复
		if((Constant.MOD_CREATE.equals(getACT()))&&(!Common.isNullOrEmpty(v00306Bean.getUpddate()))){
			this.addActionError(getText("app.msg.error.pk", new String[]{getText("app.xtwh.info.szqy")}));
		}
		//判读数据是否已经被其他用户修改
		if((Constant.MOD_UPDATE.equals(getACT()))&&(!v00306Bean.getUpddate().equals(Common.convertStringToTimestamp(txtUPDATE)))) {
...
//			fileServerPath = getSavePath() + "\\" + getUploadFileName();
	...
	private void setList(Pgv00320 v00320Bean){
		...
		if ((Common.convertToInteger(v00320Bean.getDyh()) > zcsList.size()) || ((Common.convertToInteger(v00320Bean.getDyh()) <= 0)))
		{		
			lostList.add(v00320Bean);
			return;
		}
		ArrayList dyhList = (ArrayList)zcsList.get(Common.convertToInteger(v00320Bean.getDyh())-1);
	...
	public String loadByZCDZL()throws Exception{
		...
		try{
			Pgv00306 v00306 = new Pgv00306();
			v00306.setCd00001Ssgx(sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
			v00306.setNote(Common.trim(txtNOTE));
			v00306.setClh(Common.trim(txtCLH));
...
/*********************************   set &&  get   *****************************************/
	...
	public ArrayList<Pgv00306> getTabList() {
		return tabList;
	}

	public void setTabList(ArrayList<Pgv00306> tabList) {
