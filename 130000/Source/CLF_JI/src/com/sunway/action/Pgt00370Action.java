...
public class Pgt00370Action extends ActionSupport implements SessionAware {
	...
	// 页面所需List && Bean
	private ArrayList<Pgv00370> tabList;
	...
	public void validateUpdate() throws Exception {
		...
		if (Constant.MOD_DELETE.equals(getACT())) {
			t00370.setCd_00001_sz(Common.convertEncoding(txtSZ));
			// t00370.setSe(Common.convertToDouble(txtSE));
		}

		t00370.setFcid(Common.convertEncoding(txtFCID));
		t00370.setCd_00002_czr(sessionCtrl.Get(SessionCtrl.SESSION_KEY_USERID));
		t00370.setNote(Common.convertEncoding(txtNOTE));
		...
		try {
			...
			if (Constant.MOD_CREATE.equals(getACT())) {
				...
				for (int i = 0; i < packageV00370().size(); i++) {
					...
					getT00370Bean().setSe(Common.convertToDouble(bea.getPg_se()));
					t00370Service.GetInsertCommand(getT00370Bean());
				}
				// 插入票号信息
				bea = new Pgv00370();
				bea.setFcid(txtFCID);
				bea.setFpid(Common.trim(txtFPID));
				bea.setSpid(Common.trim(txtSPID));
				bea.setDfspid(Common.trim(txtDFSPID));
	...
	public String execute3701() throws Exception {
		...
		try {
			...
			isAdd = false;
			if (resList.size() > 0) {
				if (!Common.isNullOrEmpty(resList.get(0).getFcid())) {
					isAdd = true;
				}
			}
	...
	public String executeHX() throws Exception {
		...
		StringBuffer strBuf = new StringBuffer(String.format("<FCXX SSQY='%s'/>", sessionCtrl.Get(SessionCtrl.FCBMBM)));
		Double qsV = 0.0;
		Double qtV = 0.0;
		Pgv00370 v00370 = new Pgv00370();
		...
		try {
			...
			if (Common.isNullOrEmpty(sessionCtrl.Get(SessionCtrl.FCJKDZ))) {
				txtHXSign = "Y";
			} else {
				for (int i = 0; i < resList.size(); i++) {
				...
				String strXML = person
						.getHXxml(pgv00302.getFcslh(), Common.checkNull(txtFPID), Common.checkNull(txtSPID),
								...
								String.valueOf(Common.convertToDouble(pgv00302.getScpgjg().toString())/pgv00302.getJzmj()),
						Common.checkNull(txtDFSPID), strBuf.toString());
				String resXML = null;
				try {
	...
	public ArrayList<Pgv00370> getTabList() {
		return tabList;
	}

	public void setTabList(ArrayList<Pgv00370> tabList) {
