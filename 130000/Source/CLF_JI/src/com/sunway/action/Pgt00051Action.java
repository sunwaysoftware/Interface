...
public class Pgt00051Action extends ActionSupport implements SessionAware {
	...
	public String query() throws Exception {
		...
		try {
			...
			//准备查询条件
			v00051.setCd00002Pssd(Common.convertToDate(txtPSSD));
			...
			if(null!=jsblList && jsblList.size()>0){
				rowCount = jsblList.get(0).getRecordCount();
			}else{
				rowCount = 0;
				pageIndex = 1;
			}
	...
	public String create() throws Exception{
		...
		try {
			...
			if(!Constant.MOD_CREATE.equals(getACT())){
				//取得用户选中的数据信息
				t00051.setCd00002Pssd(Common.convertToDate(txtPSSD));
				...
				if (Common.isNullOrEmpty(t00051Bean.getUpddate())) {
	...
	public void validateUpdate() throws Exception{
		...
		t00051Bean.setCd00002Pssd(Common.convertToDate(txtPSSD));
		...
		//判断PK是否重复
		if((Constant.MOD_CREATE.equals(getACT()))&&(!Common.isNullOrEmpty(t00051Bean.getUpddate()))){
			this.addActionError(getText("app.msg.error.pk", new String[]{getText("app.xtwh.t00001.pssd")}));
		}
		//判读数据是否已经被其他用户修改
		if((Constant.MOD_UPDATE.equals(getACT()))&&(!(Common.convertStringToTimestamp(txtUPDATE)).equals(t00051Bean.getUpddate()))) {
			this.addActionError(getText("app.msg.error.pktime"));
		}
		//为数据BEAN赋值
		t00051Bean.setCd00002Czr(sessionCtrl.Get(SessionCtrl.SESSION_KEY_USERID));
		t00051Bean.setJsbl(Common.convertToDouble(txtJSBL));
		t00051Bean.setSl(Common.convertToDouble(txtSL));
		t00051Bean.setNote(Common.convertEncoding(txtNOTE));
