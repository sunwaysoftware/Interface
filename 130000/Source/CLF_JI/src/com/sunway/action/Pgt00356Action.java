...
public class Pgt00356Action extends ActionSupport implements SessionAware {
	...
	//Bean数组
	private ArrayList<Pgv00356> tabList;
	...
	public String query() throws Exception {
		...
		try {
			...
			//v00356.setPssd(Common.convertStringToDate(sessionCtrl.Get(SessionCtrl.SESSION_KEY_PSSD_YMD)));
			...
			//执行查询
			tabList = t00356Service.LoadAll(v00356);
	...
	public String create() throws Exception{
		...
		try {
			...
			if(!Constant.MOD_CREATE.equals(getACT())){
				...
				if (Common.isNullOrEmpty(t00356Bean.getUpddate())) {
	...
	public void validateUpdate() throws Exception{
		...
		//判断PK是否重复
		if((Constant.MOD_CREATE.equals(getACT()))&&(!Common.isNullOrEmpty(t00356Bean.getUpddate()))){
			this.addActionError(getText("app.msg.error.pk", new String[]{getText("app.xtwh.info.szqy")}));
		}
		//判读数据是否已经被其他用户修改
		if((Constant.MOD_UPDATE.equals(getACT()))&&(!t00356Bean.getUpddate().equals(Common.convertStringToTimestamp(txtUPDATE)))) {
			this.addActionError(getText("app.msg.error.pktime"));
		}else{
			//为数据BEAN赋值
			t00356Bean.setCd00002Czr(sessionCtrl.Get(SessionCtrl.SESSION_KEY_USERID));
			t00356Bean.setXzxs(Common.convertToDouble(txtXZXS));
	...
	public String importFile() throws Exception {
		...
		try{
			...
			if(tmpV00356.getOutFlag()==3)
				...
				excelStream = new ByteArrayInputStream(out.toByteArray());
				fileName=new String("格式错误的物价指数修正数据.xls".getBytes("GBK"),"ISO-8859-1");
				return "failexport";
			}
			/*
...
/*********************** set and get ******************************/
	...
	public ArrayList<String> getMonthList() {
		...
		// 从评税时点中截取年，月
		year = Common.convertToInteger(pssdYMD.substring(0, 4));
		month = Common.convertToInteger( pssdYMD.substring(5,7));
		maxMonth = month +12;
		// 循环封装List
	 ...
	 * @return the tabList
	 */
	public ArrayList<Pgv00356> getTabList() {
		return tabList;
	}
	 ...
	 */
	public void setTabList(ArrayList<Pgv00356> tabList) {
