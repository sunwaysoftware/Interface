...
public class Pgt00333Action extends ActionSupport implements SessionAware{
	...
	private ArrayList<Pgt00333> tabList;
	...
	public void validateUpdate() throws Exception {
		...
		//判断PK是否重复
		if((Constant.MOD_CREATE.equals(ACT))&&(!Common.isNullOrEmpty(t00333Bean.getQtxzid()))){
			this.addActionError(getText("app.msg.error.pk", new String[]{getText("app.sjpg.t10033.qtxzid")}));
		}
		//判读数据是否已经被其他用户修改
		if((Constant.MOD_UPDATE.equals(ACT))&&(!t00333Bean.getUpddate().equals(Common.convertStringToTimestamp(txtUPDDATE)))) {
			...
			t00333Bean.setXzlx(Common.convertToInteger(txtXZLX));
			t00333Bean.setXzxs(Common.convertToBigDecimal(txtXZXS));
	...
	public String update() throws Exception {
		...
		try {
			...
			t00331.setCd00302Fcid(Common.convertEncoding(txtFCID));
	...
	protected void readInfo() throws Exception {
		...
		//执行查询
		tabList = t00333Service.LoadAll(new Pgt00333(txtFCID, txtPSSD));
		// 查询历史个案信息
	...
	public String modify() throws Exception {
		...
		try {
			...
			t00333.setXzxs(Common.convertToBigDecimal(txtGAJG));
			t00333.setNote(Common.trim(txtNOTE));
			//执行更新个案操作
	 ...
	 * @return the tabList
	 */
	public ArrayList<Pgt00333> getTabList() {
		return tabList;
	}
	 ...
	 */
	public void setTabList(ArrayList<Pgt00333> tabList) {
