 ...
 *
 */
public class Ss20002Action extends ActionSupport implements SessionAware {
	private static final long serialVersionUID = 7864889615301366308L;
	private ISs20000Service ss20000Service;
	...
	public String executeSsAgain() throws Exception {
		...
		try {
			...
			case 1://選擇重新評估
				...
				//執行重新評估
				if(ss20000Service.GetSsAgainCommand(v00041))
			...
			case 2://全部重新評估
				...
				if(ss20000Service.GetSsAgainAllCommand(v00041))
		...
		try {
			...
			if (!Common.isNullOrEmpty(chkSel)) {
				...
				for (int i = 0; i <= swidList.length - 1; i++) {
					...
					ss00000Bean = ss20000Service.LoadNotice(ss00000);
	...
	public String printcz() throws Exception {
		...
		try {
			...
			if (!Common.isNullOrEmpty(chkSel)) {
				...
				ss00000.setCd00001Czr(sessionCtrl.Get(SessionCtrl.SESSION_KEY_USERID));
				ss00000.setCd00001Ssgx(sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
				ss20000Service.printcz(ss00000);
			}
		} catch (Exception e) {
			LOG.error("printcz()", e);
			this.addActionError(e.getMessage());
	...
	/**
	 * @param ss20000Service the ss20000Service to set
	 */
	public void setSs20000Service(ISs20000Service ss20000Service) {
		this.ss20000Service = ss20000Service;
	}

	/**
	 * @return the ss20000Service
	 */
	@JSON(deserialize=false, serialize=false)
	public ISs20000Service getSs20000Service() {
		return ss20000Service;
	}
