...
public class Pgt00353Action extends ActionSupport implements SessionAware {
	...
	//Bean数组
	private ArrayList<Pgv00353> tabList;
	...
	public String query() throws Exception {
		...
		try {
			//sessionCtrl = new SessionCtrl(ActionContext.getContext().getSession());
			//准备查询条件
			v00353.setCd00001Szqy(Common.trim(ddlSZQYFind));
			v00353.setCd00001Root(ROOT);
			v00353.setCd00001Infoid(Common.trim(txtCGZKFind));
			v00353.setCd00002Pssd(Common.trim(txtPSSDFind));
			...
			//执行查询
			tabList = t00353Service.LoadAll(v00353);
			//分页设置
	...
	public String create() throws Exception{
		...
		try {
			...
			if(!Constant.MOD_CREATE.equals(getACT())){
				...
				if (Common.isNullOrEmpty(t00353Bean.getUpddate())) {
	...
	public void validateUpdate() throws Exception{
		...
		//判断PK是否重复
		if((Constant.MOD_CREATE.equals(getACT()))&&(!Common.isNullOrEmpty(t00353Bean.getUpddate()))){
			this.addActionError(getText("app.msg.error.pk", new String[]{getText("app.xtwh.info.cgzk")}));
		}
		//判读数据是否已经被其他用户修改
		if((Constant.MOD_UPDATE.equals(getACT()))&&(!t00353Bean.getUpddate().equals(Common.convertStringToTimestamp(txtUPDATE)))) {
			this.addActionError(getText("app.msg.error.pktime"));
		}else{
			//为数据BEAN赋值
			t00353Bean.setCd00002Czr(sessionCtrl.Get(SessionCtrl.SESSION_KEY_USERID));
			t00353Bean.setXzxs(Common.convertToDouble(txtXZXS));
	 ...
	 * @return the tabList
	 */
	public ArrayList<Pgv00353> getTabList() {
		return tabList;
	}
	 ...
	 */
	public void setTabList(ArrayList<Pgv00353> tabList) {
