...
public class Pgt00322Action extends ActionSupport implements SessionAware{

	private static final long serialVersionUID = 7965461345971341768L;

	//Service
	private IPgt00322Service t00322Service;
	...
	//Bean数组
	private ArrayList<Pgv00322> tabList;
	...
	private ArrayList<Pgv00322> ebList;
	...
	public String query() throws Exception {
		...
		Pgv00322 v00322 = new Pgv00322();
		try { 
			//sessionCtrl = new SessionCtrl(ActionContext.getContext().getSession());
			//准备查询条件
			v00322.setCd00001Szqy(ddlSZQYFind);
			v00322.setCd00352Xqdm(txtXQFind);
			v00322.setPageIndex(pageIndex);
			v00322.setPageSize(getPageSize());
			v00322.setCd00001Ssgx(sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
			//执行查询
			tabList = t00322Service.LoadAll(v00322);
	...
	public String export() throws Exception {
		...
		try {
			...
			v00322.setCd00001Szqy(ddlSZQYFind);
			v00322.setCd00352Xqdm(txtXQFind);
			v00322.setPageIndex(1);
			v00322.setPageSize(-1);
			v00322.setCd00001Ssgx(sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
			
			ByteArrayOutputStream out = (ByteArrayOutputStream) t00322Service.ExportLCXZSjcx(v00322);
	...
	public String create() throws Exception{
		...
		Pgt00322 t00322 = new Pgt00322();
		try {
			//sessionCtrl = new SessionCtrl(ActionContext.getContext().getSession());
			if(!Constant.MOD_CREATE.equals(getACT())){
				//取得用户选中的数据信息
				t00322.setLc(Common.convertToShort(LC));
				//t00355.setYwdt(YWDT);
				t00322.setCd00001Fwlx(txtFWLX);
				t00322.setZcs(Common.convertToShort(ZCS));
				t00322.setCd00001Szqy(SZQY);
				t00322.setCd00001Szqylx(sessionCtrl.Get(SessionCtrl.SESSION_INFO_SZQY));
				t00322.setCd00002Pssd(Common.trim(PSSD));
				t00322Bean = t00322Service.LoadByPrimaryKey(t00322);
				if (Common.isNullOrEmpty(t00322Bean.getUpddate())) {
	...
	public void validateUpdate() throws Exception{
		...
		//sessionCtrl = new SessionCtrl(ActionContext.getContext().getSession());
		t00322Bean = new Pgt00322();
		this.clearErrorsAndMessages();	
		t00322Bean.setLc(Common.convertToShort(txtLC));
		//t00355Bean.setYwdt(rdoYWDT);
		t00322Bean.setCd00001Fwlx(txtFWLX);
		t00322Bean.setZcs(Common.convertToShort(txtZCS));
		t00322Bean.setCd00001Szqy(ddlSZQY);
		t00322Bean.setCd00001Szqylx(sessionCtrl.Get(SessionCtrl.SESSION_INFO_SZQY));
		t00322Bean.setCd00002Pssd(Common.trim(txtPSSD));
		//根据PK信息，为数据BEAN赋值
		if (!Constant.MOD_DELETE.equals(getACT())){
			t00322Bean = t00322Service.LoadByPrimaryKey(t00322Bean);			
		}
		//判断PK是否重复
		if((Constant.MOD_CREATE.equals(getACT()))&&(!Common.isNullOrEmpty(t00322Bean.getUpddate()))){
			this.addActionError(getText("app.msg.error.pk", new String[]{getText("app.xtwh.info.szqy")}));
		}
		//判读数据是否已经被其他用户修改
		if((Constant.MOD_UPDATE.equals(getACT()))&&(!t00322Bean.getUpddate().equals(Common.convertStringToTimestamp(txtUPDATE)))) {
			this.addActionError(getText("app.msg.error.pktime"));
		}else{
			//为数据BEAN赋值
			t00322Bean.setCd00002Czr(sessionCtrl.Get(SessionCtrl.SESSION_KEY_USERID));
			t00322Bean.setXzxs(Common.convertToDouble(txtXZXS));
			t00322Bean.setNote(txtNOTE);
	...
	public String update() throws Exception {
		...
		try {
			if(Constant.MOD_CREATE.equals(getACT())){			// Create
				if(t00322Service.GetInsertCommand(getT00322Bean()))
					this.addActionMessage(getText(Constant.MSG_CREATE_OK, new String[]{getT00322Bean().getCd00001Szqy()}));
				else
					this.addActionError(getText(Constant.MSG_CREATE_NG, new String[]{getT00322Bean().getCd00001Szqy()}));
			} else if (Constant.MOD_UPDATE.equals(getACT())) {	// Update
				if(t00322Service.GetUpdateCommand(getT00322Bean())){
					t00322Bean = t00322Service.LoadByPrimaryKey(t00322Bean);
					this.addActionMessage(getText(Constant.MSG_UPDATE_OK, new String[]{getT00322Bean().getCd00001Szqy()}));
				}else
					this.addActionError(getText(Constant.MSG_UPDATE_NG, new String[]{getT00322Bean().getCd00001Szqy()}));
			} else if (Constant.MOD_DELETE.equals(getACT())) {	// Delete
				if(t00322Service.GetDeleteCommand(getT00322Bean()))
					this.addActionMessage(getText(Constant.MSG_DELETE_OK, new String[]{getT00322Bean().getCd00001Szqy()}));
				else
					this.addActionError(getText(Constant.MSG_DELETE_NG, new String[]{getT00322Bean().getCd00001Szqy()}));
			}			
			//sessionCtrl = new SessionCtrl(ActionContext.getContext().getSession());
			szqyList = sessionCtrl.ReadSzqyList();
		} catch (Exception e) {
	...
	public String createByAjax() throws Exception{
		...
		try {
			if(!Constant.MOD_CREATE.equals(getACT())){
				//sessionCtrl = new SessionCtrl(ActionContext.getContext().getSession());
				//取得用户选中的数据信息
				t00322.setLc(Common.convertToShort(txtLC));
				t00322.setCd00001Fwlx(txtFWLX);
				//t00355.setYwdt(YWDT);
				t00322.setZcs(Common.convertToShort(txtZCS));
				t00322.setCd00001Szqy(ddlSZQY);
				t00322.setCd00001Szqylx(sessionCtrl.Get(SessionCtrl.SESSION_INFO_SZQY));
				t00322.setCd00002Pssd(Common.trim(PSSD));
				t00322Bean = t00322Service.LoadByPrimaryKey(t00322);
				isExists = t00322Bean.getUpddate() == null?true:false;
			}
		} catch (Exception e) {
			LOG.error(e.getMessage());
			this.addActionError(e.getMessage());
	...
	public String delsel() throws Exception {
		...
		try {
			//sessionCtrl = new SessionCtrl(ActionContext.getContext().getSession());
			// 准备查询条件
			v00322.setChkSel(chkSel);	
			v00322.setCd00002Czr(sessionCtrl.Get(SessionCtrl.SESSION_KEY_USERID));
			v00322.setCd00001Ssgx(sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
			// 执行查询
			t00322Service.GetDeleteSelCommand(v00322);
	...
	public String upload() throws Exception {
		try{
			//sessionCtrl = new SessionCtrl(ActionContext.getContext().getSession());
			fileServerPath = getSavePath() + "\\GPSJ_" + sessionCtrl.getUserId() + getUploadFileName();
			//建立文件上传的输出流
	...
	public void validateImportFile() {
		...
		try{
			//sessionCtrl = new SessionCtrl(ActionContext.getContext().getSession());
			//检验数据合法性
			ebList = Excel.GPExcel(txtFilePath, sessionCtrl.getUserId(),sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
			...
			ex.printStackTrace();
			this.addActionError("挂牌数据不符合导入要求！" + ex.getMessage());
		}
	}
	...
	public String importFile() throws Exception {
		Pgv00322 eBean = new Pgv00322();
		try{
			eBean = t00322Service.ImportExcelData(ebList);
		}catch(Exception ex){
			ex.printStackTrace();
			...
			if(eBean.getOutFlag()==3)
				...
				ByteArrayOutputStream out = (ByteArrayOutputStream) Excel.exportDataGP(eBean.getOutList(),1);
	 ...
	 * @return
	 */
	private Boolean checkDateList(ArrayList<Pgv00322> list){
...
/*********************** set and get ******************************/
	...
	public IPgt00322Service getT00322Service() {
		return t00322Service;
	}

	/**
	 * @param t00355Service the t00355Service to set
	 */
	public void setT00322Service(IPgt00322Service t00322Service) {
		this.t00322Service = t00322Service;
	}
	 ...
	 */
	public void setT00322Bean(Pgt00322 t00322Bean) {
		this.t00322Bean = t00322Bean;
	}
	...
	*/
	 ...
	 * @return the ebList
	 */
	public ArrayList<Pgv00322> getEbList() {
		return ebList;
	}
	 ...
	 */
	public void setEbList(ArrayList<Pgv00322> ebList) {
		this.ebList = ebList;
	}
	 ...
	 * @return the tabList
	 */
	public ArrayList<Pgv00322> getTabList() {
		return tabList;
	}
	 ...
	 */
	public void setTabList(ArrayList<Pgv00322> tabList) {
