...
public class Pgt00301Action extends ActionSupport implements SessionAware{
	...
	public String query() throws Exception {
		...
		try {
			...
			//执行查询
			tabList = t00301Service.LoadAll(v00301);
			//分页设置
	...
	public void validateUpdate() throws Exception {
		...
		// 判断PK是否重复
		if ((Constant.MOD_CREATE.equals(getACT()))
				&& (!Common.isNullOrEmpty(t00301Bean.getSwid()))) {
		...
		try {
			if (Constant.MOD_CREATE.equals(getACT())) { // Create
				if (t00301Service.GetInsertCommand(t00301Bean)) {
					...
					if (Common.checkNull(forward)) SWID = t00301Bean.getSwid();
					t00301Bean.clear();
				} else{
	...
	public String detail() throws Exception {
		...
		try {
			// 准备查询条件
			v00301.setSwid(Common.convertEncoding(SWID));
			// 执行查询
	...
	public String queryCount() throws Exception {
		...
		Pgt00301 t00301 = new Pgt00301(); 
		try {
			t00301.setSwid(Common.convertEncoding(txtSWID));
	...
	public String queryZzdat() throws Exception {
		...
		try {
			//sessionCtrl = new SessionCtrl(ActionContext.getContext().getSession());
			// 准备查询条件
			v00301.setZz(Common.convertEncoding(ZZ) + Constant.STRING_PERCENT);
			v00301.setPageIndex(Common.convertToInteger(Constant.PAGE_FIRST));
			v00301.setPageSize(getPageSize());
