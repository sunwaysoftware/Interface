...
public class Pgt00305yAction extends ActionSupport implements SessionAware {
	...
	public String query() throws Exception {
		...
		try {
			...
			//准备查询条件
			v00305.setCd00301Swid(Common.getSearchLike(txtSWIDFind));
			v00305.setNsrmc(Common.getSearchLike(txtNSRMCFind));
	...
	public String detail() throws Exception {
		...
		try {
			// 准备查询条件
			v00305Bean.setCd00302Fcid(Common.convertEncoding(FCID));
		} catch (Exception e) {
			this.addActionError(e.getMessage());
			return ERROR;
		}
	...
	public String ztdetail() throws Exception {
		...
		try {
			//sessionCtrl = new SessionCtrl(ActionContext.getContext().getSession());
			// 准备查询条件
			v00305.setCd00302Fcid(Common.convertEncoding(FCID));
			v00305.setPageIndex(Common.convertToInteger(Constant.PAGE_FIRST));
			v00305.setPageSize(getPageSize());
			v00305.setCd00001Ssgx(sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
			v00305.setCd00002Pssd(sessionCtrl.Get(SessionCtrl.SESSION_KEY_PSSD));
			v00305.setSczt(Common.convertToInteger(Constant.ZT_DEFAULT));
			// 执行查询
			yList = t00305yService.LoadAll(v00305);
			if (null != yList && yList.size() > 0) {
	...
	public String backup() throws Exception{
		...
		try {
			...
			v00305.setCd00302Fcid(Common.convertEncoding(Common.trim(FCID)));
			// 执行备份
