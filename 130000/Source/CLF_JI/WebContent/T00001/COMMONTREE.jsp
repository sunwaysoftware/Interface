...
<s:else>
	...
	<p>父节点： <s:iterator value="navList" id="navEntity" status="index">
		<s:if test="!#index.last">
			<a href="javascript:void(0)" class="expanded" expINFOID="<s:property value="infoid"/>" > 
				<s:property value="infonm" />
			</a> &gt;&gt;
		</s:if>
		<s:if test="#index.last">
		...
		<s:else>
			<ul>
				<li>
                    ...
                    <a href="javascript:void(0)" title="<s:property value="note"/>" class="expanded" expINFOID="<s:property value="infoid"/>"><s:property value="infoid"/>:<s:property value="infonm" /></a>
				</li>
			</ul>
		</s:else>
	</s:iterator>
