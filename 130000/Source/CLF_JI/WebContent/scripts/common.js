...
{
	var iheight = $(document).height()-35;  
	//鍒犻櫎鍒楄〃閲屼笉璁＄畻鐨勬帶浠堕珮搴�
...
function addZHXZ(cd00053Qtxzid,ParentId,Qtxznm,IsDir,IsId){
	...
	if (IsDir)
		{
		obj.append("<span class=\"labwidth\" style=\"width:160px;\"><span class=\"spanJG\" style=\"width:60px;\">" +Qtxznm +"</span><select id='ZH" + cd00053Qtxzid + "' name='ZH" + cd00053Qtxzid + "' class=\"txtfocus txtSelect ZHXZ\"><option value=\"\" parentid=\"\">请选择...</option></select></span>");
		}
	else
		{
		if (IsId)
...
$(document).ready(function() {	
	...
	if ($('#bb').length>0)
	{
	  $('#bb').dialog({
        ...
        iconCls:"icon-help",
        open: function() { 
		  $("body > div[role=dialog]").appendTo("#findForm");  
        	    },
	       buttons:[{
				text:'确定',
				iconCls:'icon-ok',
...
{
	var fs = top.document.getElementById("Pz_frame");
	if (displayBar)
	{		
		fs.rows="22,*";
		displayBar=false;
...
function switchBarLeft(obj)
{
	var fs = parent.document.getElementById("Pz_Leftframe");
	if (displayBar)
	{		
		fs.cols="0,8,*";
		displayBar=false;
		obj.src="../images/open_center.jpg";
		obj.title="打开菜单";
	}
	else{
		fs.cols="170,8,*";
		displayBar=true;
		obj.src="../images/close_center.jpg";
		obj.title="关闭菜单";
	}
}
