package com.sunway.vo;

public class PgtFCXX_W {
	private String fcslh;
	private String fpid;
	private String spid;
	private String sfms;
	private String sfzs;
	private Double pgjg;
	private Double qsjg;
	private Double qtjg;
	private String bz;
	private String OINSID;
	private String ROOMID;
	private Integer isHX;
	private Double jyjg;
	private Double price;
	private String ownroomid;
	//------------------
	private String pgid;
	private String ssqy;
	private String dfspid;
	private Double djz_qs;		//契税
	private Double djz_yys;		//营业税
	private Double djz_cjs;		//城市维护建设税
	private Double djz_dfjys;	//教育费附加、地方教育附加
	private Double djz_grsds;	//个人所得税
	private Double djz_yhs;		//印花税
	private Double djz_tdzzs;	//土地增值税
	
	public String getFcslh() {
		return fcslh;
	}
	public void setFcslh(String fcslh) {
		this.fcslh = fcslh;
	}
	public String getFpid() {
		return fpid;
	}
	public void setFpid(String fpid) {
		this.fpid = fpid;
	}
	public String getSpid() {
		return spid;
	}
	public void setSpid(String spid) {
		this.spid = spid;
	}
	public String getSfms() {
		return sfms;
	}
	public void setSfms(String sfms) {
		this.sfms = sfms;
	}
	public String getSfzs() {
		return sfzs;
	}
	public void setSfzs(String sfzs) {
		this.sfzs = sfzs;
	}
	public Double getPgjg() {
		return pgjg;
	}
	public void setPgjg(Double pgjg) {
		this.pgjg = pgjg;
	}
	public Double getQsjg() {
		return qsjg;
	}
	public void setQsjg(Double qsjg) {
		this.qsjg = qsjg;
	}
	public Double getQtjg() {
		return qtjg;
	}
	public void setQtjg(Double qtjg) {
		this.qtjg = qtjg;
	}
	public String getBz() {
		return bz;
	}
	public void setBz(String bz) {
		this.bz = bz;
	}
	public String getOINSID() {
		return OINSID;
	}
	public void setOINSID(String oINSID) {
		OINSID = oINSID;
	}
	public String getROOMID() {
		return ROOMID;
	}
	public void setROOMID(String rOOMID) {
		ROOMID = rOOMID;
	}
	public Integer getIsHX() {
		return isHX;
	}
	public void setIsHX(Integer isHX) {
		this.isHX = isHX;
	}
	public Double getJyjg() {
		return jyjg;
	}
	public void setJyjg(Double jyjg) {
		this.jyjg = jyjg;
	}
	public Double getPrice() {
		return price;
	}
	public void setPrice(Double price) {
		this.price = price;
	}
	public String getOwnroomid() {
		return ownroomid;
	}
	public void setOwnroomid(String ownroomid) {
		this.ownroomid = ownroomid;
	}
	/**
	 * @return the pgid
	 */
	public String getPgid() {
		return pgid;
	}
	/**
	 * @param pgid the pgid to set
	 */
	public void setPgid(String pgid) {
		this.pgid = pgid;
	}
	/**
	 * @return the ssqy
	 */
	public String getSsqy() {
		return ssqy;
	}
	/**
	 * @param ssqy the ssqy to set
	 */
	public void setSsqy(String ssqy) {
		this.ssqy = ssqy;
	}
	/**
	 * @return the dfspid
	 */
	public String getDfspid() {
		return dfspid;
	}
	/**
	 * @param dfspid the dfspid to set
	 */
	public void setDfspid(String dfspid) {
		this.dfspid = dfspid;
	}
	/**
	 * @return the djz_qs
	 */
	public Double getDjz_qs() {
		return djz_qs;
	}
	/**
	 * @param djz_qs the djz_qs to set
	 */
	public void setDjz_qs(Double djz_qs) {
		this.djz_qs = djz_qs;
	}
	/**
	 * @return the djz_yys
	 */
	public Double getDjz_yys() {
		return djz_yys;
	}
	/**
	 * @param djz_yys the djz_yys to set
	 */
	public void setDjz_yys(Double djz_yys) {
		this.djz_yys = djz_yys;
	}
	/**
	 * @return the djz_cjs
	 */
	public Double getDjz_cjs() {
		return djz_cjs;
	}
	/**
	 * @param djz_cjs the djz_cjs to set
	 */
	public void setDjz_cjs(Double djz_cjs) {
		this.djz_cjs = djz_cjs;
	}
	/**
	 * @return the djz_dfjys
	 */
	public Double getDjz_dfjys() {
		return djz_dfjys;
	}
	/**
	 * @param djz_dfjys the djz_dfjys to set
	 */
	public void setDjz_dfjys(Double djz_dfjys) {
		this.djz_dfjys = djz_dfjys;
	}
	/**
	 * @return the djz_grsds
	 */
	public Double getDjz_grsds() {
		return djz_grsds;
	}
	/**
	 * @param djz_grsds the djz_grsds to set
	 */
	public void setDjz_grsds(Double djz_grsds) {
		this.djz_grsds = djz_grsds;
	}
	/**
	 * @return the djz_yhs
	 */
	public Double getDjz_yhs() {
		return djz_yhs;
	}
	/**
	 * @param djz_yhs the djz_yhs to set
	 */
	public void setDjz_yhs(Double djz_yhs) {
		this.djz_yhs = djz_yhs;
	}
	/**
	 * @return the djz_tdzzs
	 */
	public Double getDjz_tdzzs() {
		return djz_tdzzs;
	}
	/**
	 * @param djz_tdzzs the djz_tdzzs to set
	 */
	public void setDjz_tdzzs(Double djz_tdzzs) {
		this.djz_tdzzs = djz_tdzzs;
	}
}
