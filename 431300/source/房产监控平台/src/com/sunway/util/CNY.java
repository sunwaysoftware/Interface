 ...
 * 
 */
public class CNY implements Serializable {

	private static final long serialVersionUID = 8079441440303545600L;
	 ...
	 * @return 大写人民币
	 */
	public static String convertToCNY(Double number) {
		...
		for (int i = tempNumberLength; i > 0; i--) {
			if ((tempNumberLength - i + 2) % 4 == 0) {
				// 如果在（元，万，亿，万）位上的四个数都为零,如果标志为未读零，则只读零，如果标志为已读零，则略过这四位
				...
				// 如果当前位在（元，万，亿，万）位上，则设置标志为已读零（即重置读零标志）
				preReadZero = true;
			}
			...
			if (digit == 0) {
				...
				// 如果当前位是零并且在（元，万，亿，万）位上，则读出（元，万，亿，万）
				if ((tempNumberLength - i + 2) % 4 == 0) {
