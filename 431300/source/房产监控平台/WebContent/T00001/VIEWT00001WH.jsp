...
<script type="text/javascript" src="../scripts/T00001/VIEWT00001WH.js"></script>
	...
	$(function() {
		...
		$('#test').datagrid({
			...
			queryParams:{
				...
				txtINFOID : $("#txtINFOID").val()
			},
			frozenColumns:[[
				{title:'编辑',field:'edit',width:35,align:'center',formatter:function(value,rec){
					var name;
					<s:if test="%{viewKey == '014'}">
						name = "区域维护";
					</s:if>
					<s:elseif test="%{viewKey == '003'}">
   						name = "税务机关";
					</s:elseif>
					<s:elseif test="%{viewKey == '000'}">
						name = "综合修正";
					</s:elseif>
					return "<a href=\"javascript:showpParentWin('["+name+"编辑]','xtwh/ADDT00001WH.action?viewKey=<s:property value='viewKey' />&ACT=U&ROOTID=" + rec.rootid + "&INFOID="+rec.infoid+"','frameCSSZBG'); \"><img src=\"../images/ico/Edit.gif\" title=\"编辑\" alt=\"编辑\" width=\"16\" height=\"16\" align=\"absmiddle\" /></a>";
				...
				{title:'删除',field:'del',width:35,align:'center',formatter:function(value,rec){
					return "<a href=ADDT00001WH.action?viewKey=<s:property value='viewKey' />&ACT=D&ROOTID=" + rec.rootid + "&INFOID="+rec.infoid+"><img src=\"../images/ico/Delete.gif\" title=\"删除\" alt=\"删除\" width=\"16\" height=\"16\" align=\"absmiddle\" /></a>";
				}}
			]],
			columns:[[
    ...
    <div id="tabs" class="ui-tabs ui-widget ui-widget-content ui-corner-all">
   		<div class="datagrid-title">
			<span class="datagrid-title-text">
				...
				<div id="w" style="width:350px;height:420;padding:5px;background: #fafafa;">
				<div id="sxkz" style="display:none;">
				<form id="findForm" method="post" target="_blank">
			...
			<input type="hidden" name="ddlROOTIDFind" id="ddlROOTIDFind" value="<s:property value="ddlROOTIDFind" />" />
			<input type="hidden" name="txtINFOID" id="txtINFOID" />
			<input type="hidden" name="viewKey" id="viewKey" value="<s:property value="viewKey" />" />
			    <table width="300" border="0" cellspacing="0" cellpadding="0" >
			      <!--<tr>
			        <td>&nbsp;<s:property value="%{getText('app.xtwh.t00001.rootnm')}" /> </td>
			        <td>&nbsp;<select name="ddlROOTIDFind" id="ddlROOTIDFind">
					<option value="<s:property value="ddlROOTIDFind" />">全部</option>
...
<div>
	...
	<a href="ADDT00001WH.action?ACT=C&viewKey=<s:property value="viewKey" />" class="easyui-linkbutton" plain="true" iconCls="icon-add"><s:property value="getText('app.button.add')"/></a>
</div>
</div>
</div>
    </td>
