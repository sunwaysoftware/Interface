...
public class Pgt00003Action extends ActionSupport implements SessionAware {
	...
	//Bean数组
	private ArrayList<Pgv00003> rows;
	//查询条件
	...
	public String query() throws Exception {
		...
		try {
			...
			setISADMIN(ConvertUtil.toBoolean(sessionCtrl.Get(SessionCtrl.SESSION_KEY_ISADMIN)));
			//准备查询条件
			v00003.setRolenm(FormatUtil.toSearchLike(txtROLENMFind));
			...
			//执行查询
			rows = t00003Service.LoadAll(v00003);
			//分页设置
	...
	public void validateUpdate() throws Exception{
		...
		//判断PK是否重复
		if((Constant.MOD_CREATE.equals(getACT()))&&(!CheckUtil.chkEmpty(t00003Bean.getRoleid()))){
			this.addActionError(getText("app.msg.error.pk", new String[]{getText("app.xtwh.t00003.roleid")}));
		}
		//判读数据是否已经被其他用户修改
		if((Constant.MOD_UPDATE.equals(getACT()))&&(!t00003Bean.getUpddate().equals(ConvertUtil.toTimestamp(txtUPDATE)))) {
	...
	public String LoadRolesByUser() throws Exception{
		...
		try {
			...
			rows = t00003Service.LoadRoleByUserID(t00002Bean);
		} catch (Exception e) {
			LOG.error(e.getMessage());
			this.addActionError(e.getMessage());
	...
	public String LoadRolesByRight() throws Exception{
		...
		try {
			...
			rows = t00003Service.LoadRoleByRightID(t00004Bean);
		} catch (Exception e) {
			LOG.error(e.getMessage());
			this.addActionError(e.getMessage());
	 ...
	 * @return the rows
	 */
	public ArrayList<Pgv00003> getRows() {
		return rows;
	}
	 ...
	 */
	public void setRows(ArrayList<Pgv00003> rows) {
		this.rows = rows;
	}
