...
public class Pgt00354Action extends ActionSupport implements SessionAware {
	...
	public String query() throws Exception {
		...
		try {
			//sessionCtrl = new SessionCtrl(ActionContext.getContext().getSession());
			//准备查询条件
			v00354.setCd00001Szqy(CheckUtil.chkTrim(ddlSZQYFind));
			v00354.setCd00001Fwcx(CheckUtil.chkTrim(txtFWCXFind));
			v00354.setCd00002Pssd(CheckUtil.chkTrim(txtPSSDFind));
			...
			if(null!=fwcxList && fwcxList.size()>0){
				total = fwcxList.get(0).getRecordCount();
			}else{
				total = 0;
				pageIndex = 1;
			}
	...
	public String create() throws Exception{
		...
		try {
			...
			if(!Constant.MOD_CREATE.equals(getACT())){
				...
				if (CheckUtil.chkEmpty(t00354Bean.getUpddate())) {
	...
	public void validateUpdate() throws Exception{
		...
		//判断PK是否重复
		if((Constant.MOD_CREATE.equals(getACT()))&&(!CheckUtil.chkEmpty(t00354Bean.getUpddate()))){
			this.addActionError(getText("app.msg.error.pk", new String[]{getText("app.xtwh.info.fwcx")}));
		}
		//判读数据是否已经被其他用户修改
		if((Constant.MOD_UPDATE.equals(getACT()))&&(!t00354Bean.getUpddate().equals(ConvertUtil.toTimestamp(txtUPDATE)))) {
			this.addActionError(getText("app.msg.error.pktime"));
		}else{
			//为数据BEAN赋值
			t00354Bean.setCd00002Czr(sessionCtrl.Get(SessionCtrl.SESSION_KEY_USERID));
			t00354Bean.setXzxs(ConvertUtil.toDouble(txtXZXS));
