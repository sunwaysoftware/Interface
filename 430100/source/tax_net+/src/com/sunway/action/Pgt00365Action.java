...
public class Pgt00365Action extends ActionSupport implements SessionAware {
	...
	//view
	private ArrayList<Pgt00365> rows;
	...
	public String query() throws Exception {
		...
		try {
			...
			t00365.setPageSize(ConvertUtil.toInteger(sessionCtrl.Get(SessionCtrl.SESSION_KEY_DATASIZE)));
			t00365.setCd00001Ssgx(sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
			//执行查询
			rows = t00365Service.LoadAll(t00365);
			//分页设置
	...
	public void validateUpdate() throws Exception{
		...
		//判断PK是否重复
		if((Constant.MOD_CREATE.equals(getACT()))&&(!CheckUtil.chkEmpty(t00365Bean.getUpddate()))){
			this.addActionError(getText("app.msg.error.pk", new String[]{getText("app.xtwh.info.szqy")}));
		}
		//判读数据是否已经被其他用户修改
		if((Constant.MOD_UPDATE.equals(getACT()))&&(!t00365Bean.getUpddate().equals(ConvertUtil.toTimestamp(txtUPDATE)))) {
			this.addActionError(getText("app.msg.error.pktime"));
		}else{
			//为数据BEAN赋值
			t00365Bean.setCd00001Szqy(ddlSZQY);
			t00365Bean.setCd00352Xqdm(txtXQDM);
			t00365Bean.setXzxs(ConvertUtil.toInteger(rdoJJF));
	 ...
	 * @return the rows
	 */
	public ArrayList<Pgt00365> getRows() {
		return rows;
	}
	/**
	 * @param rows the rows to set
	 */
	public void setRows(ArrayList<Pgt00365> rows) {
		this.rows = rows;
	}
	/**
