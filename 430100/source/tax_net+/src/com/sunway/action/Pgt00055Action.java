...
public class Pgt00055Action extends ActionSupport implements SessionAware {
	...
	private ArrayList<Pgt00055> rows;
	//Edit
	...
	public void validateUpdate() throws Exception{
		...
		//判断PK是否重复
		if((Constant.MOD_CREATE.equals(getACT()))&&(!CheckUtil.chkEmpty(t00055Bean.getUpddate()))){
			this.addActionError(getText("app.msg.error.pk", new String[]{getText("app.xtwh.t00055.zjlx")}));
		}
		//判读数据是否已经被其他用户修改
		if((Constant.MOD_UPDATE.equals(getACT()))&&(!t00055Bean.getUpddate().equals(ConvertUtil.toTimestamp(txtUPDATE)))) {
			this.addActionError(getText("app.msg.error.pktime"));
		}else{
			//为数据BEAN赋值
			t00055Bean.setCd00001Zjlx(txtZJLXId);
			t00055Bean.setCd00001Zjlxlx(sessionCtrl.Get(SessionCtrl.SESSION_INFO_ZJLX));
			t00055Bean.setYxws(ConvertUtil.toByte(txtYXWS));
	...
	public String isYxws() throws Exception {
		...
		try {
			...
			t00055.setYxws(ConvertUtil.toByte(txtYXWS));
	 ...
	 * @return the rows
	 */
	public ArrayList<Pgt00055> getRows() {
		return rows;
	}
	 ...
	 */
	public void setRows(ArrayList<Pgt00055> rows) {
		this.rows = rows;
	}
