...
public class Pgt00009Action extends ActionSupport implements SessionAware {
	...
	public String query() throws Exception {
		...
		try {
			...
			//准备查询条件
			v00009.setCd00002Userid(FormatUtil.toSearchLike(ConvertUtil.encoding(txtUSERIDFind)));
			v00009.setUsernm(FormatUtil.toSearchLike(ConvertUtil.encoding(txtUSERNMFind)));
			...
			if(null!=ssgxList && ssgxList.size()>0){
				total = ssgxList.get(0).getRecordCount();
			}else{
				total = 0;
				pageIndex = 1;
			}
	...
	public void validateUpdate() throws Exception{
		...
		//判断PK是否重复
		if((Constant.MOD_CREATE.equals(getACT()))&&(!CheckUtil.chkEmpty(t00009Bean.getUpddate()))){
			//如果重复数据，则将操作符号修改为更新
			//setACT(Constant.MOD_UPDATE);
			setTxtUPDATE(t00009Bean.getUpddate().toString());
		}
		//判读数据是否已经被其他用户修改
		if((Constant.MOD_UPDATE.equals(getACT()))&&(!t00009Bean.getUpddate().equals(ConvertUtil.toTimestamp(txtUPDATE)))) {
			this.addActionError(getText("app.msg.error.pktime"));
		}else{
			t00009Bean.setCd00002Userid(txtUSERID);
			t00009Bean.setCd00002Czr(sessionCtrl.Get(SessionCtrl.SESSION_KEY_USERID));
			t00009Bean.setNote(ConvertUtil.encoding(txtNOTE));
