...
public class Pgt00384Action extends ActionSupport implements SessionAware {
	...
	private ArrayList<Pgv00384> rows;
	private String fcId;
	...
	public String execute() throws Exception {
		...
		try {
			...
			rows = getT00384Service().LoadAll(bean);		
	...
	public String loadYY() throws Exception {
		...
		try {
			...
			rows = getT00384Service().LoadYY(bean);			
		} catch (Exception e) {
			LOG.error("loadYY()", e);
			this.addActionError(e.getMessage());
			if (LOG.isDebugEnabled()) {
	...
	public String executeT00320()throws Exception{
		...
		try {
			...
			bean.setCd00002Pssd(sessionCtrl.Get(SessionCtrl.SESSION_KEY_PSSD));
			bean.setShlx(ConvertUtil.toInteger(type));
			rows = getT00384Service().LoadQMPG(bean);		
	 ...
	 * @return the rows
	 */
	public ArrayList<Pgv00384> getRows() {
		return rows;
	}
	 ...
	 */
	public void setRows(ArrayList<Pgv00384> rows) {
		this.rows = rows;
	}
