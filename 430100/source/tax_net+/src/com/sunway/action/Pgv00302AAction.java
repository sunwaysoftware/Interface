...
public class Pgv00302AAction extends ActionSupport implements SessionAware {
	...
	private ArrayList<Pgv00302A> rows;
	// 检索条件
	...
	public String query() throws Exception {
		...
		try {
			...
			v00302A.setFcid(ConvertUtil.encoding(CheckUtil.chkTrim(txtFCIDFind)));
			v00302A.setCd00301aSwid(FormatUtil.toSearchLike(ConvertUtil.encoding(txtSWIDFind)));
			v00302A.setNsrmc(FormatUtil.toSearchLike(ConvertUtil.encoding(txtNSRMCFind)));
			v00302A.setFdcdat(FormatUtil.toSearchLike(ConvertUtil.encoding(txtFDCDATFind)));
			v00302A.setCd00001Szqy(ddlSZQYFind);
			v00302A.setCd00352Xqdm(ConvertUtil.encoding(CheckUtil.chkTrim(txtXQFind)));
			v00302A.setFwtdzl(FormatUtil.toSearchLike(ConvertUtil.encoding(txtFWTDZLFind)));
			// 执行查询
			rows = v00302AService.LoadAll(v00302A);
			// 分页设置
	...
	public String list() throws Exception {
		...
		try {
			...
			v00302A.setPageSize(getPageSize());
			v00302A.setCd00001Ssgx(sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
			v00302A.setCzh(CheckUtil.chkTrim(CZH));
			// 执行查询
			rows = v00302AService.LoadAll(v00302A);
			// 分页设置
			if (null != rows && rows.size() > 0) {
	...
	public String detail() throws Exception {
		...
		try {
			//sessionCtrl = new SessionCtrl(ActionContext.getContext().getSession());
			// 准备查询条件
			v00302A.setPageIndex(ConvertUtil.toInteger(Constant.PAGE_FIRST));
			v00302A.setPageSize(getPageSize());
			v00302A.setCd00001Ssgx(sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
			v00302A.setCzh(CheckUtil.chkTrim(CZH));
			v00302A.setFcid(ConvertUtil.encoding(CheckUtil.chkTrim(FCID)));
			// 执行查询
			rows = v00302AService.LoadAll(v00302A);
			// 取得详细
			if (null != rows && rows.size() > 0) {
				v00302aBean = rows.get(0);
			}
		} catch (Exception e) {
			this.addActionError(e.getMessage());
			return ERROR;
	 ...
	 * @return the rows
	 */
	public ArrayList<Pgv00302A> getRows() {
		return rows;
	}
	/**
	 * @param rows the rows to set
	 */
	public void setRows(ArrayList<Pgv00302A> rows) {
		this.rows = rows;
	}
	/**
