...
public class Pgt00303Action extends ActionSupport implements SessionAware {
	...
	// Bean数组
	private ArrayList<Pgv00303> rows;
	// 检索条件
	...
	public String query() throws Exception {
		...
		try {
			...
			v00303.setCd00352Xqdm(CheckUtil.chkTrim(txtXQFind));
			v00303.setZcdzl(FormatUtil.toSearchLike(txtZCDZL));
			v00303.setPageIndex(pageIndex);
			v00303.setPageSize(getPageSize());
			v00303.setSsgx(sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
			// 执行查询
			rows = t00303Service.LoadAll(v00303);
			// 分页设置
	...
	public String create() throws Exception {
		...
		try {
			...
			if (Constant.MOD_CREATE.equals(getACT())
					&& !CheckUtil.chkEmpty(ddlSZQY)
					&& !CheckUtil.chkEmpty(txtXQDM)) {
	...
	public String queryLhdzdat() throws Exception {
		...
		try {
			...
			v00303.setLh(FormatUtil.toSearchLike(txtLH));
			// 执行查询
	...
	public String queryDyhdzdat() throws Exception {
		...
		try {
			...
			v00303.setZcdzl(txtZCDZL);
			v00303.setLh(txtLH);
			v00303.setDyh(FormatUtil.toSearchLike(txtDyh));
			// 执行查询
	...
	public String queryFhdzdat() throws Exception {
		...
		try {
			...
			v00303.setFh(FormatUtil.toSearchLike(txtFh));
			// 执行查询
	...
	public String beforeCreateByAjax() throws Exception{
		...
		try {
			// 准备查询条件
			v00303.setZcdzl(ConvertUtil.encoding(txtZCDZL));
			v00303.setSsgx(sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
			// 执行查询
	...
	public String hbUpdate() throws Exception {
		...
		try {
			...
			t00303Bean.setZlc(ConvertUtil.toShort("7"));
			t00303Bean.setSsgx(sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
	...
	public String findFwdz() throws Exception {
		...
		try {
			...
			v00303Bean.setZcdzbm(ConvertUtil.encoding(txtZCDZBM));
...
//			setExcelStream(new ByteArrayInputStream(out.toByteArray()));
	 ...
	 * @return the rows
	 */
	public ArrayList<Pgv00303> getRows() {
		return rows;
	}
	 ...
	 */
	public void setRows(ArrayList<Pgv00303> rows) {
		this.rows = rows;
	}
