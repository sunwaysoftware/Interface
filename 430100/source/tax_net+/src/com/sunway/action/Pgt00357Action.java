...
public class Pgt00357Action extends ActionSupport implements SessionAware {
	...
	// Bean数组
	private ArrayList<Pgv00357> rows;
	...
	public String query() throws Exception {
		...
		try {
			// 准备查询条件
			v00357.setSlid(FormatUtil.toSearchLike(ConvertUtil.encoding(txtSLIDFind)));
			v00357.setCd00301Swid(FormatUtil.toSearchLike(CheckUtil.chkTrim(txtSWIDFind)));
			v00357.setNsrmc(FormatUtil.toSearchLike(txtNSRMCFind));
			v00357.setFdcdat(CheckUtil.chkTrim(txtFDCDATFind));
			v00357.setCd00001Szqy(ddlSZQYFind);
			v00357.setCd00352Xqdm(CheckUtil.chkTrim(txtXQFind));
			v00357.setZcdzl(FormatUtil.toSearchLike(txtZCDZL));
			v00357.setPageIndex(pageIndex);
			v00357.setPageSize(getPageSize());
			v00357.setSsgx(sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
			// 执行查询
			rows = t00357Service.LoadAll(v00357);
			// 分页设置
	...
	public String create() throws Exception {
		...
		try {
			if (!Constant.MOD_CREATE.equals(getACT())) {
				// 取得用户选中的数据信息
				t00357.setSlid(SLID);
				bean = t00357Service.LoadByPrimaryKey(t00357);
				//sessionCtrl = new SessionCtrl(ActionContext.getContext().getSession());
	...
	public void validateUpdate() throws Exception {
		...
		if (Constant.MOD_UPDATE.equals(getACT())) {
			bean = t00357Service.LoadByPrimaryKey(bean);
		}
		// 判读数据是否已经被其他用户修改
		if ((Constant.MOD_UPDATE.equals(getACT()))
			...
			bean.setPgjg(ConvertUtil.toBigDecimal(txtPGJG));
			...
			bean.setNote(txtNOTET00357);
			bean.setFczh(txtFCZH);
			bean.setQdh(txtQDH);
			bean.setCb(txtCB);
			bean.setGhyt(txtGHYT);
	...
	public String update() throws Exception {
		...
		try {
			if (Constant.MOD_CREATE.equals(getACT())) { // Create
				...
				if (t00357Service.GetDeleteCommand(bean)) {
					this.addActionMessage(getText(Constant.MSG_DELETE_OK, new String[] { bean.getFczh() }));
					rtn = "successDEL";
				} else {
					this.addActionError(getText(Constant.MSG_DELETE_NG, new String[] { bean.getFczh() }));
				}
			}
	...
	public String detail() throws Exception {
		...
		try {
			// 准备查询条件
			v00357.setSlid(ConvertUtil.encoding(SLID));
			// 执行查询
	...
	public String queryKBSL()throws Exception{
		...
		try {
			...
			v00357.setPageSize(-1);
			v00357.setSlid(SLID);
			v00357.setCsyf(ConvertUtil.toInteger(txtCSYF));
			
			// 执行查询
			rows = t00357Service.LoadCSKbslk(v00357);
			// 分页设置
			if (null != rows && rows.size() > 0) {
	 ...
	 * @return the rows
	 */
	public ArrayList<Pgv00357> getRows() {
		return rows;
	}
	/**
	 * @param rows the rows to set
	 */
	public void setRows(ArrayList<Pgv00357> rows) {
		this.rows = rows;
	}
	/**
	...
	public Pgt00357 getBean() {
		return bean;
	}


	public void setBean(Pgt00357 bean) {
		this.bean = bean;
	}
