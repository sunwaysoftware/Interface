...
public class Pgt00306fAction extends ActionSupport implements SessionAware{
	...
	private ArrayList<Pgv00306f> rows;
	//查询条件
	...
	public String queryByDz()throws Exception{
		...
		try{
			...
			rows = t00306fService.LoadAllByDz(v00306f);
			String url = "http://" + ip;
			
			for(int i = 0;i < rows.size();i++){
				Pgv00306f e = rows.get(i);
...
//					e.setZpljDownLoad(url +ConvertUtil.pathURL(e.getZpljDownLoad()));
			}
		}catch(Exception e){
	...
	public String query()throws Exception{
		...
		try{
			Pgv00306f v00306f = new Pgv00306f();
			v00306f.setCd00306Id(ConvertUtil.toInteger(CD00306ID));
			
			rows = t00306fService.LoadAll(v00306f);
			String url = "http://" + ip;
	...
	public String create() throws Exception {
		if (LOG.isDebugEnabled()) {
			LOG.debug("create() ********** start **********"+MakeUtil.currentTime());
		}	
		...
		try{
			...
			{					
				...
				inpic.close();
				//生成縮略圖
				if(!mypic.pic(txtFile.toString(), RadFile+"/"+shortFileName,true)){
					throw new Exception(getText("app.msg.error.filezoomout"));
				}
				
				//生成显示圖
				if(!mypic.pic(txtFile.toString(), RadFile+"/"+viewFileName, 550, 550,true)){
			...
			t00306f.setCd00306Id(ConvertUtil.toInteger(CD00306ID));
			t00306f.setCd00352Xqdm(txtXQDM);
			//t00306f.setCd00352Xqdm(txtXQDMHM);						//2011-12-7修改  小区编码为代码号
			t00306f.setZplx(ConvertUtil.toInteger(txtZPLX)); 
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("create() ********** end **********"+MakeUtil.currentTime());
		}
		return SUCCESS;
	...
	public String delPhotoByLF()throws Exception{
		...
		try{
			...
			//String fileServerPath = ServletActionContext.getServletContext().getRealPath("upload//xqPic");
			Pgt00306f t00306f = new Pgt00306f();
			t00306f.setCd00306Id(ConvertUtil.toInteger(CD00306IDDEL));
	...
	public ArrayList<Pgv00306f> getRows() {
		return rows;
	}

	public void setRows(ArrayList<Pgv00306f> rows) {
		this.rows = rows;
	}
