...
public class Pgt00004Action extends ActionSupport implements SessionAware {
	...
	//Bean数组
	private ArrayList<Pgv00004> rows;
	//查询条件
	...
	public String query() throws Exception {
		...
		try {
			...
			//准备查询条件
			v00004.setRightnm(FormatUtil.toSearchLike(txtRIGHTNMFind));
			v00004.setPageIndex(pageIndex);
			v00004.setPageSize(getPageSize());
			//执行查询
			rows = t00004Service.LoadAll(v00004);
			//分页设置
	...
	public void validateUpdate() throws Exception{
		...
		//判断PK是否重复
		if((Constant.MOD_CREATE.equals(getACT()))&&(!CheckUtil.chkEmpty(t00004Bean.getRightid()))){
			this.addActionError(getText("app.msg.error.pk", new String[]{getText("app.xtwh.t00002.userid")}));
		}
		//判读数据是否已经被其他用户修改
		if((Constant.MOD_UPDATE.equals(getACT()))&&(!t00004Bean.getUpddate().equals(ConvertUtil.toTimestamp(txtUPDATE)))) {
	...
	public String LoadRightsByRole ()throws Exception{
		...
		try {
			...
			rows = t00004Service.LoadRightByRoleID(t00003Bean);
	...
	public String LoadRightsByUser()throws Exception{
		...
		try {
			...
			rows = t00004Service.LoadRightByUserID(t00002Bean);
			isAdmin = ConvertUtil.toBoolean(sessionCtrl.Get(SessionCtrl.SESSION_KEY_ISADMIN));
		} catch (Exception e) {
			return ERROR;
		}
	 ...
	 * @return the rows
	 */
	public ArrayList<Pgv00004> getRows() {
		return rows;
	}
	 ...
	 */
	public void setRows(ArrayList<Pgv00004> rows) {
		this.rows = rows;
	}
