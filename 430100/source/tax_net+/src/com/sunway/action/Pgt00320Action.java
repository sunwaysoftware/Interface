...
public class Pgt00320Action extends ActionSupport implements SessionAware{
	...
	//页面所需列表
	private ArrayList<Pgv00320> rows;
	private ArrayList<Pgv00052> szqyList;
	private Pgv00320 bean;
	...
	public String query() throws Exception {
		...
		try{
			...
			v00320.setXqnm(FormatUtil.toSearchLike(txtXQTIP));
			v00320.setJysj(ConvertUtil.toUtilDate(txtJYSJFind));
			v00320.setIsPgjg(ConvertUtil.toInteger(rdoPGJG));
			...
			rows = t00320Service.LoadAll(v00320);
			// 分页设置
	...
	public String create()throws Exception{
		...
		try{
			...
			if(!Constant.MOD_CREATE.equals(getACT())){
				Pgv00320 v00320 = new Pgv00320();
				v00320.setFcid(FCID);
				bean = t00320Service.LoadByPrimaryKey(v00320);
				//为303赋值
				v00303Bean = new Pgv00303();
				v00303Bean.setCd00001Szqy(bean.getCd00001Szqy());
				v00303Bean.setXqnm(bean.getXqnm());
				v00303Bean.setCd00352Xqdm(bean.getCd00352Xqdm());
				v00303Bean.setZlc(bean.getZlc().shortValue());
			}else{
	...
	public void validateUpdate()throws Exception{
		...
		try{
			...
			if(Constant.MOD_UPDATE.equals(getACT())){
				bean = t00320Service.LoadByPrimaryKey(bean);
			}
			//判断数据是否已被其他用户修改
			if ((Constant.MOD_UPDATE.equals(getACT()))&& (!bean.getUpddate().equals(ConvertUtil.toTimestamp(txtUPDATE)))) {
				...
				bean.setHdjg(ConvertUtil.toDouble(txtHDJG));
				bean.setCd00352Xqdm(txtXQDM);
				bean.setZlc(ConvertUtil.toInteger(txtZLC));
				bean.setSzlc(ConvertUtil.toInteger(txtSZLC));
				bean.setCd00001Fwlx(txtFWLX);
				bean.setCd00001Jzjg(txtJZJG);
				bean.setCd00001Ghyt(txtGHYT);
	...
	public String update()throws Exception{
		...
		try{
			...
			if(Constant.MOD_UPDATE.equals(getACT())){
				if(t00320Service.GetUpdateCommand(bean)){
					//为303赋值
					v00303Bean = new Pgv00303();
					v00303Bean.setCd00001Szqy(bean.getCd00001Szqy());
					v00303Bean.setXqnm(bean.getXqnm());
				...
				if(t00320Service.GetDeleteCommand(bean)){
...
//			fileServerPath = getSavePath() + "\\" + getUploadFileName();
	...
	public String executePG() throws Exception {
		...
		try {
			...
			v00320.setPssdYMD(ConvertUtil.toUtilDate(sessionCtrl.Get(SessionCtrl.SESSION_KEY_PSSD_YMD)));
			v00320.setCd00001Ssgx(sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
			...
			case 2:// 全部评税
				...
				rows = t00320Service.LoadPGList(v00320);
				pgTotal = rows.size();
				for (Integer i = 0; i < rows.size(); i++) {
					try {
						v00320.setFcid(rows.get(i).getFcid());
						// 執行評估
						if (t00320Service.GetExecPG(v00320))
							pgCount++;
						else
	...
	public String detail() throws Exception {
		...
		try {
			// 准备查询条件
			v00320.setFcid(CheckUtil.chkTrim(FCID));
			// 执行查询
	...
	public String executeTZD()throws Exception{
		...
		try{
			...
			v00302.setJysj(ConvertUtil.toUtilDate(txtJYSJ));
			v00302.setDjrq(ConvertUtil.toUtilDate(txtDJRQ));
			v00302.setJyjg(ConvertUtil.toDouble(txtJYJG));
	...
	public String isExistQmpg() throws Exception {
		...
		try {
			...
			v00320.setFh(txtFH);
			// 执行查询
			rows = t00320Service.LoadQMPGByFCXX(v00320);
		} catch (Exception e) {
			this.addActionError(e.getMessage());
			LOG.error(e.getMessage());
			return ERROR;
	...
	public String queryDZ()throws Exception{
		...
		try{
			...
			v00320.setPageSize(getPageSize());
			v00320.setCd00001Szqy(ddlSZQYFind);
			v00320.setZcdzl(FormatUtil.toSearchLike(txtZCDZLFind));
			v00320.setZh(FormatUtil.toSearchLike(txtZHFind));
			v00320.setDyh(FormatUtil.toSearchLike(txtDYFind));
			v00320.setFh(FormatUtil.toSearchLike(txtBWJFHFind));
			v00320.setXqnm(FormatUtil.toSearchLike(txtXQTIP));
			...
			rows = t00320Service.LoadAllDZ(v00320);
			// 分页设置
			if (null != rows && rows.size() > 0) {
				total = rows.get(0).getRecordCount();
			} else {
	...
	public ArrayList<Pgv00320> getRows() {
		return rows;
	}


	public void setRows(ArrayList<Pgv00320> rows) {
		this.rows = rows;
	}
	...
	public Pgv00320 getBean() {
		return bean;
	}


	public void setBean(Pgv00320 bean) {
		this.bean = bean;
	}
