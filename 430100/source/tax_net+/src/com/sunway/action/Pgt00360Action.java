...
public class Pgt00360Action extends ActionSupport implements SessionAware {
	...
	//view
	private ArrayList<Pgt00360> rows;
	...
	public String query() throws Exception {
		...
		try {
			...
			//执行查询
			rows = t00360Service.LoadAll(t00360);
			//分页设置
	...
	public void validateUpdate() throws Exception{
		...
		//判读数据是否已经被其他用户修改
		if((Constant.MOD_UPDATE.equals(ACT))&&(!t00360Bean.getUpddate().equals(ConvertUtil.toTimestamp(txtUPDATE)))) {
			...
			t00360Bean.setXzxs(BigDecimal.valueOf(ConvertUtil.toDouble(txtXZXS)));
			t00360Bean.setJzmjMin(BigDecimal.valueOf(ConvertUtil.toDouble(txtJZMJMIN)));
			t00360Bean.setJzmjMax(BigDecimal.valueOf(ConvertUtil.toDouble(txtJZMJMAX)));
	...
	public String exportT00360() throws Exception{
		...
		try{
			...
			t00360.setCd00001Fwlx(ConvertUtil.encoding(CheckUtil.chkTrim(txtFWLX)));
	...
	public void validateImportFile()throws Exception{
		...
		try{
			//sessionCtrl = new SessionCtrl(ActionContext.getContext().getSession());
			rows = t00360Service.ImportData(txtFilePath, sessionCtrl.getUserId(), sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
			if(rows.size() < 1){
	...
	public String importFile()throws Exception{
		...
		try{
			Pgt00360 errBean = t00360Service.GetImportCommand(rows);
	 ...
	 * @return the rows
	 */
	public ArrayList<Pgt00360> getRows() {
		return rows;
	}
	 ...
	 */
	public void setRows(ArrayList<Pgt00360> rows) {
		this.rows = rows;
	}
