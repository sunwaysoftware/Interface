...
public class Pgt00364Action extends ActionSupport implements SessionAware {
	...
	//view
	private ArrayList<Pgt00364> rows;
	...
	public String query() throws Exception {
		...
		try {
			...
			t00364.setPageSize(ConvertUtil.toInteger(sessionCtrl.Get(SessionCtrl.SESSION_KEY_DATASIZE)));
			t00364.setCd00001Ssgx(sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
			//执行查询
			rows = t00364Service.LoadAll(t00364);
			//分页设置
	...
	public String queryA() throws Exception {
		...
		try {
			...
			t00364.setCd00001Ssgx(sessionCtrl.Get(SessionCtrl.SESSION_KEY_SSGX));
			//执行查询
			rows = t00364Service.LoadAllA(t00364);
			//分页设置
			// 分页设置
			if (null != rows && rows.size() > 0) {
				total = CheckUtil.chkNull(rows.get(0).getRecordCount());
	...
	public void validateUpdate() throws Exception{
		...
		//判断PK是否重复
		if((Constant.MOD_CREATE.equals(getACT()))&&(!CheckUtil.chkEmpty(t00364Bean.getUpddate()))){
			this.addActionError(getText("app.msg.error.pk", new String[]{getText("app.xtwh.info.szqy")}));
		}
		//判读数据是否已经被其他用户修改
		if((Constant.MOD_UPDATE.equals(getACT()))&&(!t00364Bean.getUpddate().equals(ConvertUtil.toTimestamp(txtUPDATE)))) {
			this.addActionError(getText("app.msg.error.pktime"));
		}else{
			//为数据BEAN赋值
			t00364Bean.setCd00001Szqy(ddlSZQY);
			//t00364Bean.setCd00352Xqdm(txtXQFind);
			t00364Bean.setXzxs(BigDecimal.valueOf(ConvertUtil.toDouble(txtXZXS)));
			t00364Bean.setPgxzxs(BigDecimal.valueOf(ConvertUtil.toDouble(txtPGXZXS)));
	 ...
	 * @return the rows
	 */
	public ArrayList<Pgt00364> getRows() {
		return rows;
	}
	/**
	 * @param rows the rows to set
	 */
	public void setRows(ArrayList<Pgt00364> rows) {
		this.rows = rows;
	}
	/**
