...
public class Pgt00352Action extends ActionSupport implements SessionAware {
	...
	// Bean数组
	private ArrayList<Pgv00352> rows;
	...
	public String update() throws Exception {
		...
		try {
			if (Constant.MOD_CREATE.equals(getACT())) { // Create
				//此处状态为估价分区采集页面默认
				t00352Bean.setXqzt(ConvertUtil.toByte("0"));
	...
	public String loadXQ() throws Exception{		
		return getXQ(CheckUtil.chkTrim(XQNM),SQZY,rdoXQ);
	}
	private String getXQ(String XQNM,String SQZY,String rdoXQ) throws Exception {
		...
		try {
			ReadInfo();
			if (!CheckUtil.chkEmpty(SQZY)){
				v00352.setCd00001Szqy(SQZY);
			}
			if (!CheckUtil.chkEmpty(XQNM)) {
				v00352.setXqnm(XQNM);
			}
			if (!CheckUtil.chkEmpty(rdoXQ)) {
	...
	public String queryXQNM() throws Exception {
		...
		try {
			...
			dataList = t00352Service.GetXQNM(t00352);
		} catch (Exception e) {
			LOG.error(e.getMessage());
			this.addActionError(e.getMessage());
	...
	public String export() throws Exception {
		...
		try {
			...
			// 准备查询条件
			v00352.setCd00001Szqy(ddlSZQYFind);
			v00352.setXqnm(FormatUtil.toSearchLike(ConvertUtil.encoding(txtXqnmFind)));
			v00352.setXqdm(ConvertUtil.encoding(txtXqIdFind));
			v00352.setNote(FormatUtil.toSearchLike(ConvertUtil.encoding(txtXqDzFind)));
...
//			setExcelStream(new ByteArrayInputStream(out.toByteArray()));
	...
	public String validateXqdmhmIsExist()throws Exception{
		...
		try{
			Pgv00352 v00352 = new Pgv00352();
			v00352.setXqdmh(CheckUtil.chkTrim(txtXQDMH));
			v00352.setCd00001Szqy(ddlSZQY);
			isExist = t00352Service.validateXqdmhmIsExist(v00352);
			if(!isExist){
				resSign = "0";
	 ...
	 * @return the rows
	 */
	public ArrayList<Pgv00352> getRows() {
		return rows;
	}
	 ...
	 */
	public void setRows(ArrayList<Pgv00352> rows) {
		this.rows = rows;
	}
