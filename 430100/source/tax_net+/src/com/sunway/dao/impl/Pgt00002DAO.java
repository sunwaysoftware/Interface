...
public class Pgt00002DAO extends BaseDAO implements IPgt00002DAO {
	...
	private static final String USERGH = "USERGH";									//用户工号
	private static final String SSGXS = "SSGX";									//税收管辖名称
	/* (non-Javadoc)
	...
	public boolean GetInsertCommand(Pgt00002 user) throws Exception {
		...
		try {
			...
			call.setString(5, user.getUserip());
			call.setString(6, user.getPhone());
			call.setBoolean(7, user.getIslockedout());
			call.setDate(8, ConvertUtil.utilDateToSqlDate(user.getLastlockedoutdate()));
			call.setInt(9, user.getPageSize());
			call.setDate(10, ConvertUtil.utilDateToSqlDate(user.getPssd()));
			call.setBoolean(11, user.getIsadmin());
			call.setString(12, user.getCd00001Ssgx());
			call.setString(13, user.getRoleids());
			call.setString(14, user.getRightids());
			call.setString(15, user.getUsergh());
			call.execute();
			tran.commit();
			bResult = true;
		} catch (Exception e) {
	...
	public boolean GetUpdateCommand(Pgt00002 user) throws Exception {
		...
		try {
			...
			call.setString(6, user.getUserip());
			call.setString(7, user.getPhone());
			call.setBoolean(8, user.getIslockedout());
			call.setBoolean(9, user.getIsadmin());
			call.setTimestamp(10, user.getLastlockedoutdate());
			call.setInt(11, user.getPageSize());
			call.setDate(12, ConvertUtil.utilDateToSqlDate(user.getPssd()));
			call.setString(13, user.getRoleids());
			call.setString(14, user.getRightids());
			call.setString(15, user.getCd00001Ssgx());
			call.setString(16, user.getUsergh());
			call.execute();
			tran.commit(); 
			bResult = true;
		} catch (Exception e) {
	...
	protected Pgv00002 SetVParameters(ResultSet rs) throws Exception {
		...
		e.setUpddate(FormatUtil.toYMDHMS(rs.getTimestamp(UPDDATE)));
		e.setCd00002Czr(rs.getString(CD00002CZR));
		e.setNote(rs.getString(NOTE));
		e.setUserip(rs.getString(USERIP));
		e.setRecordCount(rs.getInt(TOTAL));
		e.setCzr(rs.getString(CZR));
		e.setPageSize(rs.getInt(PAGECOUNT));
		e.setUserip(rs.getString(USERIP));
		e.setLastlogindate(FormatUtil.toYMDHMS(rs.getTimestamp(LASTLOGINDATE)));
		e.setPhone(rs.getString(PHONE));
		e.setIslockedout(rs.getBoolean(ISLOCKEDOUT));
		e.setLastlockedoutdate(FormatUtil.toYMDHMS(rs.getTimestamp(LASTLOCKEDOUTDATE)));
		...
		e.setIsAdmin(rs.getBoolean(ISADMIN));
		e.setPssd(rs.getDate(PSSD));
		e.setUsergh(rs.getString(USERGH));
		return e;
	}
	...
	public Pgt00002 CheckLogin(Pgt00002 user) throws Exception {
		ArrayList<Pgt00002> listResult = new ArrayList<Pgt00002>();
		LogInfo loginfo = user.getLoginfo();
		...
		try {
			...
			conn = super.getConnection();
			String sql = "{call PGSP_CZ_00002(?,?,?,?,?,?)}";
			call = conn.prepareCall(loginfo.addSql(sql));
			//注册输出参数
			call.registerOutParameter(1, OracleTypes.CURSOR);
			call.registerOutParameter(5, OracleTypes.INTEGER);
			call.registerOutParameter(6, OracleTypes.VARCHAR);
			//传入输入参数
			call.setString(2, loginfo.addParam(user.getUserid()));
			call.setString(3, loginfo.addParam(user.getUserpwd()));
			call.setString(4, loginfo.addParam(user.getUserip()));
			call.execute();
			...
			resultT00002.setLoginState(call.getInt(5));
			resultT00002.setLoginMessage(call.getString(6));
		} catch (Exception e) {
			loginfo.setErrInfo(e.getMessage());
			throw e;
		}finally{
			execShowSQL(loginfo);
			getFree(rs, call, conn, session);
...
//			rs = (ResultSet)call.getObject(5);
	...
	public boolean SettingPersonal(Pgt00002 user) throws Exception {
		...
		try {
			...
			//传入输入参数
			call.setString(1, user.getUserid());
			call.setInt(2, user.getPageSize());
			call.setDate(3, ConvertUtil.utilDateToSqlDate(user.getPssd()));
			call.execute();
			tran.commit(); 
			bResult = true;
		} catch (Exception e) {
...
//				            .append(CheckUtil.chkNull(rs.getString(USERIP))).append(Constant.STRING_COMMA)
//				            .append(CheckUtil.chkNull(rs.getString(PHONE))).append(Constant.STRING_COMMA)
//				            .append(FormatUtil.toYMDHMS(rs.getTimestamp(LASTLOCKEDOUTDATE))).append(Constant.STRING_COMMA)
//				            .append(rs.getDate(PSSD).toString()).append(Constant.STRING_COMMA)
//				            .append(rs.getString(SSGX)).append(Constant.STRING_COMMA)
//				            .append(FormatUtil.toYMDHMS(rs.getTimestamp(UPDDATE)))
...
//	}
	...
	@Override
	public OutputStream ExportAll(Pgv00002 user) throws Exception {
		...
		try {
			...
			if(rs!=null){
				...
				sheet.addCell(label);	
				label = new Label(2, 0, "IP地址",wcf);
				sheet.addCell(label);	          
				label = new Label(3, 0, "联系电话",wcf);
				sheet.addCell(label);
				label = new Label(4, 0, "最后解锁时间",wcf);
				sheet.addCell(label);	
				...
				sheet.addCell(label);
				label = new Label(8, 0, "税收管辖",wcf);
				sheet.addCell(label);
				label = new Label(9, 0, "更新时间",wcf);
				sheet.addCell(label);
				...
				while(null!=rs && rs.next()){
					...
					sheet.addCell(label);		
					label = new Label(2, rowIndex, rs.getString(USERIP));
					sheet.addCell(label);	
					label = new Label(3, rowIndex, rs.getString(PHONE));
					sheet.addCell(label);
					label = new Label(4, rowIndex, rs.getString(LASTLOCKEDOUTDATE));
					sheet.addCell(label);
					...
					sheet.addCell(label);	
					if("0".equals(rs.getString(ISLOCKEDOUT))){
					...
					sheet.addCell(label);
					label = new Label(8, rowIndex, rs.getString(SSGX));
					sheet.addCell(label);
					label = new Label(9, rowIndex, FormatUtil.toYMDHMS(rs.getTimestamp(UPDDATE)));
					sheet.addCell(label);
				}
	            workbook.write();   
	            workbook.close();  }
	...
	public Pgt00002 ChkVerify(Pgt00002 user) throws Exception {
		...
		try {
			session = super.getSession();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_CZ_000024(?,?,?,?,?)}");
			//注册输出参数
			call.registerOutParameter(1, OracleTypes.INTEGER);
			call.registerOutParameter(2, OracleTypes.NVARCHAR);
			//传入输入参数
			call.setString(3, user.getUserid());
			call.setString(4, user.getUserpwd());
			call.setString(5, user.getCd00001Ssgx());
			call.execute();
			bResult.setLoginState(call.getInt(1));
			bResult.setLoginMessage(call.getString(2));
		} catch (Exception e) {
			throw e;
		}finally{
			getFree(rs, call, conn, session);
