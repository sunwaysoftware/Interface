package com.sunway.dao.impl;

import java.io.ByteArrayOutputStream;
import java.io.OutputStream;
import java.sql.CallableStatement;
import java.sql.Connection;
import java.sql.ResultSet;
import java.util.ArrayList;

import jxl.Workbook;
import jxl.write.Label;
import jxl.write.WritableCellFormat;
import jxl.write.WritableSheet;
import jxl.write.WritableWorkbook;
import oracle.jdbc.OracleTypes;

import org.hibernate.Session;
import org.hibernate.Transaction;

import com.sunway.dao.IPgt02052DAO;

import com.sunway.util.Excel;
import com.sunway.util.FormatUtil;
import com.sunway.vo.Pgt02052;
import com.sunway.vo.Pgv02052;

/**
 * @category 商业建筑层次修正
 * @author Lee
 * @version 1.0
 *
 */
public class Pgt02052DAO extends BaseDAO implements IPgt02052DAO {
	
	private static final String id = "ID";							//流水号
	private static final String CD00001SZQYLX = "cd_00001_szqylx";	//所在区域类型
	private static final String CD00001SZQY = "cd_00001_szqy";;		//所在区域编码
	private static final String SZQY =  "szqy";						//所在区域名称
	private static final String CD00001FWLXLX = "cd_00001_fwlxlx";	//房屋类型类型
	private static final String CD00001FWLX = "cd_00001_fwlx";;		//房屋类型编码
	private static final String FWLX =  "fwlx";			//房屋类型名称
	private static final String XZXS = "xzxs";						//修正系数
	private static final String LC =  "lc";				//楼层
	private static final String ZCS =  "zlc";						//总楼层
	private static final String UPDDATE="upddate";					//更改时间
	private static final String CD00002CZR = "cd_00002_czr";		//操作人编号	
	private static final String CZR = "czr";						//操作人
	private static final String NOTE = "note";						//备注信息
	private static final String TOTAL = "total";					//总纪录数
	//private static final String CZLX = "czlx";						//操作类型
	private static final String CD02050XQDM = "cd_02050_xqdm";    //小区编码
	private static final String XQNM = "xqnm";         
	
	/* (non-Javadoc)
	 * @see com.sunway.dao.IPgt10051DAO#GetDeleteCommand(com.sunway.vo.Pgt10051)
	 */
	
	@Override
	public boolean GetDeleteCommand(Pgt02052 lcxz) throws Exception {
		boolean bResult = false;
		CallableStatement call = null;
		Connection conn = null;
		Session session = null;
		Transaction tran = null;
		try {
			session = getSession();
			tran = session.beginTransaction();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_DELT02052(?,?,?)}");
			call.setString(1, lcxz.getId());
			call.setString(2, lcxz.getCd00002Czr());
			call.setString(3, lcxz.getCd00001Ssgx());
			call.execute();
			tran.commit();
			bResult = true;
		} catch (Exception e) {
			tran.rollback();
			throw e;
		} finally {
			getFree(call, conn, session);
		}
		return bResult;
	}

	
	
	/* (non-Javadoc)
	 * @see com.sunway.dao.IPgt10051DAO#GetDeleteCommand(com.sunway.vo.Pgt10051)
	 */
	
	@Override
	public boolean GetDeleteCommandA(Pgt02052 lcxz) throws Exception {
		boolean bResult = false;
		CallableStatement call = null;
		Connection conn = null;
		Session session = null;
		Transaction tran = null;
		try {
			session = getSession();
			tran = session.beginTransaction();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_DELT02052A(?,?,?)}");
			call.setString(1, lcxz.getId());
			call.setString(2, lcxz.getCd00002Czr());
			call.setString(3, lcxz.getCd00001Ssgx());
			call.execute();
			tran.commit();
			bResult = true;
		} catch (Exception e) {
			tran.rollback();
			throw e;
		} finally {
			getFree(call, conn, session);
		}
		return bResult;
	}

	
	/* (non-Javadoc)
	 * @see com.sunway.dao.IPgt10051DAO#GetInsertCommand(com.sunway.vo.Pgt10051)
	 */
	
	@Override
	public boolean GetInsertCommand(Pgt02052 lcxz) throws Exception {
		boolean bResult = false;
		CallableStatement call = null;
		Connection conn = null;
		Session session = null;
		Transaction tran = null;
		try {
			session = getSession();
			tran = session.beginTransaction();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_ADDT02052(?,?,?,?,?,?,?,?)}");
			call.setString(1, lcxz.getLc());
			call.setString(2, lcxz.getCd00001Szqy());
			call.setDouble(3, lcxz.getXzxs());
			call.setString(4, lcxz.getCd00002Czr());
			call.setString(5, lcxz.getNote());
			call.setString(6, lcxz.getCd00001Ssgx());
			call.setString(7, lcxz.getCd00001Fwlx());
			call.setString(8, lcxz.getZcs());
			call.execute();
			tran.commit();
			bResult = true;
		} catch (Exception e) {
			tran.rollback();
			throw e;
		} finally {
			getFree(call, conn, session);
		}
		return bResult;
	}

	
	/* (non-Javadoc)
	 * @see com.sunway.dao.IPgt10051DAO#GetInsertCommand(com.sunway.vo.Pgt10051)
	 */
	
	@Override
	public boolean GetInsertCommandA(Pgt02052 lcxz) throws Exception {
		boolean bResult = false;
		CallableStatement call = null;
		Connection conn = null;
		Session session = null;
		Transaction tran = null;
		try {
			session = getSession();
			tran = session.beginTransaction();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_ADDT02052A(?,?,?,?,?,?,?,?)}");
			call.setString(1, lcxz.getLc());
			call.setString(2, lcxz.getCd02050Xqdm());
			//call.setString(4, lcxz.getCd00001Szqy());
			call.setDouble(3, lcxz.getXzxs());
			call.setString(4, lcxz.getCd00002Czr());
			call.setString(5, lcxz.getNote());
			call.setString(6, lcxz.getCd00001Ssgx());
			call.setString(7, lcxz.getCd00001Fwlx());
			call.setString(8, lcxz.getZcs());
			call.execute();
			tran.commit();
			bResult = true;
		} catch (Exception e) {
			tran.rollback();
			throw e;
		} finally {
			getFree(call, conn, session);
		}
		return bResult;
	}
	
	
	/* (non-Javadoc)
	 * @see com.sunway.dao.IPgt10051DAO#GetUpdateCommand(com.sunway.vo.Pgt10051)
	 */
	
	@Override
	public boolean GetUpdateCommand(Pgt02052 lcxz) throws Exception {
		boolean bResult = false;
		CallableStatement call = null;
		Connection conn = null;
		Session session = null;
		Transaction tran = null;
		try {
			session = getSession();
			tran = session.beginTransaction();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_UPDT02052(?,?,?,?,?,?,?,?,?)}");
			call.setString(1, lcxz.getId());
			call.setString(2, lcxz.getLc());
			call.setString(3, lcxz.getCd00001Szqy());
			call.setDouble(4, lcxz.getXzxs());
			call.setString(5, lcxz.getCd00002Czr());
			call.setString(6, lcxz.getNote());
			call.setString(7, lcxz.getCd00001Ssgx());
			call.setString(8, lcxz.getCd00001Fwlx());
			call.setString(9, lcxz.getZcs());
			call.execute();
			tran.commit();
			bResult = true;
		} catch (Exception e) {
			tran.rollback();
			throw e;
		} finally {
			getFree(call, conn, session);
		}
		return bResult;
	}

	
	/* (non-Javadoc)
	 * @see com.sunway.dao.IPgt10051DAO#GetUpdateCommand(com.sunway.vo.Pgt10051)
	 */
	
	@Override
	public boolean GetUpdateCommandA(Pgt02052 lcxz) throws Exception {
		boolean bResult = false;
		CallableStatement call = null;
		Connection conn = null;
		Session session = null;
		Transaction tran = null;
		try {
			session = getSession();
			tran = session.beginTransaction();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_UPDT02052A(?,?,?,?,?,?,?,?,?)}");
			call.setString(1, lcxz.getId());
			call.setString(2, lcxz.getLc());
			call.setString(3,lcxz.getCd02050Xqdm());
			//call.setString(4, lcxz.getCd00001Szqy());
			call.setDouble(4, lcxz.getXzxs());
			call.setString(5, lcxz.getCd00002Czr());
			call.setString(6, lcxz.getNote());
			call.setString(7, lcxz.getCd00001Ssgx());
			call.setString(8, lcxz.getCd00001Fwlx());
			call.setString(9, lcxz.getZcs());
			call.execute();
			tran.commit();
			bResult = true;
		} catch (Exception e) {
			tran.rollback();
			throw e;
		} finally {
			getFree(call, conn, session);
		}
		return bResult;
	}
	/* (non-Javadoc)
	 * @see com.sunway.dao.IPgt10051DAO#LoadAll(com.sunway.vo.Pgv10051)
	 */
	
	@Override
	public ArrayList<Pgv02052> LoadAll(Pgv02052 lcxz) throws Exception {
		ArrayList<Pgv02052> listResult = new ArrayList<Pgv02052>();
		ResultSet rs = null;
		CallableStatement call = null;
		Connection conn = null;
		
		try {
			
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_GETALLT02052(?,?,?,?,?,?,?,?)}");
			//注册输出参数
			call.registerOutParameter(1, OracleTypes.CURSOR);
			//注册输入参数
			call.setInt(2, lcxz.getPageIndex());
			call.setInt(3, lcxz.getPageSize());
			call.setString(4, lcxz.getLc());
			call.setString(5, lcxz.getCd00001Fwlx());
			call.setString(6, lcxz.getCd00001Szqy());
			call.setString(7, lcxz.getCd00001Ssgx());
			call.setString(8, lcxz.getZcs());
			call.execute();
			rs = (ResultSet) call.getObject(1); 
			while(null!=rs && rs.next()){
				listResult.add(SetVParameters(rs));
			}
		} catch (Exception e) {
			throw e;
		} finally {
			getFree(rs, call, conn, null);
		}
		return listResult;
	}
	
	/* (non-Javadoc)
	 * @see com.sunway.dao.IPgt10051DAO#LoadAll(com.sunway.vo.Pgv10051)
	 */
	
	@Override
	public ArrayList<Pgv02052> LoadAllA(Pgv02052 lcxz) throws Exception {
		ArrayList<Pgv02052> listResult = new ArrayList<Pgv02052>();
		ResultSet rs = null;
		CallableStatement call = null;
		Connection conn = null;
		
		try {
			
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_GETALLT02052A(?,?,?,?,?,?,?,?,?)}");
			//注册输出参数
			call.registerOutParameter(1, OracleTypes.CURSOR);
			//注册输入参数
			call.setInt(2, lcxz.getPageIndex());
			call.setInt(3, lcxz.getPageSize());
			call.setString(4, lcxz.getLc());
			call.setString(5, lcxz.getCd00001Fwlx());
			call.setString(6, lcxz.getCd00001Szqy());
			call.setString(7, lcxz.getCd00001Ssgx());
			call.setString(8, lcxz.getCd02050Xqdm());
			call.setString(9, lcxz.getZcs());
			call.execute();
			rs = (ResultSet) call.getObject(1); 
			while(null!=rs && rs.next()){
				listResult.add(SetVParametersA(rs));
			}
		} catch (Exception e) {
			throw e;
		} finally {
			getFree(rs, call, conn, null);
		}
		return listResult;
	}
	
	/**
	 * View数据转存
	 * @param rs数据集
	 * @return 数据实体
	 * @throws Exception
	 */
	protected Pgv02052 SetVParametersA(ResultSet rs) throws Exception {
		Pgv02052 e = new Pgv02052();
		e.setId(rs.getString(id));
		e.setLc(rs.getString(LC));
		e.setZcs(rs.getString(ZCS));
		e.setCd00001Szqylx(rs.getString(CD00001SZQYLX));
		e.setCd00001Szqy(rs.getString(CD00001SZQY));
		e.setSzqy(rs.getString(SZQY));
		e.setXzxs(rs.getDouble(XZXS));
		e.setUpddate(rs.getTimestamp(UPDDATE));
		e.setCd00002Czr(rs.getString(CD00002CZR));
		e.setNote(rs.getString(NOTE));
		e.setCzr(rs.getString(CZR));
		e.setRecordCount(rs.getInt(TOTAL));
		e.setCd00001Fwlxlx(rs.getString(CD00001FWLXLX));
		e.setCd00001Fwlx(rs.getString(CD00001FWLX));
		e.setFwlx(rs.getString(FWLX));
		e.setCd02050Xqdm(rs.getString(CD02050XQDM));
		e.setXqnm(rs.getString(XQNM));
		e.setXqdmhm(rs.getString("XQDMHM"));
		return e;
	}
	/**
	 * View数据转存
	 * @param rs数据集
	 * @return 数据实体
	 * @throws Exception
	 */
	protected Pgv02052 SetVParameters(ResultSet rs) throws Exception {
		Pgv02052 e = new Pgv02052();
		e.setId(rs.getString(id));
		e.setLc(rs.getString(LC));
		e.setZcs(rs.getString(ZCS));
		e.setCd00001Szqylx(rs.getString(CD00001SZQYLX));
		e.setCd00001Szqy(rs.getString(CD00001SZQY));
		e.setSzqy(rs.getString(SZQY));
		e.setXzxs(rs.getDouble(XZXS));
		e.setUpddate(rs.getTimestamp(UPDDATE));
		e.setCd00002Czr(rs.getString(CD00002CZR));
		e.setNote(rs.getString(NOTE));
		e.setCzr(rs.getString(CZR));
		e.setRecordCount(rs.getInt(TOTAL));
		e.setCd00001Fwlxlx(rs.getString(CD00001FWLXLX));
		e.setCd00001Fwlx(rs.getString(CD00001FWLX));
		e.setFwlx(rs.getString(FWLX));
		return e;
	}
	
	/* (non-Javadoc)
	 * @see com.sunway.dao.IPgt10051DAO#LoadByPrimaryKey(com.sunway.vo.Pgt10051)
	 */
	
	@Override
	public Pgt02052 LoadByPrimaryKey(Pgt02052 lcxz) throws Exception {
		ArrayList<Pgt02052> listResult = new ArrayList<Pgt02052>();
		ResultSet rs = null;
		CallableStatement call = null;
		Connection conn = null;
		try {
			
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_GETT02052(?,?)}");
			//注册输出参数
			call.registerOutParameter(1, OracleTypes.CURSOR);
			//传入输入参数
			call.setString(2, lcxz.getId());
			call.execute();
			//返回数据集
			rs = (ResultSet)call.getObject(1);
			while (null!=rs && rs.next()) {
				listResult.add(SetTParameters(rs));			
			}
		} catch (Exception e) {
			throw e;
		}finally{
			getFree(rs, call, conn, null);
		}
		if(listResult!=null && listResult.size()>0)
			return listResult.get(0);
		else
			return lcxz;
	}
	
	/* (non-Javadoc)
	 * @see com.sunway.dao.IPgt10051DAO#LoadByPrimaryKey(com.sunway.vo.Pgt10051)
	 */
	
	@Override
	public Pgt02052 LoadByPrimaryAddKey(Pgt02052 lcxz) throws Exception {
		ArrayList<Pgt02052> listResult = new ArrayList<Pgt02052>();
		ResultSet rs = null;
		CallableStatement call = null;
		Connection conn = null;
		try {
			
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_GETT020521(?,?,?,?,?)}");
			//注册输出参数
			call.registerOutParameter(1, OracleTypes.CURSOR);
			//传入输入参数
			call.setString(2, lcxz.getCd00001Szqy());
			call.setString(3, lcxz.getCd00001Fwlx());
			call.setString(4, lcxz.getLc());
			call.setString(5, lcxz.getZcs());
			call.execute();
			//返回数据集
			rs = (ResultSet)call.getObject(1);
			while (null!=rs && rs.next()) {
				listResult.add(SetTParameters(rs));			
			}
		} catch (Exception e) {
			throw e;
		}finally{
			getFree(rs, call, conn, null);
		}
		if(listResult!=null && listResult.size()>0)
			return listResult.get(0);
		else
			return lcxz;
	}
	
	
	/* (non-Javadoc)
	 * @see com.sunway.dao.IPgt10051DAO#LoadByPrimaryKey(com.sunway.vo.Pgt10051)
	 */
	
	public Pgt02052 LoadByPrimaryKeyA(Pgt02052 lcxz) throws Exception {
		ArrayList<Pgt02052> listResult = new ArrayList<Pgt02052>();
		ResultSet rs = null;
		CallableStatement call = null;
		Connection conn = null;
		
		try {
			
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_GETT02052A(?,?)}");
			//注册输出参数
			call.registerOutParameter(1, OracleTypes.CURSOR);
			//传入输入参数
			call.setString(2, lcxz.getId());
			call.execute();
			//返回数据集
			rs = (ResultSet)call.getObject(1);
			while (null!=rs && rs.next()) {
				listResult.add(SetTParametersA1(rs));			
			}
		} catch (Exception e) {
			throw e;
		}finally{
			getFree(rs, call, conn, null);
		}
		if(listResult!=null && listResult.size()>0)
			return listResult.get(0);
		else
			return lcxz;
	}
	
	/* (non-Javadoc)
	 * @see com.sunway.dao.IPgt10051DAO#LoadByPrimaryKey(com.sunway.vo.Pgt10051)
	 */
	
	public Pgt02052 LoadByPrimaryAddKeyA(Pgt02052 lcxz) throws Exception {
		ArrayList<Pgt02052> listResult = new ArrayList<Pgt02052>();
		ResultSet rs = null;
		CallableStatement call = null;
		Connection conn = null;
		
		try {
			
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_GETT02052A1(?,?,?,?,?)}");
			//注册输出参数
			call.registerOutParameter(1, OracleTypes.CURSOR);
			//传入输入参数
			call.setString(2, lcxz.getCd02050Xqdm());
			call.setString(3, lcxz.getCd00001Fwlx());
			call.setString(4, lcxz.getLc());
			call.setString(5, lcxz.getZcs());
			call.execute();
			//返回数据集
			rs = (ResultSet)call.getObject(1);
			while (null!=rs && rs.next()) {
				listResult.add(SetTParametersA1(rs));			
			}
		} catch (Exception e) {
			throw e;
		}finally{
			getFree(rs, call, conn, null);
		}
		if(listResult!=null && listResult.size()>0)
			return listResult.get(0);
		else
			return lcxz;
	}
	/**
	 * View数据转存
	 * @param rs数据集
	 * @return 数据实体
	 * @throws Exception
	 */
	protected Pgt02052 SetTParametersA1(ResultSet rs) throws Exception {
		Pgt02052 e = new Pgt02052();
		e.setZcs(rs.getString(ZCS));
		e.setLc(rs.getString(LC));
		e.setCd00001Szqy(rs.getString(CD00001SZQY));
		e.setXzxs(rs.getDouble(XZXS));
		e.setUpddate(rs.getTimestamp(UPDDATE));
		e.setCd00002Czr(rs.getString(CD00002CZR));
		e.setNote(rs.getString(NOTE));
		e.setCd00001Fwlxlx(rs.getString(CD00001FWLXLX));
		e.setCd00001Fwlx(rs.getString(CD00001FWLX));
		e.setCd02050Xqdm(rs.getString(CD02050XQDM));
		e.setXqnm(rs.getString(XQNM));
		e.setId(rs.getString(id));
		return e;
	}
	/**
	 * View数据转存
	 * @param rs数据集
	 * @return 数据实体
	 * @throws Exception
	 */
	protected Pgt02052 SetTParameters(ResultSet rs) throws Exception {
		Pgt02052 e = new Pgt02052();
		e.setLc(rs.getString(LC));
		e.setZcs(rs.getString(ZCS));
		e.setCd00001Szqylx(rs.getString(CD00001SZQYLX));
		e.setCd00001Szqy(rs.getString(CD00001SZQY));
		e.setXzxs(rs.getDouble(XZXS));
		e.setUpddate(rs.getTimestamp(UPDDATE));
		e.setCd00002Czr(rs.getString(CD00002CZR));
		e.setNote(rs.getString(NOTE));
		e.setCd00001Fwlxlx(rs.getString(CD00001FWLXLX));
		e.setCd00001Fwlx(rs.getString(CD00001FWLX));
		e.setId(rs.getString(id));
		return e;
	}

	/* (non-Javadoc)
	 * @see com.sunway.dao.IPgt10051DAO#ExecuteParamCopy(com.sunway.vo.Pgt10051)
	 */
	
	@Override
	public boolean ExecuteParamCopy(Pgt02052 lcxz) throws Exception {
		boolean bResult = false;
		CallableStatement call = null;
		Connection conn = null;
		Session session = null;
		Transaction tran = null;
		try {	
			session = getSession();
			tran = session.beginTransaction();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_CZ_02052(?,?,?)}");
			call.setString(1, lcxz.getCd00001Szqy());
			call.setString(2, lcxz.getCd00002Czr());
			call.setString(3, lcxz.getCd00001Ssgx());
			call.execute();
			tran.commit();
			bResult = true;
		} catch (Exception e) {
			tran.rollback();
			throw e;
		} finally {
			getFree(call, conn, session);
		}
		return bResult;
	}

	
	@Override
	public OutputStream ExportT052(Pgv02052 v02052) throws Exception {
		ByteArrayOutputStream strBufResult = null;
		WritableWorkbook workbook =null;
		Connection conn = null;
		
		CallableStatement call = null;
		ResultSet rs = null;
		try {
			strBufResult = new ByteArrayOutputStream();
			
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_GETALLT02052(?,?,?,?,?,?,?,?)}");
			//注册输出参数
			call.registerOutParameter(1, OracleTypes.CURSOR);
			//注册输入参数
			call.setInt(2, v02052.getPageIndex());
			call.setInt(3, v02052.getPageSize());
			call.setString(4, v02052.getLc());
			call.setString(5, v02052.getCd00001Fwlx());
			call.setString(6, v02052.getCd00001Szqy());
			call.setString(7, v02052.getCd00001Ssgx());
			call.setString(8, v02052.getZcs());
			call.execute();
			
			rs = (ResultSet)call.getObject(1);
			if(rs != null){
				Label label;
				workbook = Workbook.createWorkbook(strBufResult);
				WritableSheet sheet = workbook.createSheet("楼层修正", 0);
				WritableCellFormat wcf =Excel.getExcelTitleStyle();
				
				label = new Label(0, 0, "所在区域",wcf);
				sheet.addCell(label);
				label = new Label(1, 0, "房屋类型",wcf);
				sheet.addCell(label);
				label = new Label(2, 0, "所在楼层",wcf);
				sheet.addCell(label);
				label = new Label(3, 0, "总楼层",wcf);
				sheet.addCell(label);			
				label = new Label(4, 0, "修正值(%)",wcf);
				sheet.addCell(label);
				label = new Label(5, 0, "更新时间",wcf);
				sheet.addCell(label);
				label = new Label(6, 0, "操作人",wcf);
				sheet.addCell(label);
				label = new Label(7, 0, "备注",wcf);
				sheet.addCell(label);
				
				
				while(rs != null && rs.next()){
					Integer rowIndex = rs.getRow();
					
					
					label = new Label(0, rowIndex, rs.getString(SZQY));
					sheet.addCell(label);
					label = new Label(1, rowIndex, rs.getString(FWLX));
					sheet.addCell(label);
					label = new Label(2, rowIndex, rs.getString(LC));
					sheet.addCell(label);
					label = new Label(3, rowIndex, rs.getString(ZCS));
					sheet.addCell(label);				
					label = new Label(4, rowIndex, rs.getString(XZXS));
					sheet.addCell(label);
					label = new Label(5, rowIndex, FormatUtil.toYMDHMS(rs.getTimestamp(UPDDATE)));
					sheet.addCell(label);
					label = new Label(6, rowIndex, rs.getString(CZR));
					sheet.addCell(label);
					label = new Label(7, rowIndex, rs.getString(NOTE));
					sheet.addCell(label);
					
				}
				
				workbook.write();
				//workbook.close();
			} 
			//填写导出日志
			//WriteLogExp(v02052.getCd00001Ssgx(), "PGT02052", v02052.getCd00002Czr(), "楼层修正导出成功");
		}catch(Exception e){
			e.printStackTrace();
			throw e;
		}finally{
			if(null != strBufResult){
				try{
					strBufResult.flush();
					strBufResult.close();
				}catch(Exception e1){}
			}
			if(null != workbook){
				try{
					workbook.close();
				}catch(Exception e){}
			}
			getFree(rs, call, conn, null);
		}
		return strBufResult;
	}

	
	
	
	@Override
	public OutputStream ExportT052A(Pgv02052 v02052) throws Exception {
		ByteArrayOutputStream strBufResult = null;
		WritableWorkbook workbook =null;
		Connection conn = null;
		
		CallableStatement call = null;
		ResultSet rs = null;
		try {
			strBufResult = new ByteArrayOutputStream();
			
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_GETALLT02052A(?,?,?,?,?,?,?,?,?)}");
			//注册输出参数
			call.registerOutParameter(1, OracleTypes.CURSOR);
			//注册输入参数
			call.setInt(2, v02052.getPageIndex());
			call.setInt(3, v02052.getPageSize());
			call.setString(4, v02052.getLc());
			call.setString(5, v02052.getCd00001Fwlx());
			call.setString(6, v02052.getCd00001Szqy());
			call.setString(7, v02052.getCd00001Ssgx());
			call.setString(8, v02052.getCd02050Xqdm());
			call.setString(9, v02052.getZcs());
			call.execute();
			
			rs = (ResultSet)call.getObject(1);
			if(rs != null){
				Label label;
				workbook = Workbook.createWorkbook(strBufResult);
				WritableSheet sheet = workbook.createSheet("楼层修正", 0);
				WritableCellFormat wcf =Excel.getExcelTitleStyle();
				
				label = new Label(0, 0, "所在区域",wcf);
				sheet.addCell(label);
				label = new Label(1, 0, "代码号",wcf);
				sheet.addCell(label);
				label = new Label(2, 0, "小区名称",wcf);
				sheet.addCell(label);
				label = new Label(3, 0, "房屋类型",wcf);
				sheet.addCell(label);
				label = new Label(4, 0, "所在楼层",wcf);
				sheet.addCell(label);	
				label = new Label(5, 0, "总楼层",wcf);
				sheet.addCell(label);
				label = new Label(6, 0, "修正值(%)",wcf);
				sheet.addCell(label);
				label = new Label(7, 0, "更新时间",wcf);
				sheet.addCell(label);
				label = new Label(8, 0, "操作人",wcf);
				sheet.addCell(label);
				label = new Label(9, 0, "备注",wcf);
				sheet.addCell(label);
				
				
				while(rs != null && rs.next()){
					Integer rowIndex = rs.getRow();
					
					
					label = new Label(0, rowIndex, rs.getString(SZQY));
					sheet.addCell(label);
					label = new Label(1, rowIndex, rs.getString("XQDMHM"));
					sheet.addCell(label);
					label = new Label(2, rowIndex, rs.getString(XQNM));
					sheet.addCell(label);
					label = new Label(3, rowIndex, rs.getString(FWLX));
					sheet.addCell(label);
					label = new Label(4, rowIndex, rs.getString(LC));
					sheet.addCell(label);
					label = new Label(5, rowIndex, rs.getString(ZCS));
					sheet.addCell(label);
					label = new Label(6, rowIndex, rs.getString(XZXS));
					sheet.addCell(label);
					label = new Label(7, rowIndex, FormatUtil.toYMDHMS(rs.getTimestamp(UPDDATE)));
					sheet.addCell(label);
					label = new Label(8, rowIndex, rs.getString(CZR));
					sheet.addCell(label);
					label = new Label(9, rowIndex, rs.getString(NOTE));
					sheet.addCell(label);
					
				}
				
				workbook.write();
				//workbook.close();
			} 
			//填写导出日志
			//WriteLogExp(v02052.getCd00001Ssgx(), "PGT02052", v02052.getCd00002Czr(), "楼层修正导出成功");
		}catch(Exception e){
			e.printStackTrace();
			throw e;
		}finally{
			getFree(rs, call, conn, null);
			if(null !=strBufResult){
				try{
					strBufResult.flush();
					strBufResult.close();
				}catch(Exception e1){}
			}
			if(null !=workbook){
				try{
					workbook.close();
				}catch(Exception e){}
			}
		}
		return strBufResult;
	}
	
	
	@Override
	public Pgv02052 ImportExcelData(ArrayList<Pgv02052> v02052List)
			throws Exception {
		Pgv02052 v02052 = new Pgv02052();
		ArrayList<Pgv02052> tempList = new ArrayList<Pgv02052>();
		Integer resultValue = 0;
		Integer sResultCount = 0;
		Integer fResultCount = 0;
		
		Connection conn = null;
		Session session = null;
		CallableStatement call = null;
		Transaction tran = null;
		ResultSet rs = null;
		try{
			session = getSession();
			conn = super.getConnection();
			
			call = conn.prepareCall("{call PGSP_ADDT020521(?,?,?,?,?,?,?,?,?)}");
			
			for(int i = 0; i < v02052List.size();i++){
				Pgv02052 v02052Bean = v02052List.get(i);
				try{				
					tran = session.beginTransaction();
					call.registerOutParameter(1, OracleTypes.CURSOR);
					call.setString(2, v02052Bean.getLc());
					call.setString(3, v02052Bean.getSzqy());
					call.setDouble(4, v02052Bean.getXzxs());
					call.setString(5, v02052Bean.getCd00002Czr());
					call.setString(6, v02052Bean.getNote());
					call.setString(7, v02052Bean.getCd00001Ssgx());
					call.setString(8, v02052Bean.getFwlx());
					call.setString(9, v02052Bean.getZcs());
					call.execute();
					rs = (ResultSet)call.getObject(1);
					if(rs != null && rs.next()){
						if(rs.getInt("flag") == 0){
							sResultCount++;
							v02052Bean.setCd00001Fwlx(rs.getString("TFWLX"));
							v02052Bean.setCd00001Szqy(rs.getString("SZQYID"));
							v02052Bean.setImpErrorMsg("");
						
							tempList.add(v02052Bean);
							v02052.setOutList(tempList);
						}
					}
					tran.commit();
				}catch(Exception e){
					tran.rollback();
					sResultCount++;
					e.printStackTrace();
					v02052Bean.setImpErrorMsg(e.getMessage());
					tempList.add(v02052Bean);
					v02052.setOutList(tempList);
					continue;
				}finally{
					try{
						if(null!=rs) 
							rs.close();
					}catch (Exception e) {
						e.printStackTrace();
					}finally {
						rs = null;
					}
				}
			}
			
		}catch(Exception e){			
			e.printStackTrace();
		}finally{
			getFree(rs, call, conn, session);
			if(sResultCount == 0){
				resultValue = 2;
				//WriteLogImp(v02052List.get(0).getCd00001Ssgx(), "PGT02052", v02052List.get(0).getCd00002Czr(), "楼层修正导入成功");
			}else if(sResultCount > 0 && fResultCount == 0){
				resultValue = 1;
				//WriteLogImp(v02052List.get(0).getCd00001Ssgx(), "PGT02052", v02052List.get(0).getCd00002Czr(), "楼层修正导入有异常");
			}
			v02052.setOutFlag(resultValue);
		}
		return v02052;
	}

	
	//新导入
	@Override
	public Pgv02052 ImportExcelDataA(ArrayList<Pgv02052> v02052List)
			throws Exception {
		ArrayList<Pgv02052> tempList = new ArrayList<Pgv02052>();
		Pgv02052 v02052 = new Pgv02052();
		Integer resultValue = 0;
		Integer sResultCount = 0;
		Integer fResultCount = 0;
		
		Connection conn = null;
		Session session = null;
		CallableStatement call = null;
		Transaction tran = null;
		ResultSet rs = null;
		try{
			session = getSession();
			conn = super.getConnection();
			
			call = conn.prepareCall("{call PGSP_ADDT02052A1(?,?,?,?,?,?,?,?,?,?,?)}");
			
			for(int i = 0; i < v02052List.size();i++){
				Pgv02052 v02052Bean = v02052List.get(i);
				try{
					
					tran = session.beginTransaction();
					call.registerOutParameter(1, OracleTypes.CURSOR);
					call.setString(2, v02052Bean.getLc());
					call.setString(3, v02052Bean.getSzqy());
					call.setDouble(4, v02052Bean.getXzxs());
					call.setString(5, v02052Bean.getCd00002Czr());
					call.setString(6, v02052Bean.getNote());
					call.setString(7, v02052Bean.getCd00001Ssgx());
					call.setString(8, v02052Bean.getFwlx());
					call.setString(9, v02052Bean.getZcs());
					call.setString(10, v02052Bean.getXqnm());
					call.setString(11, v02052Bean.getDmh());
					call.execute();
					rs = (ResultSet)call.getObject(1);
					if(rs != null && rs.next()){
						if(rs.getInt("flag") == 0){
							sResultCount++;
							v02052Bean.setCd00001Fwlx(rs.getString("FWLX"));
							v02052Bean.setCd00001Szqy(rs.getString("SZQYID"));
							v02052Bean.setCd02050Xqdm(rs.getString("XQDM"));
							v02052Bean.setDmhId(rs.getString("TDMH"));
							v02052Bean.setImpErrorMsg("");
							tempList.add(v02052Bean);
							v02052.setOutList(tempList);
						}
					}
					tran.commit();
				}catch(Exception e){
					tran.rollback();
					sResultCount++;
					e.printStackTrace();
					v02052Bean.setImpErrorMsg(e.getMessage());
					tempList.add(v02052Bean);
					v02052.setOutList(tempList);
					continue;
				}finally{
					try{
						if(null!=rs) 
							rs.close();
					}catch (Exception e) {
						e.printStackTrace();
					}finally {
						rs = null;
					}
				}
			}
			
		}catch(Exception e){
			
			e.printStackTrace();
		}finally{
			getFree(rs, call, conn, session);
			if(sResultCount == 0){
				resultValue = 2;
				//WriteLogImp(v02052List.get(0).getCd00001Ssgx(), "PGT02052", v02052List.get(0).getCd00002Czr(), "楼层修正导入成功");
			}else if(sResultCount > 0 && fResultCount == 0){
				resultValue = 1;
				//WriteLogImp(v02052List.get(0).getCd00001Ssgx(), "PGT02052", v02052List.get(0).getCd00002Czr(), "楼层修正导入有异常");
			}
			v02052.setOutFlag(resultValue);
		}
		return v02052;
	}
	
	
	
	
	@Override
	public boolean GetSelDeleteCommand(Pgt02052 lcxs) throws Exception {
		boolean bResult = false;
		Connection conn = null;
		Session session = null;
		Transaction tran = null;
		CallableStatement call = null;
		try{
			session = getSession();
			tran = session.beginTransaction();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_DELT020521(?,?,?)}");
			call.setString(1, lcxs.getChkDel());
			call.setString(2, lcxs.getCd00002Czr());
			call.setString(3, lcxs.getCd00001Ssgx());
			call.execute();
			tran.commit();
			bResult = true;
		}catch(Exception e){
			tran.rollback();
			e.printStackTrace();
			throw e;
		}finally{
			getFree(call, conn, session);
		}
		return bResult;
	}
	
	
	
	@Override
	public boolean GetSelDeleteCommandA(Pgt02052 lcxs) throws Exception {
		boolean bResult = false;
		Connection conn = null;
		Session session = null;
		Transaction tran = null;
		CallableStatement call = null;
		try{
			session = getSession();
			tran = session.beginTransaction();
			conn = super.getConnection();
			call = conn.prepareCall("{call PGSP_DELT02052A1(?,?,?)}");
			call.setString(1, lcxs.getChkDel());
			call.setString(2, lcxs.getCd00002Czr());
			call.setString(3, lcxs.getCd00001Ssgx());
			call.execute();
			tran.commit();
			bResult = true;
		}catch(Exception e){
			tran.rollback();
			e.printStackTrace();
			throw e;
		}finally{
			getFree(call, conn, session);
		}
		return bResult;
	}
	
}
	

