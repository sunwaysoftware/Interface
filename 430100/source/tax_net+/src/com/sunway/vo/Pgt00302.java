package com.sunway.vo;

import java.math.BigDecimal;
import java.sql.Timestamp;
import java.util.Date;

/**
 * Pgt00302 entity. 
 * @author Lee
 */

public class Pgt00302 implements java.io.Serializable {

	private static final long serialVersionUID = -6178135605563305216L;
	private String fcid;
	private String id;
	private String cd00301Swid;
	private String cd00303Lfid;
	private String cd00001Fwlx;
	private String cd00001Jylx;
	private String cd00001Jzjg;
	private String cd00001Ghyt;
	private BigDecimal jzmj;
	private String cd00001Fwcx;
	private String cd00001Cgzk;
	private Short szlc;
	private String bwjfh;
	private BigDecimal jyjg;
	private Double dtgj;
	private Double tdsyqmj;
	private Double rjl;
	private Timestamp jysj;
	private String fdcdat;
	private Date upddate;
	private String cd00002Czr;
	private String note;
	private String cd00352Xqdm;
	private String cd00001Jzjg1;
	private Boolean ywdt;
	private Short zlc;
	private String fwtdzl;
	private String note1;
	private String cd00053Qtxzid;
	private String fwlxSc;
	private String jylxSc;
	private String jzjg;
	private String fwcx;
	private String cgzkSc;
	private String nsrmc;
	private Integer fcCount;
	private Date bgsj;
	private Integer bglx;
	private String cd00001Zjlx;
	private String zz;
	private String lxdh;
	private String yddh;
	private String note2;
	private String cd00001Ssgx;
	private String cd00002Pssd;
	private String fczh;
	private String qdh;
	private String cb;
	private String ghyt;
	private String hxjg;
	private String jcsj;
	private Date djrq;
	private String shr;
	private String cd00001Jtzk;
	private String cd00001Wyzk;
	private String cd00001Zxzk;
	private String jtzk;
	private String wyzk;
	private String zxzk;
	private String zcdzl;
	private String zcdzbm;
	private Byte   cd00352Xqzt;
	private String cd00001Szqy;
	private String cd00352Parentdm;
	private String cd00352Xqnm;
	private String lh;
	private String zhxz;
	private String clh;
	private Boolean ywjkc;
	private String cd00001csfzjlx;	
	
	private String txtCSFZJLX;   // 承受方证件类型
	private String txtCSFNSRMC;  //承受方姓名
	private String txtCSFZJHM;   //承受方证件号
	private String csflxdh;
	private String txtFCSLH;     //国土受理号
	private Integer oinsid;
	private Double yjg;
	private Double sbpgjg;
	private String roomid;
	private String fpid;
	private String spid;
	private String dfspid;
	private String ownRoomid;
	private Integer sfsyfc;
	private Double wsjs;
	private Date wsrq;
	private Date htqdsj;
	
	private String fwlx_sc;
	private String jylx_sc;
	private String cd00001Mfgjdm;	
	private String cd00001Gmfgjdm;	
	private String mfgjdm;
	private String gmfgjdm;
	private String sbh_zr;
	private String sbh_cs;
	private Integer nsrLx_zr;
	private Integer nsrLx_cs;
	private String dyh;
	private Integer isExistQmpg;
	//WS用
	private Boolean wsIsPg;
	private String wsPgMsg;
	private Double wsPgValue;
	//合同编号
	private String htbh;
	
	/** default constructor */
	public Pgt00302() {
	}

	/** full constructor */
	public Pgt00302(String fcid, String cd00301Swid, String cd00303Lfid,
			String cd00001Fwlx, String cd00001Jylx, String cd00001Jzjg,
			BigDecimal jzmj, String cd00001Fwcx, String cd00001Cgzk, Short szlc,
			String bwjfh, BigDecimal jyjg, Double dtgj, Double tdsyqmj, Double rjl,
			Timestamp jysj, String fdcdat, Date upddate, String cd00002Czr,
			String note, String txtCSFZJLX, String txtCSFNSRMC, String txtCSFZJHM, String txtFCSLH) {
		this.fcid = fcid;
		this.cd00301Swid = cd00301Swid;
		this.cd00303Lfid = cd00303Lfid;
		this.cd00001Fwlx = cd00001Fwlx;
		this.cd00001Jylx = cd00001Jylx;
		this.cd00001Jzjg = cd00001Jzjg;
		this.jzmj = jzmj;
		this.cd00001Fwcx = cd00001Fwcx;
		this.cd00001Cgzk = cd00001Cgzk;
		this.szlc = szlc;
		this.bwjfh = bwjfh;
		this.jyjg = jyjg;
		this.dtgj = dtgj;
		this.tdsyqmj = tdsyqmj;
		this.rjl = rjl;
		this.jysj = jysj;
		this.fdcdat = fdcdat;
		this.upddate = upddate;
		this.cd00002Czr = cd00002Czr;
		this.note = note;
		this.txtCSFZJLX=txtCSFZJLX;
		this.txtCSFNSRMC=txtCSFNSRMC;
		this.txtCSFZJHM=txtCSFZJHM;
		this.txtFCSLH=txtFCSLH;
		
	}
	
	/**
	 * @return the fczh
	 */
	public String getFczh() {
		return fczh;
	}

	/**
	 * @param fczh the fczh to set
	 */
	public void setFczh(String fczh) {
		this.fczh = fczh;
	}

	/**
	 * @return the qdh
	 */
	public String getQdh() {
		return qdh;
	}

	/**
	 * @param qdh the qdh to set
	 */
	public void setQdh(String qdh) {
		this.qdh = qdh;
	}

	/**
	 * @return the cb
	 */
	public String getCb() {
		return cb;
	}

	/**
	 * @param cb the cb to set
	 */
	public void setCb(String cb) {
		this.cb = cb;
	}

	/**
	 * @return the ghyt
	 */
	public String getGhyt() {
		return ghyt;
	}

	/**
	 * @param ghyt the ghyt to set
	 */
	public void setGhyt(String ghyt) {
		this.ghyt = ghyt;
	}

	/**
	 * @return the hxjg
	 */
	public String getHxjg() {
		return hxjg;
	}

	/**
	 * @param hxjg the hxjg to set
	 */
	public void setHxjg(String hxjg) {
		this.hxjg = hxjg;
	}

	/**
	 * @return the jcsj
	 */
	public String getJcsj() {
		return jcsj;
	}

	/**
	 * @param jcsj the jcsj to set
	 */
	public void setJcsj(String jcsj) {
		this.jcsj = jcsj;
	}

	/**
	 * @return the djrq
	 */
	public Date getDjrq() {
		return djrq;
	}

	/**
	 * @param djrq the djrq to set
	 */
	public void setDjrq(Date djrq) {
		this.djrq = djrq;
	}

	/**
	 * @return the shr
	 */
	public String getShr() {
		return shr;
	}

	/**
	 * @param shr the shr to set
	 */
	public void setShr(String shr) {
		this.shr = shr;
	}

	/**
	 * @return the cd00001Jtzk
	 */
	public String getCd00001Jtzk() {
		return cd00001Jtzk;
	}

	/**
	 * @param cd00001Jtzk the cd00001Jtzk to set
	 */
	public void setCd00001Jtzk(String cd00001Jtzk) {
		this.cd00001Jtzk = cd00001Jtzk;
	}

	/**
	 * @return the cd00001Wyzk
	 */
	public String getCd00001Wyzk() {
		return cd00001Wyzk;
	}

	/**
	 * @param cd00001Wyzk the cd00001Wyzk to set
	 */
	public void setCd00001Wyzk(String cd00001Wyzk) {
		this.cd00001Wyzk = cd00001Wyzk;
	}

	/**
	 * @return the cd00001Zxzk
	 */
	public String getCd00001Zxzk() {
		return cd00001Zxzk;
	}

	/**
	 * @param cd00001Zxzk the cd00001Zxzk to set
	 */
	public void setCd00001Zxzk(String cd00001Zxzk) {
		this.cd00001Zxzk = cd00001Zxzk;
	}

	/**
	 * @return the fcid
	 */
	public String getFcid() {
		return fcid;
	}
	/**
	 * @param fcid the fcid to set
	 */
	public void setFcid(String fcid) {
		this.fcid = fcid;
	}
	/**
	 * @return the cd00301Swid
	 */
	public String getCd00301Swid() {
		return cd00301Swid;
	}
	/**
	 * @param cd00301Swid the cd00301Swid to set
	 */
	public void setCd00301Swid(String cd00301Swid) {
		this.cd00301Swid = cd00301Swid;
	}
	/**
	 * @return the cd00303Lfid
	 */
	public String getCd00303Lfid() {
		return cd00303Lfid;
	}
	/**
	 * @param cd00303Lfid the cd00303Lfid to set
	 */
	public void setCd00303Lfid(String cd00303Lfid) {
		this.cd00303Lfid = cd00303Lfid;
	}
	/**
	 * @return the cd00001Fwlx
	 */
	public String getCd00001Fwlx() {
		return cd00001Fwlx;
	}
	/**
	 * @param cd00001Fwlx the cd00001Fwlx to set
	 */
	public void setCd00001Fwlx(String cd00001Fwlx) {
		this.cd00001Fwlx = cd00001Fwlx;
	}
	/**
	 * @return the cd00001Jylx
	 */
	public String getCd00001Jylx() {
		return cd00001Jylx;
	}
	/**
	 * @param cd00001Jylx the cd00001Jylx to set
	 */
	public void setCd00001Jylx(String cd00001Jylx) {
		this.cd00001Jylx = cd00001Jylx;
	}
	/**
	 * @return the cd00001Jzjg
	 */
	public String getCd00001Jzjg() {
		return cd00001Jzjg;
	}
	/**
	 * @param cd00001Jzjg the cd00001Jzjg to set
	 */
	public void setCd00001Jzjg(String cd00001Jzjg) {
		this.cd00001Jzjg = cd00001Jzjg;
	}
	/**
	 * @return the cd00001Fwcx
	 */
	public String getCd00001Fwcx() {
		return cd00001Fwcx;
	}
	/**
	 * @param cd00001Fwcx the cd00001Fwcx to set
	 */
	public void setCd00001Fwcx(String cd00001Fwcx) {
		this.cd00001Fwcx = cd00001Fwcx;
	}
	/**
	 * @return the cd00001Cgzk
	 */
	public String getCd00001Cgzk() {
		return cd00001Cgzk;
	}
	/**
	 * @param cd00001Cgzk the cd00001Cgzk to set
	 */
	public void setCd00001Cgzk(String cd00001Cgzk) {
		this.cd00001Cgzk = cd00001Cgzk;
	}
	/**
	 * @return the szlc
	 */
	public Short getSzlc() {
		return szlc;
	}
	/**
	 * @param szlc the szlc to set
	 */
	public void setSzlc(Short szlc) {
		this.szlc = szlc;
	}
	/**
	 * @return the bwjfh
	 */
	public String getBwjfh() {
		return bwjfh;
	}
	/**
	 * @param bwjfh the bwjfh to set
	 */
	public void setBwjfh(String bwjfh) {
		this.bwjfh = bwjfh;
	}
	/**
	 * @return the jyjg
	 */
	public BigDecimal getJyjg() {
		return jyjg;
	}
	/**
	 * @param jyjg the jyjg to set
	 */
	public void setJyjg(BigDecimal jyjg) {
		this.jyjg = jyjg;
	}
	/**
	 * @return the dtgj
	 */
	public Double getDtgj() {
		return dtgj;
	}
	/**
	 * @param dtgj the dtgj to set
	 */
	public void setDtgj(Double dtgj) {
		this.dtgj = dtgj;
	}
	/**
	 * @return the tdsyqmj
	 */
	public Double getTdsyqmj() {
		return tdsyqmj;
	}
	/**
	 * @param tdsyqmj the tdsyqmj to set
	 */
	public void setTdsyqmj(Double tdsyqmj) {
		this.tdsyqmj = tdsyqmj;
	}
	/**
	 * @return the rjl
	 */
	public Double getRjl() {
		return rjl;
	}
	/**
	 * @param rjl the rjl to set
	 */
	public void setRjl(Double rjl) {
		this.rjl = rjl;
	}
	/**
	 * @return the jysj
	 */
	public Timestamp getJysj() {
		return jysj;
	}
	/**
	 * @param jysj the jysj to set
	 */
	public void setJysj(Timestamp jysj) {
		this.jysj = jysj;
	}
	/**
	 * @return the fdcdat
	 */
	public String getFdcdat() {
		return fdcdat;
	}
	/**
	 * @param fdcdat the fdcdat to set
	 */
	public void setFdcdat(String fdcdat) {
		this.fdcdat = fdcdat;
	}
	/**
	 * @return the upddate
	 */
	public Date getUpddate() {
		return upddate;
	}
	/**
	 * @param upddate the upddate to set
	 */
	public void setUpddate(Date upddate) {
		this.upddate = upddate;
	}
	/**
	 * @return the cd00002Czr
	 */
	public String getCd00002Czr() {
		return cd00002Czr;
	}
	/**
	 * @param cd00002Czr the cd00002Czr to set
	 */
	public void setCd00002Czr(String cd00002Czr) {
		this.cd00002Czr = cd00002Czr;
	}
	/**
	 * @return the note
	 */
	public String getNote() {
		return note;
	}
	/**
	 * @param note the note to set
	 */
	public void setNote(String note) {
		this.note = note;
	}
	/**
	 * @return the cd00352Xqdm
	 */
	public String getCd00352Xqdm() {
		return cd00352Xqdm;
	}
	/**
	 * @param cd00352Xqdm the cd00352Xqdm to set
	 */
	public void setCd00352Xqdm(String cd00352Xqdm) {
		this.cd00352Xqdm = cd00352Xqdm;
	}
	/**
	 * @return the cd00001Jzjg1
	 */
	public String getCd00001Jzjg1() {
		return cd00001Jzjg1;
	}
	/**
	 * @param cd00001Jzjg1 the cd00001Jzjg1 to set
	 */
	public void setCd00001Jzjg1(String cd00001Jzjg1) {
		this.cd00001Jzjg1 = cd00001Jzjg1;
	}
	/**
	 * @return the ywdt
	 */
	public Boolean getYwdt() {
		return ywdt;
	}
	/**
	 * @param ywdt the ywdt to set
	 */
	public void setYwdt(Boolean ywdt) {
		this.ywdt = ywdt;
	}
	/**
	 * @return the zlc
	 */
	public Short getZlc() {
		return zlc;
	}
	/**
	 * @param zlc the zlc to set
	 */
	public void setZlc(Short zlc) {
		this.zlc = zlc;
	}
	/**
	 * @return the fwtdzl
	 */
	public String getFwtdzl() {
		return fwtdzl;
	}
	/**
	 * @param fwtdzl the fwtdzl to set
	 */
	public void setFwtdzl(String fwtdzl) {
		this.fwtdzl = fwtdzl;
	}
	/**
	 * @return the note1
	 */
	public String getNote1() {
		return note1;
	}
	/**
	 * @param note1 the note1 to set
	 */
	public void setNote1(String note1) {
		this.note1 = note1;
	}
	/**
	 * @return the cd00053Qtxzid
	 */
	public String getCd00053Qtxzid() {
		return cd00053Qtxzid;
	}

	/**
	 * @param cd00053Qtxzid the cd00053Qtxzid to set
	 */
	public void setCd00053Qtxzid(String cd00053Qtxzid) {
		this.cd00053Qtxzid = cd00053Qtxzid;
	}
	/**
	 * @return the fwlxSc
	 */
	public String getFwlxSc() {
		return fwlxSc;
	}
	/**
	 * @param fwlxSc the fwlxSc to set
	 */
	public void setFwlxSc(String fwlxSc) {
		this.fwlxSc = fwlxSc;
	}
	/**
	 * @return the jylxSc
	 */
	public String getJylxSc() {
		return jylxSc;
	}
	/**
	 * @param jylxSc the jylxSc to set
	 */
	public void setJylxSc(String jylxSc) {
		this.jylxSc = jylxSc;
	}
	/**
	 * @return the jzjg
	 */
	public String getJzjg() {
		return jzjg;
	}
	/**
	 * @param jzjg the jzjg to set
	 */
	public void setJzjg(String jzjg) {
		this.jzjg = jzjg;
	}
	/**
	 * @return the fwcx
	 */
	public String getFwcx() {
		return fwcx;
	}
	/**
	 * @param fwcx the fwcx to set
	 */
	public void setFwcx(String fwcx) {
		this.fwcx = fwcx;
	}
	/**
	 * @return the cgzkSc
	 */
	public String getCgzkSc() {
		return cgzkSc;
	}
	/**
	 * @param cgzkSc the cgzkSc to set
	 */
	public void setCgzkSc(String cgzkSc) {
		this.cgzkSc = cgzkSc;
	}
	/**
	 * @param nsrmc the nsrmc to set
	 */
	public void setNsrmc(String nsrmc) {
		this.nsrmc = nsrmc;
	}
	/**
	 * @return the nsrmc
	 */
	public String getNsrmc() {
		return nsrmc;
	}
	/**
	 * @return the fcCount
	 */
	public Integer getFcCount() {
		return fcCount;
	}
	/**
	 * @param fcCount the fcCount to set
	 */
	public void setFcCount(Integer fcCount) {
		this.fcCount = fcCount;
	}

	/**
	 * 初始化除SWID,NSRMC之外的屬性
	 */
	public void clear() {
		this.fcid = null;
		this.cd00303Lfid = null;
		this.cd00001Fwlx = null;
		this.cd00001Jylx = null;
		this.cd00001Jzjg = null;
		this.jzmj = null;
		this.cd00001Fwcx = null;
		this.cd00001Cgzk = null;
		this.szlc = null;
		this.bwjfh = null;
		this.jyjg = null;
		this.dtgj = null;
		this.tdsyqmj = null;
		this.rjl = null;
		this.jysj = null;
		this.fdcdat = null;
		this.upddate = null;
		this.cd00002Czr = null;
		this.note = null;
		this.cd00352Xqdm = null;
		this.cd00001Jzjg1 = null;
		this.ywdt = null;
		this.zlc = null;
		this.fwtdzl = null;
		this.note1 = null;
		this.cd00053Qtxzid = null;
		this.cd00001Ssgx = null;
		this.fwlxSc = null;
		this.jylxSc = null;
		this.jzjg = null;
		this.fwcx = null;
		this.cgzkSc = null;
		this.fcCount = null;
	}

	/**
	 * @param bgsj the bgsj to set
	 */
	public void setBgsj(Date bgsj) {
		this.bgsj = bgsj;
	}

	/**
	 * @return the bgsj
	 */
	public Date getBgsj() {
		return bgsj;
	}

	/**
	 * @param bglx the bglx to set
	 */
	public void setBglx(Integer bglx) {
		this.bglx = bglx;
	}

	/**
	 * @return the bglx
	 */
	public Integer getBglx() {
		return bglx;
	}

	/**
	 * @return the cd00001Zjlx
	 */
	public String getCd00001Zjlx() {
		return cd00001Zjlx;
	}

	/**
	 * @param cd00001Zjlx the cd00001Zjlx to set
	 */
	public void setCd00001Zjlx(String cd00001Zjlx) {
		this.cd00001Zjlx = cd00001Zjlx;
	}

	/**
	 * @return the zz
	 */
	public String getZz() {
		return zz;
	}

	/**
	 * @param zz the zz to set
	 */
	public void setZz(String zz) {
		this.zz = zz;
	}

	/**
	 * @return the lxdh
	 */
	public String getLxdh() {
		return lxdh;
	}

	/**
	 * @param lxdh the lxdh to set
	 */
	public void setLxdh(String lxdh) {
		this.lxdh = lxdh;
	}

	/**
	 * @return the yddh
	 */
	public String getYddh() {
		return yddh;
	}

	/**
	 * @param yddh the yddh to set
	 */
	public void setYddh(String yddh) {
		this.yddh = yddh;
	}

	/**
	 * @return the note2
	 */
	public String getNote2() {
		return note2;
	}

	/**
	 * @param note2 the note2 to set
	 */
	public void setNote2(String note2) {
		this.note2 = note2;
	}

	/**
	 * @return the cd00001Ssgx
	 */
	public String getCd00001Ssgx() {
		return cd00001Ssgx;
	}

	/**
	 * @param cd00001Ssgx the cd00001Ssgx to set
	 */
	public void setCd00001Ssgx(String cd00001Ssgx) {
		this.cd00001Ssgx = cd00001Ssgx;
	}

	/**
	 * @return the cd00002Pssd
	 */
	public String getCd00002Pssd() {
		return cd00002Pssd;
	}

	/**
	 * @param cd00002Pssd the cd00002Pssd to set
	 */
	public void setCd00002Pssd(String cd00002Pssd) {
		this.cd00002Pssd = cd00002Pssd;
	}

	/**
	 * @return the jtzk
	 */
	public String getJtzk() {
		return jtzk;
	}

	/**
	 * @param jtzk the jtzk to set
	 */
	public void setJtzk(String jtzk) {
		this.jtzk = jtzk;
	}

	/**
	 * @return the wyzk
	 */
	public String getWyzk() {
		return wyzk;
	}

	/**
	 * @param wyzk the wyzk to set
	 */
	public void setWyzk(String wyzk) {
		this.wyzk = wyzk;
	}

	/**
	 * @return the zxzk
	 */
	public String getZxzk() {
		return zxzk;
	}

	/**
	 * @param zxzk the zxzk to set
	 */
	public void setZxzk(String zxzk) {
		this.zxzk = zxzk;
	}

	/**
	 * @return the zcdzl
	 */
	public String getZcdzl() {
		return zcdzl;
	}

	/**
	 * @param zcdzl the zcdzl to set
	 */
	public void setZcdzl(String zcdzl) {
		this.zcdzl = zcdzl;
	}

	/**
	 * @return the zcdzbm
	 */
	public String getZcdzbm() {
		return zcdzbm;
	}

	/**
	 * @param zcdzbm the zcdzbm to set
	 */
	public void setZcdzbm(String zcdzbm) {
		this.zcdzbm = zcdzbm;
	}

	/**
	 * @return the wsIsPg
	 */
	public Boolean getWsIsPg() {
		return wsIsPg;
	}

	/**
	 * @param wsIsPg the wsIsPg to set
	 */
	public void setWsIsPg(Boolean wsIsPg) {
		this.wsIsPg = wsIsPg;
	}

	/**
	 * @return the wsPgMsg
	 */
	public String getWsPgMsg() {
		return wsPgMsg;
	}

	/**
	 * @param wsPgMsg the wsPgMsg to set
	 */
	public void setWsPgMsg(String wsPgMsg) {
		this.wsPgMsg = wsPgMsg;
	}

	/**
	 * @return the wsPgValue
	 */
	public Double getWsPgValue() {
		return wsPgValue;
	}

	/**
	 * @param wsPgValue the wsPgValue to set
	 */
	public void setWsPgValue(Double wsPgValue) {
		this.wsPgValue = wsPgValue;
	}

	/**
	 * @return the jzmj
	 */
	public BigDecimal getJzmj() {
		return jzmj;
	}

	/**
	 * @param jzmj the jzmj to set
	 */
	public void setJzmj(BigDecimal jzmj) {
		this.jzmj = jzmj;
	}

	/**
	 * @return the cd00352Xqzt
	 */
	public Byte getCd00352Xqzt() {
		return cd00352Xqzt;
	}

	/**
	 * @param cd00352Xqzt the cd00352Xqzt to set
	 */
	public void setCd00352Xqzt(Byte cd00352Xqzt) {
		this.cd00352Xqzt = cd00352Xqzt;
	}

	/**
	 * @return the cd00001Szqy
	 */
	public String getCd00001Szqy() {
		return cd00001Szqy;
	}

	/**
	 * @param cd00001Szqy the cd00001Szqy to set
	 */
	public void setCd00001Szqy(String cd00001Szqy) {
		this.cd00001Szqy = cd00001Szqy;
	}

	/**
	 * @return the cd00352Parentdm
	 */
	public String getCd00352Parentdm() {
		return cd00352Parentdm;
	}

	/**
	 * @param cd00352Parentdm the cd00352Parentdm to set
	 */
	public void setCd00352Parentdm(String cd00352Parentdm) {
		this.cd00352Parentdm = cd00352Parentdm;
	}

	/**
	 * @return the cd00352Xqnm
	 */
	public String getCd00352Xqnm() {
		return cd00352Xqnm;
	}

	/**
	 * @param cd00352Xqnm the cd00352Xqnm to set
	 */
	public void setCd00352Xqnm(String cd00352Xqnm) {
		this.cd00352Xqnm = cd00352Xqnm;
	}

	/**
	 * @return the lh
	 */
	public String getLh() {
		return lh;
	}

	/**
	 * @param lh the lh to set
	 */
	public void setLh(String lh) {
		this.lh = lh;
	}

	/**
	 * @param zhxz the zhxz to set
	 */
	public void setZhxz(String zhxz) {
		this.zhxz = zhxz;
	}

	/**
	 * @return the zhxz
	 */
	public String getZhxz() {
		return zhxz;
	}

	/**
	 * @return the clh
	 */
	public String getClh() {
		return clh;
	}

	/**
	 * @param clh the clh to set
	 */
	public void setClh(String clh) {
		this.clh = clh;
	}

	public Boolean getYwjkc() {
		return ywjkc;
	}

	public void setYwjkc(Boolean ywjkc) {
		this.ywjkc = ywjkc;
	}

	/**
	 * @return the cd00001Ghyt
	 */
	public String getCd00001Ghyt() {
		return cd00001Ghyt;
	}

	/**
	 * @param cd00001Ghyt the cd00001Ghyt to set
	 */
	public void setCd00001Ghyt(String cd00001Ghyt) {
		this.cd00001Ghyt = cd00001Ghyt;
	}

	
	/**新建字段
	 * @return the txtCSFZJLX
	 */
	public String getTxtCSFZJLX() {
		return txtCSFZJLX;
	}

	/**
	 * @param txtCSFZJLX the txtCSFZJLX to set
	 */
	public void setTxtCSFZJLX(String txtCSFZJLX) {
		this.txtCSFZJLX = txtCSFZJLX;
	}

	/**
	 * @return the txtCSFNSRMC
	 */
	public String getTxtCSFNSRMC() {
		return txtCSFNSRMC;
	}

	/**
	 * @param txtCSFNSRMC the txtCSFNSRMC to set
	 */
	public void setTxtCSFNSRMC(String txtCSFNSRMC) {
		this.txtCSFNSRMC = txtCSFNSRMC;
	}

	/**
	 * @return the txtCSFZJHM
	 */
	public String getTxtCSFZJHM() {
		return txtCSFZJHM;
	}

	/**
	 * @param txtCSFZJHM the txtCSFZJHM to set
	 */
	public void setTxtCSFZJHM(String txtCSFZJHM) {
		this.txtCSFZJHM = txtCSFZJHM;
	}

	/**
	 * @return the txtFCSLH
	 */
	public String getTxtFCSLH() {
		return txtFCSLH;
	}

	/**
	 * @param txtFCSLH the txtFCSLH to set
	 */
	public void setTxtFCSLH(String txtFCSLH) {
		this.txtFCSLH = txtFCSLH;
	}

	/**
	 * @return the cd00001csfzjlx
	 */
	public String getCd00001csfzjlx() {
		return cd00001csfzjlx;
	}

	/**
	 * @param cd00001csfzjlx the cd00001csfzjlx to set
	 */
	public void setCd00001csfzjlx(String cd00001csfzjlx) {
		this.cd00001csfzjlx = cd00001csfzjlx;
	}

	public Integer getOinsid() {
		return oinsid;
	}

	public void setOinsid(Integer oinsid) {
		this.oinsid = oinsid;
	}

	public Double getYjg() {
		return yjg;
	}

	public void setYjg(Double yjg) {
		this.yjg = yjg;
	}

	public Double getSbpgjg() {
		return sbpgjg;
	}

	public void setSbpgjg(Double sbpgjg) {
		this.sbpgjg = sbpgjg;
	}

	public String getRoomid() {
		return roomid;
	}

	public void setRoomid(String roomid) {
		this.roomid = roomid;
	}

	public void setFpid(String fpid) {
		this.fpid = fpid;
	}

	public String getFpid() {
		return fpid;
	}

	public void setSpid(String spid) {
		this.spid = spid;
	}

	public String getSpid() {
		return spid;
	}

	public void setDfspid(String dfspid) {
		this.dfspid = dfspid;
	}

	public String getDfspid() {
		return dfspid;
	}

	public String getOwnRoomid() {
		return ownRoomid;
	}

	public void setOwnRoomid(String ownRoomid) {
		this.ownRoomid = ownRoomid;
	}

	public Integer getSfsyfc() {
		return sfsyfc;
	}

	public void setSfsyfc(Integer sfsyfc) {
		this.sfsyfc = sfsyfc;
	}

	public String getFwlx_sc() {
		return fwlx_sc;
	}

	public void setFwlx_sc(String fwlxSc) {
		fwlx_sc = fwlxSc;
	}

	public String getJylx_sc() {
		return jylx_sc;
	}

	public void setJylx_sc(String jylxSc) {
		jylx_sc = jylxSc;
	}

	/**
	 * @return the dyh
	 */
	public String getDyh() {
		return dyh;
	}

	/**
	 * @param dyh the dyh to set
	 */
	public void setDyh(String dyh) {
		this.dyh = dyh;
	}

	/**
	 * @return the isExistQmpg
	 */
	public Integer getIsExistQmpg() {
		return isExistQmpg;
	}

	/**
	 * @param isExistQmpg the isExistQmpg to set
	 */
	public void setIsExistQmpg(Integer isExistQmpg) {
		this.isExistQmpg = isExistQmpg;
	}

	/**
	 * @return the csflxdh
	 */
	public String getCsflxdh() {
		return csflxdh;
	}

	/**
	 * @param csflxdh the csflxdh to set
	 */
	public void setCsflxdh(String csflxdh) {
		this.csflxdh = csflxdh;
	}

	/**
	 * @return the wsjs
	 */
	public Double getWsjs() {
		return wsjs;
	}

	/**
	 * @param wsjs the wsjs to set
	 */
	public void setWsjs(Double wsjs) {
		this.wsjs = wsjs;
	}

	/**
	 * @return the wsrq
	 */
	public Date getWsrq() {
		return wsrq;
	}

	/**
	 * @param wsrq the wsrq to set
	 */
	public void setWsrq(Date wsrq) {
		this.wsrq = wsrq;
	}

	public String getCd00001Mfgjdm() {
		return cd00001Mfgjdm;
	}

	public void setCd00001Mfgjdm(String cd00001Mfgjdm) {
		this.cd00001Mfgjdm = cd00001Mfgjdm;
	}

	public String getCd00001Gmfgjdm() {
		return cd00001Gmfgjdm;
	}

	public void setCd00001Gmfgjdm(String cd00001Gmfgjdm) {
		this.cd00001Gmfgjdm = cd00001Gmfgjdm;
	}

	public String getMfgjdm() {
		return mfgjdm;
	}

	public void setMfgjdm(String mfgjdm) {
		this.mfgjdm = mfgjdm;
	}

	public String getGmfgjdm() {
		return gmfgjdm;
	}

	public void setGmfgjdm(String gmfgjdm) {
		this.gmfgjdm = gmfgjdm;
	}

	/**
	 * @return the sbh_zr
	 */
	public String getSbh_zr() {
		return sbh_zr;
	}

	/**
	 * @param sbh_zr the sbh_zr to set
	 */
	public void setSbh_zr(String sbh_zr) {
		this.sbh_zr = sbh_zr;
	}

	/**
	 * @return the sbh_cs
	 */
	public String getSbh_cs() {
		return sbh_cs;
	}

	/**
	 * @param sbh_cs the sbh_cs to set
	 */
	public void setSbh_cs(String sbh_cs) {
		this.sbh_cs = sbh_cs;
	}

	/**
	 * @return the nsrLx_zr
	 */
	public Integer getNsrLx_zr() {
		return nsrLx_zr;
	}

	/**
	 * @param nsrLx_zr the nsrLx_zr to set
	 */
	public void setNsrLx_zr(Integer nsrLx_zr) {
		this.nsrLx_zr = nsrLx_zr;
	}

	/**
	 * @return the nsrLx_cs
	 */
	public Integer getNsrLx_cs() {
		return nsrLx_cs;
	}

	/**
	 * @param nsrLx_cs the nsrLx_cs to set
	 */
	public void setNsrLx_cs(Integer nsrLx_cs) {
		this.nsrLx_cs = nsrLx_cs;
	}

	/**
	 * @return the htqdsj
	 */
	public Date getHtqdsj() {
		return htqdsj;
	}

	/**
	 * @param htqdsj the htqdsj to set
	 */
	public void setHtqdsj(Date htqdsj) {
		this.htqdsj = htqdsj;
	}	

	public String getId() {
		return id;
	}

	public void setId(String id) {
		this.id = id;
	}

	/**
	 * @return the htbh
	 */
	public String getHtbh() {
		return htbh;
	}

	/**
	 * @param htbh the htbh to set
	 */
	public void setHtbh(String htbh) {
		this.htbh = htbh;
	}
	
}
