package com.sunway.vo;

import java.util.Date;

/**
 * PgvBb00004_b entity. 
 * @author Lee
 */

public class PgvBb00004_b implements java.io.Serializable {

	private static final long serialVersionUID = 715442752191132072L;
	private String cd00002Pssd;
	private Double cd12003Fcyz;
	private Double cbpgjg;
	private Double cbfcpgjg;
	private Double cbdcpgjg;
	private Double cbdcypgjg;
	private String cd00001Hy;
	private String cd00001Ssgx;
	private Date upddate;
	private String cd00002Czr;
	private String note;
	private Double zje1;
	private Double zjl1;
	private Double zje2;
	private Double zjl2;
	private Double pgjgY;
	private Double zje3;
	private Double zjl3;
	private String hy;
	private Integer bm;
	private Boolean sfssgx;

	/** default constructor */
	public PgvBb00004_b() {
	}

	/**
	 * @return the cd00002Pssd
	 */
	public String getCd00002Pssd() {
		return cd00002Pssd;
	}
	/**
	 * @param cd00002Pssd the cd00002Pssd to set
	 */
	public void setCd00002Pssd(String cd00002Pssd) {
		this.cd00002Pssd = cd00002Pssd;
	}
	/**
	 * @return the cd12003Fcyz
	 */
	public Double getCd12003Fcyz() {
		return cd12003Fcyz;
	}
	/**
	 * @param cd12003Fcyz the cd12003Fcyz to set
	 */
	public void setCd12003Fcyz(Double cd12003Fcyz) {
		this.cd12003Fcyz = cd12003Fcyz;
	}
	/**
	 * @return the cbpgjg
	 */
	public Double getCbpgjg() {
		return cbpgjg;
	}
	/**
	 * @param cbpgjg the cbpgjg to set
	 */
	public void setCbpgjg(Double cbpgjg) {
		this.cbpgjg = cbpgjg;
	}
	/**
	 * @return the cbfcpgjg
	 */
	public Double getCbfcpgjg() {
		return cbfcpgjg;
	}
	/**
	 * @param cbfcpgjg the cbfcpgjg to set
	 */
	public void setCbfcpgjg(Double cbfcpgjg) {
		this.cbfcpgjg = cbfcpgjg;
	}
	/**
	 * @return the cbdcpgjg
	 */
	public Double getCbdcpgjg() {
		return cbdcpgjg;
	}
	/**
	 * @param cbdcpgjg the cbdcpgjg to set
	 */
	public void setCbdcpgjg(Double cbdcpgjg) {
		this.cbdcpgjg = cbdcpgjg;
	}
	/**
	 * @return the cbdcypgjg
	 */
	public Double getCbdcypgjg() {
		return cbdcypgjg;
	}
	/**
	 * @param cbdcypgjg the cbdcypgjg to set
	 */
	public void setCbdcypgjg(Double cbdcypgjg) {
		this.cbdcypgjg = cbdcypgjg;
	}
	/**
	 * @return the cd00001Hy
	 */
	public String getCd00001Hy() {
		return cd00001Hy;
	}
	/**
	 * @param cd00001Hy the cd00001Hy to set
	 */
	public void setCd00001Hy(String cd00001Hy) {
		this.cd00001Hy = cd00001Hy;
	}
	/**
	 * @return the cd00001Ssgx
	 */
	public String getCd00001Ssgx() {
		return cd00001Ssgx;
	}
	/**
	 * @param cd00001Ssgx the cd00001Ssgx to set
	 */
	public void setCd00001Ssgx(String cd00001Ssgx) {
		this.cd00001Ssgx = cd00001Ssgx;
	}
	/**
	 * @return the upddate
	 */
	public Date getUpddate() {
		return upddate;
	}
	/**
	 * @param upddate the upddate to set
	 */
	public void setUpddate(Date upddate) {
		this.upddate = upddate;
	}
	/**
	 * @return the cd00002Czr
	 */
	public String getCd00002Czr() {
		return cd00002Czr;
	}
	/**
	 * @param cd00002Czr the cd00002Czr to set
	 */
	public void setCd00002Czr(String cd00002Czr) {
		this.cd00002Czr = cd00002Czr;
	}
	/**
	 * @return the note
	 */
	public String getNote() {
		return note;
	}
	/**
	 * @param note the note to set
	 */
	public void setNote(String note) {
		this.note = note;
	}
	/**
	 * @return the zje1
	 */
	public Double getZje1() {
		return zje1;
	}
	/**
	 * @param zje1 the zje1 to set
	 */
	public void setZje1(Double zje1) {
		this.zje1 = zje1;
	}
	/**
	 * @return the zjl1
	 */
	public Double getZjl1() {
		return zjl1;
	}
	/**
	 * @param zjl1 the zjl1 to set
	 */
	public void setZjl1(Double zjl1) {
		this.zjl1 = zjl1;
	}
	/**
	 * @return the zje2
	 */
	public Double getZje2() {
		return zje2;
	}
	/**
	 * @param zje2 the zje2 to set
	 */
	public void setZje2(Double zje2) {
		this.zje2 = zje2;
	}
	/**
	 * @return the zjl2
	 */
	public Double getZjl2() {
		return zjl2;
	}
	/**
	 * @param zjl2 the zjl2 to set
	 */
	public void setZjl2(Double zjl2) {
		this.zjl2 = zjl2;
	}
	/**
	 * @return the pgjgY
	 */
	public Double getPgjgY() {
		return pgjgY;
	}
	/**
	 * @param pgjgY the pgjgY to set
	 */
	public void setPgjgY(Double pgjgY) {
		this.pgjgY = pgjgY;
	}
	/**
	 * @return the zje3
	 */
	public Double getZje3() {
		return zje3;
	}
	/**
	 * @param zje3 the zje3 to set
	 */
	public void setZje3(Double zje3) {
		this.zje3 = zje3;
	}
	/**
	 * @return the zjl3
	 */
	public Double getZjl3() {
		return zjl3;
	}
	/**
	 * @param zjl3 the zjl3 to set
	 */
	public void setZjl3(Double zjl3) {
		this.zjl3 = zjl3;
	}
	/**
	 * @return the hy
	 */
	public String getHy() {
		return hy;
	}
	/**
	 * @param hy the hy to set
	 */
	public void setHy(String hy) {
		this.hy = hy;
	}
	/**
	 * @return the bm
	 */
	public Integer getBm() {
		return bm;
	}
	/**
	 * @param bm the bm to set
	 */
	public void setBm(Integer bm) {
		this.bm = bm;
	}
	/**
	 * @return the sfssgx
	 */
	public Boolean getSfssgx() {
		return sfssgx;
	}
	/**
	 * @param sfssgx the sfssgx to set
	 */
	public void setSfssgx(Boolean sfssgx) {
		this.sfssgx = sfssgx;
	}
}
