/**
 * 
 */
package com.sunway.vo;

import java.io.Serializable;
import java.math.BigDecimal;
import java.util.Date;

/**
 * @author Andy.Gao
 *
 */
public class BF00000 implements Serializable {
	
	private static final long serialVersionUID = -97644988959437975L;
	private String fcid;
	private String nsrmc;
	private String zjlx;
	private String swid;
	private String lxdh;
	private String zcdzl;
	private Double jzmj;
	private BigDecimal jyjg;
	private String szqy;
	private String xqnm;
	private Integer zlc;
	private String szlc;
	private String jylx;
	private String fwlx;
	private String jzjg;
	private String ywdt;
	private String fczh;
	private Date djrq;
	private String zhxz;
	private BigDecimal pgjg;
	private BigDecimal pgjgF;
	private String pgjgZh;
	private String pgjgZhF;
	private String czr;
	private String sysSsgx;
	private String csfmc;
	private String csfsflx;
	private String csfsfid;
	private String swjg;
	private String ssgx;
	private String csflxdh;
	private String jcnf;
	private BigDecimal qswsjs;
	private Date qswsrq;
	private String sbh_zr;
	private String htbh;
	
	/**
	 * @return the fcid
	 */
	public String getFcid() {
		return fcid;
	}
	/**
	 * @param fcid the fcid to set
	 */
	public void setFcid(String fcid) {
		this.fcid = fcid;
	}
	/**
	 * @return the nsrmc
	 */
	public String getNsrmc() {
		return nsrmc;
	}
	/**
	 * @param nsrmc the nsrmc to set
	 */
	public void setNsrmc(String nsrmc) {
		this.nsrmc = nsrmc;
	}
	/**
	 * @return the zjlx
	 */
	public String getZjlx() {
		return zjlx;
	}
	/**
	 * @param zjlx the zjlx to set
	 */
	public void setZjlx(String zjlx) {
		this.zjlx = zjlx;
	}
	/**
	 * @return the swid
	 */
	public String getSwid() {
		return swid;
	}
	/**
	 * @param swid the swid to set
	 */
	public void setSwid(String swid) {
		this.swid = swid;
	}
	/**
	 * @return the lxdh
	 */
	public String getLxdh() {
		return lxdh;
	}
	/**
	 * @param lxdh the lxdh to set
	 */
	public void setLxdh(String lxdh) {
		this.lxdh = lxdh;
	}
	/**
	 * @return the zcdzl
	 */
	public String getZcdzl() {
		return zcdzl;
	}
	/**
	 * @param zcdzl the zcdzl to set
	 */
	public void setZcdzl(String zcdzl) {
		this.zcdzl = zcdzl;
	}
	/**
	 * @return the jzmj
	 */
	public Double getJzmj() {
		return jzmj;
	}
	/**
	 * @param jzmj the jzmj to set
	 */
	public void setJzmj(Double jzmj) {
		this.jzmj = jzmj;
	}
	/**
	 * @return the jyjg
	 */
	public BigDecimal getJyjg() {
		return jyjg;
	}
	/**
	 * @param jyjg the jyjg to set
	 */
	public void setJyjg(BigDecimal jyjg) {
		this.jyjg = jyjg;
	}
	/**
	 * @return the szqy
	 */
	public String getSzqy() {
		return szqy;
	}
	/**
	 * @param szqy the szqy to set
	 */
	public void setSzqy(String szqy) {
		this.szqy = szqy;
	}
	/**
	 * @return the xqnm
	 */
	public String getXqnm() {
		return xqnm;
	}
	/**
	 * @param xqnm the xqnm to set
	 */
	public void setXqnm(String xqnm) {
		this.xqnm = xqnm;
	}
	/**
	 * @return the zlc
	 */
	public Integer getZlc() {
		return zlc;
	}
	/**
	 * @param zlc the zlc to set
	 */
	public void setZlc(Integer zlc) {
		this.zlc = zlc;
	}
	/**
	 * @return the szlc
	 */
	public String getSzlc() {
		return szlc;
	}
	/**
	 * @param szlc the szlc to set
	 */
	public void setSzlc(String szlc) {
		this.szlc = szlc;
	}
	/**
	 * @return the jylx
	 */
	public String getJylx() {
		return jylx;
	}
	/**
	 * @param jylx the jylx to set
	 */
	public void setJylx(String jylx) {
		this.jylx = jylx;
	}
	/**
	 * @return the fwlx
	 */
	public String getFwlx() {
		return fwlx;
	}
	/**
	 * @param fwlx the fwlx to set
	 */
	public void setFwlx(String fwlx) {
		this.fwlx = fwlx;
	}
	/**
	 * @return the jzjg
	 */
	public String getJzjg() {
		return jzjg;
	}
	/**
	 * @param jzjg the jzjg to set
	 */
	public void setJzjg(String jzjg) {
		this.jzjg = jzjg;
	}
	/**
	 * @return the ywdt
	 */
	public String getYwdt() {
		return ywdt;
	}
	/**
	 * @param ywdt the ywdt to set
	 */
	public void setYwdt(String ywdt) {
		this.ywdt = ywdt;
	}
	/**
	 * @return the fczh
	 */
	public String getFczh() {
		return fczh;
	}
	/**
	 * @param fczh the fczh to set
	 */
	public void setFczh(String fczh) {
		this.fczh = fczh;
	}
	/**
	 * @return the djrq
	 */
	public Date getDjrq() {
		return djrq;
	}
	/**
	 * @param djrq the djrq to set
	 */
	public void setDjrq(Date djrq) {
		this.djrq = djrq;
	}
	/**
	 * @return the zhxz
	 */
	public String getZhxz() {
		return zhxz;
	}
	/**
	 * @param zhxz the zhxz to set
	 */
	public void setZhxz(String zhxz) {
		this.zhxz = zhxz;
	}
	
	/**
	 * @return the czr
	 */
	public String getCzr() {
		return czr;
	}
	/**
	 * @param czr the czr to set
	 */
	public void setCzr(String czr) {
		this.czr = czr;
	}
	/**
	 * @return the sysSsgx
	 */
	public String getSysSsgx() {
		return sysSsgx;
	}
	/**
	 * @param sysSsgx the sysSsgx to set
	 */
	public void setSysSsgx(String sysSsgx) {
		this.sysSsgx = sysSsgx;
	}
	/**
	 * @param pgjgZh the pgjgZh to set
	 */
	public void setPgjgZh(String pgjgZh) {
		this.pgjgZh = pgjgZh;
	}
	/**
	 * @return the pgjgZh
	 */
	public String getPgjgZh() {
		return pgjgZh;
	}
	public String getCsfmc() {
		return csfmc;
	}
	public void setCsfmc(String csfmc) {
		this.csfmc = csfmc;
	}
	public String getCsfsflx() {
		return csfsflx;
	}
	public void setCsfsflx(String csfsflx) {
		this.csfsflx = csfsflx;
	}
	public String getCsfsfid() {
		return csfsfid;
	}
	public void setCsfsfid(String csfsfid) {
		this.csfsfid = csfsfid;
	}
	public String getSwjg() {
		return swjg;
	}
	public void setSwjg(String swjg) {
		this.swjg = swjg;
	}
	/**
	 * @return the pgjg
	 */
	public BigDecimal getPgjg() {
		return pgjg;
	}
	/**
	 * @param pgjg the pgjg to set
	 */
	public void setPgjg(BigDecimal pgjg) {
		this.pgjg = pgjg;
	}
	/**
	 * @return the ssgx
	 */
	public String getSsgx() {
		return ssgx;
	}
	/**
	 * @param ssgx the ssgx to set
	 */
	public void setSsgx(String ssgx) {
		this.ssgx = ssgx;
	}
	/**
	 * @return the csflxdh
	 */
	public String getCsflxdh() {
		return csflxdh;
	}
	/**
	 * @param csflxdh the csflxdh to set
	 */
	public void setCsflxdh(String csflxdh) {
		this.csflxdh = csflxdh;
	}
	/**
	 * @return the jcnf
	 */
	public String getJcnf() {
		return jcnf;
	}
	/**
	 * @param jcnf the jcnf to set
	 */
	public void setJcnf(String jcnf) {
		this.jcnf = jcnf;
	}
	/**
	 * @return the qswsjs
	 */
	public BigDecimal getQswsjs() {
		return qswsjs;
	}
	/**
	 * @param qswsjs the qswsjs to set
	 */
	public void setQswsjs(BigDecimal qswsjs) {
		this.qswsjs = qswsjs;
	}
	/**
	 * @return the qswsrq
	 */
	public Date getQswsrq() {
		return qswsrq;
	}
	/**
	 * @param qswsrq the qswsrq to set
	 */
	public void setQswsrq(Date qswsrq) {
		this.qswsrq = qswsrq;
	}
	public BigDecimal getPgjgF() {
		return pgjgF;
	}
	public void setPgjgF(BigDecimal pgjgF) {
		this.pgjgF = pgjgF;
	}
	public String getPgjgZhF() {
		return pgjgZhF;
	}
	public void setPgjgZhF(String pgjgZhF) {
		this.pgjgZhF = pgjgZhF;
	}
	public String getSbh_zr() {
		return sbh_zr;
	}
	public void setSbh_zr(String sbh_zr) {
		this.sbh_zr = sbh_zr;
	}
	/**
	 * @return the htbh
	 */
	public String getHtbh() {
		return htbh;
	}
	/**
	 * @param htbh the htbh to set
	 */
	public void setHtbh(String htbh) {
		this.htbh = htbh;
	}
	
}
