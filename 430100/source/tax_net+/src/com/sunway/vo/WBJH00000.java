/**
 * 
 */
package com.sunway.vo;

/**
 * 
 * 金三企业信息
 * @author Amani
 *
 */
public class WBJH00000 implements java.io.Serializable {

	private static final long serialVersionUID = -686846560364367047L;

	// 金三数据源配置信息
	private String gt3DriverClass;
	private String gt3DSUrl;
	private String gt3User;
	private String gt3Password;
	
	// 纳税人识别号
	private String nsrsbh;
	// 纳税人名称
	private String nsrmc;
	// 主管税务机关
	private String zgswjg;
	// 主管税务所（科、分局）
	private String zgsws;
	// 法定代表人姓名
	private String fddbr;
	// 法定代表人（负责人、业主）身份证件种类
	private String frzjlx;
	// 法定代表人（负责人、业主）身份证件种类
	private String frzjlxdm;
	// 法定代表人（负责人、业主）身份证件号码
	private String frzjhm;
	// 组织机构代码
	private String zzjgdm;
	// 纳税人状态
	private String nsrzt;
	// 生产经营地址
	private String scjydz;
	// 经营范围
	private String jyfw;
	// 注册地址
	private String zcdz;
	// 登记序号
	private String djxh;	
	//==========================
	private String rootid;
	
	private String infoid;
	
	private String wbjhinfoid;
	
	//==========================
	private String cd00001Sz;
	private Double ynse;
	private Double jmse;
	private Double ybtse;
	private String fwuuid;	
	
	/**
	 * @return the nsrsbh
	 */
	public String getNsrsbh() {
		return nsrsbh;
	}
	/**
	 * @param nsrsbh the nsrsbh to set
	 */
	public void setNsrsbh(String nsrsbh) {
		this.nsrsbh = nsrsbh;
	}
	/**
	 * @return the nsrmc
	 */
	public String getNsrmc() {
		return nsrmc;
	}
	/**
	 * @param nsrmc the nsrmc to set
	 */
	public void setNsrmc(String nsrmc) {
		this.nsrmc = nsrmc;
	}
	/**
	 * @return the zgswjg
	 */
	public String getZgswjg() {
		return zgswjg;
	}
	/**
	 * @param zgswjg the zgswjg to set
	 */
	public void setZgswjg(String zgswjg) {
		this.zgswjg = zgswjg;
	}
	/**
	 * @return the zgsws
	 */
	public String getZgsws() {
		return zgsws;
	}
	/**
	 * @param zgsws the zgsws to set
	 */
	public void setZgsws(String zgsws) {
		this.zgsws = zgsws;
	}
	/**
	 * @return the fddbr
	 */
	public String getFddbr() {
		return fddbr;
	}
	/**
	 * @param fddbr the fddbr to set
	 */
	public void setFddbr(String fddbr) {
		this.fddbr = fddbr;
	}
	/**
	 * @return the frzjlx
	 */
	public String getFrzjlx() {
		return frzjlx;
	}
	/**
	 * @param frzjlx the frzjlx to set
	 */
	public void setFrzjlx(String frzjlx) {
		this.frzjlx = frzjlx;
	}
	/**
	 * @return the frzjhm
	 */
	public String getFrzjhm() {
		return frzjhm;
	}
	/**
	 * @param frzjhm the frzjhm to set
	 */
	public void setFrzjhm(String frzjhm) {
		this.frzjhm = frzjhm;
	}
	/**
	 * @return the zzjgdm
	 */
	public String getZzjgdm() {
		return zzjgdm;
	}
	/**
	 * @param zzjgdm the zzjgdm to set
	 */
	public void setZzjgdm(String zzjgdm) {
		this.zzjgdm = zzjgdm;
	}
	/**
	 * @return the nsrzt
	 */
	public String getNsrzt() {
		return nsrzt;
	}
	/**
	 * @param nsrzt the nsrzt to set
	 */
	public void setNsrzt(String nsrzt) {
		this.nsrzt = nsrzt;
	}
	/**
	 * @return the scjydz
	 */
	public String getScjydz() {
		return scjydz;
	}
	/**
	 * @param scjydz the scjydz to set
	 */
	public void setScjydz(String scjydz) {
		this.scjydz = scjydz;
	}
	/**
	 * @return the jyfw
	 */
	public String getJyfw() {
		return jyfw;
	}
	/**
	 * @param jyfw the jyfw to set
	 */
	public void setJyfw(String jyfw) {
		this.jyfw = jyfw;
	}
	/**
	 * @return the zcdz
	 */
	public String getZcdz() {
		return zcdz;
	}
	/**
	 * @param zcdz the zcdz to set
	 */
	public void setZcdz(String zcdz) {
		this.zcdz = zcdz;
	}
	/**
	 * @return the gt3DriverClass
	 */
	public String getGt3DriverClass() {
		return gt3DriverClass;
	}
	/**
	 * @param gt3DriverClass the gt3DriverClass to set
	 */
	public void setGt3DriverClass(String gt3DriverClass) {
		this.gt3DriverClass = gt3DriverClass;
	}
	/**
	 * @return the gt3DSUrl
	 */
	public String getGt3DSUrl() {
		return gt3DSUrl;
	}
	/**
	 * @param gt3dsUrl the gt3DSUrl to set
	 */
	public void setGt3DSUrl(String gt3dsUrl) {
		gt3DSUrl = gt3dsUrl;
	}
	/**
	 * @return the gt3User
	 */
	public String getGt3User() {
		return gt3User;
	}
	/**
	 * @param gt3User the gt3User to set
	 */
	public void setGt3User(String gt3User) {
		this.gt3User = gt3User;
	}
	/**
	 * @return the gt3Password
	 */
	public String getGt3Password() {
		return gt3Password;
	}
	/**
	 * @param gt3Password the gt3Password to set
	 */
	public void setGt3Password(String gt3Password) {
		this.gt3Password = gt3Password;
	}
	public String getDjxh() {
		return djxh;
	}
	public void setDjxh(String djxh) {
		this.djxh = djxh;
	}
	public String getRootid() {
		return rootid;
	}
	public void setRootid(String rootid) {
		this.rootid = rootid;
	}
	public String getInfoid() {
		return infoid;
	}
	public void setInfoid(String infoid) {
		this.infoid = infoid;
	}
	public String getWbjhinfoid() {
		return wbjhinfoid;
	}
	public void setWbjhinfoid(String wbjhinfoid) {
		this.wbjhinfoid = wbjhinfoid;
	}
	public String getCd00001Sz() {
		return cd00001Sz;
	}
	public void setCd00001Sz(String cd00001Sz) {
		this.cd00001Sz = cd00001Sz;
	}
	public Double getYnse() {
		return ynse;
	}
	public void setYnse(Double ynse) {
		this.ynse = ynse;
	}
	public Double getJmse() {
		return jmse;
	}
	public void setJmse(Double jmse) {
		this.jmse = jmse;
	}
	public Double getYbtse() {
		return ybtse;
	}
	public void setYbtse(Double ybtse) {
		this.ybtse = ybtse;
	}
	public String getFwuuid() {
		return fwuuid;
	}
	public void setFwuuid(String fwuuid) {
		this.fwuuid = fwuuid;
	}
	public String getFrzjlxdm() {
		return frzjlxdm;
	}
	public void setFrzjlxdm(String frzjlxdm) {
		this.frzjlxdm = frzjlxdm;
	}	
	
	

}
