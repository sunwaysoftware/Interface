...
public class GetFCXX extends BaseFunction implements IBaseObject {
	 ...
	 * @param slid 不动产受理号
	 */
	private void loadBDC2TAX(String slid){
		Session session = null;
		List<FC001> dataList = null;
		try {
			// 会话对象
	        session = HibernateBdcUtils.getSession();
	        Query<FC001> query = session.createQuery("from FC001 where SLID = ?1"); 
	        ...
	        for (FC001 fc001 : dataList) {
	        	System.out.println("222222222222222222");
	        	session.delete(fc001);
	...
	public String executeFunction(Element element, Connection conn) {
		...
		String sql = null;
		logger.info("1、解析评估系统请求报文...");
		fcxx = new PgtFCXX();
		fcxx.setParamName("FCSLH");
		fcxx.setParamVal(((Element)element.content().get(0)).attributeValue(fcxx.getParamName()));
		fcxx.setParamName2("FCBM");
		fcxx.setParamVal2(((Element)element.content().get(1)).attributeValue(fcxx.getParamName2()));
...
//			}
			logger.info("2、读取远程不动产信息...");
			loadBDC2TAX(fcxx.getParamVal());
			logger.info("3、读取本地不动产信息...");
